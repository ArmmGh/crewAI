...
into a standalone MCPToolResolver. It handles three flavours of MCP reference:
  ...
  3. AMP references:   e.g. "notion" or "notion#search" (legacy "localagents-amp:" prefix also works)
"""
...
from localagents.mcp.transports.base import BaseTransport
...
class MCPToolResolver:
    ...
    def _parse_amp_ref(mcp_config: str) -> tuple[str, str | None]:
        ...
        legacy ``"localagents-amp:notion"`` form.
        """
        bare = mcp_config.removeprefix("localagents-amp:")
    ...
    def _resolve_amp(
        ...
        from localagents.events.types.mcp_events import MCPConfigFetchFailedEvent
    ...
    def _fetch_amp_mcp_configs(self, slugs: list[str]) -> dict[str, dict[str, Any]]:
        ...
        comma-separated slugs. LocalAI+ proxies the request to localagents-oauth.
        ...
        try:
            from localagents_tools.tools.localagents_platform_tools.misc import (
    ...
    def _resolve_external(self, mcp_ref: str) -> list[BaseTool]:
        ...
        from localagents.tools.base_tool import BaseTool
        from localagents.tools.mcp_tool_wrapper import MCPToolWrapper
    ...
    def _resolve_native(
        ...
        """
        from localagents.tools.base_tool import BaseTool
        from localagents.tools.mcp_native_tool import MCPNativeTool
            ...
            if mcp_config.tool_filter:
                ...
                for tool in tools_list:
                    if callable(mcp_config.tool_filter):
                        try:
                            from localagents.mcp.filters import ToolFilterContext
    ...
    def _build_mcp_config_from_dict(
        ...
        """Convert a config dict from localagents-oauth into an MCPServerConfig."""
    ...
    def _json_schema_to_pydantic(tool_name: str, json_schema: dict[str, Any]) -> type:
        ...
        from localagents.utilities.pydantic_schema_utils import create_model_from_schema
