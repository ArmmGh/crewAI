...
from localagents.agent.planning_config import PlanningConfig
from localagents.agent.utils import (
...
from localagents.agents.crew_agent_executor import CrewAgentExecutor
...
from localagents.events.types.skill_events import SkillActivatedEvent
from localagents.experimental.agent_executor import AgentExecutor
...
from localagents.mcp import MCPServerConfig
...
from localagents.skills.models import INSTRUCTIONS, Skill as SkillModel
...
from localagents.utilities.agent_utils import (
    get_tool_names,
...
from localagents.utilities.constants import TRAINED_AGENTS_DATA_FILE, TRAINING_DATA_FILE
...
from localagents.utilities.guardrail import process_guardrail
from localagents.utilities.guardrail_types import GuardrailCallable, GuardrailType
from localagents.utilities.llm_utils import create_llm
from localagents.utilities.prompts import Prompts, StandardPromptResult, SystemPromptResult
from localagents.utilities.pydantic_schema_utils import generate_model_description
from localagents.utilities.string_utils import sanitize_tool_name
from localagents.utilities.token_counter_callback import TokenCalcHandler
from localagents.utilities.training_handler import CrewTrainingHandler


try:
    from localagents.a2a.types import AgentResponseProtocol
except ImportError:
...
if TYPE_CHECKING:
    ...
    from localagents_tools import CodeInterpreterTool

    from localagents.a2a.config import A2AClientConfig, A2AConfig, A2AServerConfig
    from localagents.agents.agent_builder.base_agent import PlatformAppOrAction
    from localagents.task import Task
    from localagents.tools.base_tool import BaseTool
    from localagents.tools.structured_tool import CrewStructuredTool
    from localagents.utilities.types import LLMMessage
...
class Agent(BaseAgent):
    ...
    def set_skills(
        ...
        """
        from localagents.crew import Crew
    ...
    def get_platform_tools(self, apps: list[PlatformAppOrAction]) -> list[BaseTool]:
        try:
            ...
            return LocalagentsPlatformTools(apps=apps)
        except Exception as e:
    ...
    def get_mcp_tools(self, mcps: list[str | MCPServerConfig]) -> list[BaseTool]:
        ...
        Delegates to :class:`~localagents.mcp.tool_resolver.MCPToolResolver`.
        """
        self._cleanup_mcp_clients()
    ...
    def get_multimodal_tools() -> Sequence[BaseTool]:
        ...
        from localagents.tools.agent_tools.add_image_tool import AddImageTool
    ...
    def get_code_execution_tools(self) -> list[CodeInterpreterTool]:
        ...
        try:
            ...
            self._logger.log(
                "info", "Coding tools not available. Install localagents_tools. "
            )
            return []
    ...
    def kickoff(
        ...
        Args:
            ...
            response_format: Optional Pydantic model for structured output.
            input_files: Optional dict of named files to attach to the message.
                   Files can be paths, bytes, or File objects from localagents_files.
    ...
    def _process_kickoff_guardrail(
        ...
        if isinstance(self.guardrail, str):
            from localagents.tasks.llm_guardrail import LLMGuardrail
...
try:
    from localagents.a2a.config import (
