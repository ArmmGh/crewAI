...
from localagents.cli.add_crew_to_flow import add_crew_to_flow
from localagents.cli.authentication.main import AuthenticationCommand
from localagents.cli.config import Settings
from localagents.cli.create_crew import create_crew
from localagents.cli.create_flow import create_flow
from localagents.cli.crew_chat import run_chat
from localagents.cli.deploy.main import DeployCommand
from localagents.cli.enterprise.main import EnterpriseConfigureCommand
from localagents.cli.evaluate_crew import evaluate_crew
from localagents.cli.install_crew import install_crew
from localagents.cli.kickoff_flow import kickoff_flow
from localagents.cli.organization.main import OrganizationCommand
from localagents.cli.plot_flow import plot_flow
from localagents.cli.replay_from_task import replay_task_command
from localagents.cli.reset_memories_command import reset_memories_command
from localagents.cli.run_crew import run_crew
from localagents.cli.settings.main import SettingsCommand
from localagents.cli.shared.token_manager import TokenManager
from localagents.cli.tools.main import ToolCommand
from localagents.cli.train_crew import train_crew
from localagents.cli.triggers.main import TriggersCommand
from localagents.cli.update_crew import update_crew
from localagents.cli.utils import build_env_with_tool_repository_credentials, read_toml
from localagents.memory.storage.kickoff_task_outputs_storage import (
    KickoffTaskOutputsSQLiteStorage,
)


@click.group()
@click.version_option(get_version("localagents"))
def localagents() -> None:
    """Top-level command group for localagents."""
...
def version(tools: bool) -> None:
    """Show the installed version of LocalAI (localagents package)."""
    try:
        localagents_version = get_version("localagents")
    except Exception:
        localagents_version = "unknown version"
    click.echo(f"localai version: {localagents_version}")

    if tools:
        try:
            tools_version = get_version("localagents")
...
@click.option(
    ...
    type=str,
    default=None,
    help="Path to LanceDB memory directory. If omitted, uses ./.localagents/memory.",
)
@click.option(
    "--embedder-provider",
    type=str,
...
def memory(
    ...
    try:
        from localagents.cli.memory_tui import MemoryTUI
    except ImportError as exc:
        click.echo(
            "Textual is required for the memory TUI but could not be imported. "
            "Try reinstalling localagents or: pip install textual"
        )
...
@localagents.group()
def triggers() -> None:
    """Trigger related commands. Use 'localagents triggers list' to see available triggers, or 'localagents triggers run app_slug/trigger_slug' to execute."""
...
@localagents.group(invoke_without_command=True)
def org() -> None:
...
@org.command()
def current() -> None:
    """Show current organization when 'localagents org' is called without subcommands."""
    org_command = OrganizationCommand()
    org_command.current()
...
def env_view() -> None:
    ...
    # Check LOCALAGENTS_TRACING_ENABLED
    localagents_tracing = os.getenv("LOCALAGENTS_TRACING_ENABLED", "")
    if localagents_tracing:
        table.add_row(
            "LOCALAGENTS_TRACING_ENABLED",
            localagents_tracing,
            "Environment/Shell",
        )
    else:
    ...
    localagents_testing = os.getenv("LOCALAGENTS_TESTING", "")
    if localagents_testing:
        table.add_row("LOCALAGENTS_TESTING", localagents_testing, "Environment/Shell")

    localagents_user_id = os.getenv("LOCALAGENTS_USER_ID", "")
    if localagents_user_id:
        table.add_row("LOCALAGENTS_USER_ID", localagents_user_id, "Environment/Shell")

    localagents_org_id = os.getenv("LOCALAGENTS_ORG_ID", "")
    if localagents_org_id:
        table.add_row("LOCALAGENTS_ORG_ID", localagents_org_id, "Environment/Shell")
    ...
    if env_file_exists:
        console.print(
            "\n[dim]💡 Tip: To enable tracing via .env, add: LOCALAGENTS_TRACING_ENABLED=true[/dim]"
        )
    else:
        console.print(
            "\n[dim]💡 Tip: Create a .env file in your project root and add: LOCALAGENTS_TRACING_ENABLED=true[/dim]"
        )
    console.print()
...
def traces_enable() -> None:
    ...
    from localagents.events.listeners.tracing.utils import update_user_data
    ...
    panel = Panel(
        ...
        "Use 'localagents traces disable' to turn off trace collection.",
...
def traces_disable() -> None:
    ...
    panel = Panel(
        ...
        "Use 'localagents traces enable' to turn trace collection back on.",
...
def traces_status() -> None:
    ...
    from localagents.events.listeners.tracing.utils import (
        _load_user_data,
        is_tracing_enabled,
    )
    ...
    env_enabled = os.getenv("LOCALAGENTS_TRACING_ENABLED", "false")
    table.add_row("LOCALAGENTS_TRACING_ENABLED", env_enabled)
