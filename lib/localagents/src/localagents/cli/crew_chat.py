...
from localagents.cli.utils import read_toml
...
from localagents.types.crew_chat import ChatInputField, ChatInputs
...
def check_conversational_crews_version(
    localagents_version: str, pyproject_data: dict[str, Any]
) -> bool:
    """
    Check if the installed localAI version supports conversational crews.

    Args:
        localagents_version: The current version of localAI.
    ...
    """
    try:
        if version.parse(localagents_version) < version.parse(MIN_REQUIRED_VERSION):
            click.secho(
                "You are using an older version of localAI that doesn't support conversational crews. "
                "Run 'uv upgrade localagents' to get the latest version.",
                fg="red",
            )
            return False
...
def run_chat() -> None:
    ...
    if not check_conversational_crews_version(localagents_version, pyproject_data):
        return
