...
from localagents.cli import git
from localagents.cli.command import BaseCommand, PlusAPIMixin
from localagents.cli.config import Settings
from localagents.cli.constants import DEFAULT_LOCALAGENTS_ENTERPRISE_URL
from localagents.cli.utils import (
...
from localagents.events.listeners.tracing.utils import get_user_id
...
class ToolCommand(BaseCommand, PlusAPIMixin):
    ...
    def login(self) -> None:
        ...
        if login_response.status_code != 200:
            console.print(
                "Authentication failed. Verify if the currently active organization can access the tool repository, and run 'localagents login' again.",
                style="bold red",
            )
            try:
                console.print(
