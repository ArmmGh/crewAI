...
from localagents.cli.constants import (
    ...
    LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_PROVIDER,
    DEFAULT_LOCALAGENTS_ENTERPRISE_URL,
)
...
DEFAULT_CONFIG_PATH = Path.home() / ".config" / "localagents" / "settings.json"
...
def get_writable_config_path() -> Path | None:
    ...
    Tries in order:
    1. Default: ~/.config/localagents/settings.json
    2. Temp directory: /tmp/localagents_settings.json (or OS equivalent)
    3. Current directory: ./localagents_settings.json
    ...
    fallback_paths = [
        ...
        Path(tempfile.gettempdir()) / "localagents_settings.json",  # Temporary directory
        Path.cwd() / "localagents_settings.json",  # Current working directory
    ]

    for config_path in fallback_paths:
        try:
            config_path.parent.mkdir(parents=True, exist_ok=True)
            test_file = config_path.parent / ".localagents_write_test"
            try:
...
DEFAULT_CLI_SETTINGS: dict[str, Any] = {
    "enterprise_base_url": DEFAULT_LOCALAGENTS_ENTERPRISE_URL,
    "oauth2_provider": LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_PROVIDER,
    "oauth2_audience": LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_AUDIENCE,
    "oauth2_client_id": LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_CLIENT_ID,
    "oauth2_domain": LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN,
    "oauth2_extra": {},
}
...
class Settings(BaseModel):
    enterprise_base_url: str | None = Field(
        ...
        description="Base URL of the LocalAI AMP instance",
    )
