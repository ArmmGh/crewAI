...
def _get_cache_file() -> Path:
    ...
    """
    cache_dir = Path(appdirs.user_cache_dir("localagents"))
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir / "version_cache.json"


def get_localagents_version() -> str:
    """Get the version number of LocalAI running the CLI."""
    return importlib.metadata.version("localagents")
...
def is_current_version_yanked() -> tuple[bool, str]:
    ...
    if cache_file.exists():
        try:
            cache_data = json.loads(cache_file.read_text())
            if _is_cache_valid(cache_data) and "current_version" in cache_data:
                current = get_localagents_version()
