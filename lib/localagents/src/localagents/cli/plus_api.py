...
from localagents.cli.constants import DEFAULT_LOCALAGENTS_ENTERPRISE_URL
...
class PlusAPI:
    ...
    TOOLS_RESOURCE = "/localagents_plus/api/v1/tools"
    ORGANIZATIONS_RESOURCE = "/localagents_plus/api/v1/me/organizations"
    CREWS_RESOURCE = "/localagents_plus/api/v1/crews"
    AGENTS_RESOURCE = "/localagents_plus/api/v1/agents"
    TRACING_RESOURCE = "/localagents_plus/api/v1/tracing"
    EPHEMERAL_TRACING_RESOURCE = "/localagents_plus/api/v1/tracing/ephemeral"
    INTEGRATIONS_RESOURCE = "/localagents_plus/api/v1/integrations"

    def __init__(self, api_key: str | None = None) -> None:
        self.api_key = api_key
        self.headers = {
            "Content-Type": "application/json",
            "User-Agent": f"LocalAI-CLI/{get_localagents_version()}",
            "X-Localagents-Version": get_localagents_version(),
        }
        if api_key:
            self.headers["Authorization"] = f"Bearer {api_key}"
        settings = Settings()
        if settings.org_uuid:
            self.headers["X-Localagents-Organization-Id"] = settings.org_uuid

        self.base_url = (
            os.getenv("LOCALAGENTS_PLUS_URL")
            or str(settings.enterprise_base_url)
            or DEFAULT_LOCALAGENTS_ENTERPRISE_URL
        )
