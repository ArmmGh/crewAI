...
> **Auto-generated** by `localagents create`. This file helps AI coding assistants
...
> [LocalAI repository](https://github.com/localAgentsInc/localAgents).
...
1. **Check the installed version**: Run `uv run python -c "import localagents; print(localagents.__version__)"` to get the exact version in use.
2. **Check PyPI for latest**: Fetch `https://pypi.org/pypi/localagents/json` to see the latest available version. If the installed version is behind, inform the user.
3. **Read the changelog**: Fetch `https://docs.localagents.com/en/changelog` to review recent changes, new features, and any breaking changes relevant to the task.
4. **Consult the relevant docs page**: Before implementing a feature (e.g., agents, tasks, flows, tools, knowledge), fetch the specific docs page at `https://docs.localagents.com/en/concepts/<feature>` to get the current API.
...
- ❌ Passing raw OpenAI client objects → ✅ Use `localagents.LLM` wrapper

### How to verify you're using current patterns:
1. You ran the version check and docs lookup steps above before writing code
2. All LLM references use `localagents.LLM` or string shorthand (`"openai/gpt-4o"`)
3. All tool imports come from `localagents.tools` or `localagents_tools`
...
# Project scaffolding
localagents create crew <name> --skip_provider   # New crew project
localagents create flow <name> --skip_provider  # New flow project

# Running
localagents run                  # Run crew or flow (auto-detects from pyproject.toml)
localagents flow kickoff         # Legacy flow execution

# Testing & training
localagents test                           # Test crew (default: 2 iterations, gpt-4o-mini)
localagents test -n 5 -m gpt-4o           # Custom iterations and model
localagents train -n 5 -f training.json   # Train crew

# Memory management
localagents reset-memories -a              # Reset all memories
localagents reset-memories -s              # Short-term only
localagents reset-memories -l              # Long-term only
localagents reset-memories -e              # Entity only
localagents reset-memories -kn             # Knowledge only
localagents reset-memories -akn            # Agent knowledge only

# Debugging
localagents log-tasks-outputs              # Show latest task outputs
localagents replay -t <task_id>            # Replay from specific task

# Interactive
localagents chat                           # Interactive session (requires chat_llm in crew.py)

# Visualization
localagents flow plot                      # Generate flow diagram HTML

# Deployment to LocalAI AMP
localagents login                          # Authenticate with AMP
localagents deploy create                  # Create new deployment
localagents deploy push                    # Push code updates
localagents deploy status                  # Check deployment status
localagents deploy logs                    # View deployment logs
localagents deploy list                    # List all deployments
localagents deploy remove <id>             # Delete a deployment
```
...
### LLM Configuration
**IMPORTANT**: Always use `localagents.LLM` LLM class.
...
# Full configuration with localagents.LLM
llm = LLM(
...
### Conditional Routing
```python
from localagents.flow.flow import Flow, listen, router, start
...
```python
from localagents.flow.flow import or_, and_
...
```python
from localagents.agent import Agent
...
```python
from localagents.flow.flow import Flow, listen, start
from localagents.flow.human_feedback import human_feedback
...
### State Persistence
```python
from localagents.flow.flow import persist
...
### Built-in Tools (install with `uv add localagents-tools`)
...
Always check https://docs.localagents.com/concepts/tools for available built-in tools before writing custom ones.
...
```python
from localagents.knowledge.source.string_knowledge_source import StringKnowledgeSource
from localagents.knowledge.source.pdf_knowledge_source import PDFKnowledgeSource
...
```python
from localagents.events import BaseEventListener, CrewKickoffStartedEvent

class MyListener(BaseEventListener):
    def __init__(self):
        super().__init__()

    def setup_listeners(self, localagents_event_bus):
        @localagents_event_bus.on(CrewKickoffStartedEvent)
...
- `pyproject.toml` has `[tool.localagents]` with correct type (`"crew"` or `"flow"`)
...
localagents deploy create

# Monitor (first deploy takes 10-15 min)
localagents deploy status
localagents deploy logs

# Manage deployments
localagents deploy list              # List all deployments
localagents deploy push              # Push code updates
localagents deploy remove <id>       # Delete deployment
```

### Web Interface Deployment
1. Push code to GitHub
2. Log into https://app.localagents.com
...
Get a Personal Access Token from app.localagents.com → Settings → Account → Personal Access Token.
Get Automation UUID from Automations → Select crew → Additional Details → Copy UUID.

```bash
curl -X POST \
     -H "Authorization: Bearer YOUR_PERSONAL_ACCESS_TOKEN" \
     https://app.localagents.com/localagents_plus/api/v1/crews/YOUR-AUTOMATION-UUID/deploy
```
  ...
  deploy:
    ...
    steps:
      - name: Trigger LocalAI Redeployment
        run: |
          curl -X POST \
               -H "Authorization: Bearer ${{ secrets.LOCALAGENTS_PAT }}" \
               https://app.localagents.com/localagents_plus/api/v1/crews/${{ secrets.LOCALAGENTS_AUTOMATION_UUID }}/deploy
```
...
### Installation
```bash
uv tool install localagents        # Install LocalAI CLI
uv tool list                  # Verify installation
localagents create crew my_crew --skip_provider   # Scaffold a new project
localagents install                # Install project dependencies
localagents run                    # Execute
```
...
10. **Test with** `localagents test` to evaluate crew performance across iterations
...
- **Using `ChatOpenAI()`** — Always use `localagents.LLM` or string shorthand like `"openai/gpt-4o"`
...
- Not installing tools package: `uv add localagents-tools`
