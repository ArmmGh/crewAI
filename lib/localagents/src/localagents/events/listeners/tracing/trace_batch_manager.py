...
class TraceBatch:
    ...
    version: str = field(default_factory=get_localagents_version)
        ...
        try:
            payload = {
                ...
                "execution_context": {
                    ...
                    "localagents_version": self.current_batch.version,
    ...
    def _finalize_backend_batch(self, events_count: int = 0) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                console = Console()
                settings = Settings()
                base_url = settings.enterprise_base_url or DEFAULT_LOCALAGENTS_ENTERPRISE_URL
                return_link = (
                    f"{base_url}/localagents_plus/trace_batches/{self.trace_batch_id}"
                    if not self.is_current_batch_ephemeral and access_code is None
                    else f"{base_url}/localagents_plus/ephemeral_trace_batches/{self.trace_batch_id}?access_code={access_code}"
                )
