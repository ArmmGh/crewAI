...
from localagents.cli.version import is_current_version_yanked, is_newer_version_available
...
class ConsoleFormatter:
    ...
    def _show_version_update_message_if_needed(self) -> None:
        ...
        if os.getenv("LOCALAGENTS_DISABLE_VERSION_CHECK", "").lower() in ("true", "1"):
            return
...
To update, run: uv sync --upgrade-package localagents"""
...
• Run: localagents traces enable"""
    ...
    def handle_a2a_response_received(
        ...
        localagents_agent_role = self._pending_a2a_agent_role or agent_role or "User"
        ...
        if message_content:
            content.append(f"{localagents_agent_role}: ", style="blue bold")
            msg_preview = (
                message_content[:200] + "..."
