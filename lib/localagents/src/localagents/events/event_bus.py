...
from localagents.events.base_events import BaseEvent, get_next_emission_sequence
...
from localagents.events.handler_graph import build_execution_plan
from localagents.events.types.event_bus_types import (
    AsyncHandler,
    AsyncHandlerSet,
    ExecutionPlan,
    Handler,
...
from localagents.events.types.llm_events import LLMStreamChunkEvent
from localagents.events.utils.console_formatter import ConsoleFormatter
from localagents.events.utils.handlers import is_async_handler, is_call_handler_safe
from localagents.utilities.rw_lock import RWLock
...
class LocalAgentsEventsBus:
    ...
    def __new__(cls) -> Self:
        ...
        Returns:
            The singleton LocalAgentsEventsBus instance
        """
    ...
    def _initialize(self) -> None:
        ...
        self._sync_executor = ThreadPoolExecutor(
            max_workers=10,
            thread_name_prefix="LocalAgentsSyncHandler",
        )
    ...
    def on(
        ...
        Example:
            >>> from localagents.events import localagents_event_bus, Depends
            >>> from localagents.events.types.llm_events import LLMCallStartedEvent
            >>>
            >>> @localagents_event_bus.on(LLMCallStartedEvent)
            >>> def setup_context(source, event):
            ...     print("Setting up context")
            >>>
            >>> @localagents_event_bus.on(LLMCallStartedEvent, depends_on=Depends(setup_context))
    ...
    def _call_handlers(
        ...
        if errors:
            for handler, error in errors:
                self._console.print(
                    f"[LocalAgentsEventsBus] Sync handler error in {handler.__name__}: {error}"
                )
    ...
    async def _acall_handlers(
        ...
        for handler, result in zip(handlers, results, strict=False):
            if isinstance(result, Exception):
                self._console.print(
                    f"[LocalAgentsEventsBus] Async handler error in {getattr(handler, '__name__', handler)}: {result}"
                )
    ...
    def emit(self, source: Any, event: BaseEvent) -> Future[None] | None:
        ...
        Example:
            >>> future = localagents_event_bus.emit(source, event)
            >>> if future:
        ...
        with self._rwlock.r_locked():
            if self._shutting_down:
                self._console.print(
                    "[LocalAgentsEventsBus] Warning: Attempted to emit event during shutdown. Ignoring."
                )
                return None
    ...
    def flush(self, timeout: float | None = 30.0) -> bool:
        ...
        for error in errors:
            self._console.print(
                f"[LocalAgentsEventsBus] Handler exception during flush: {error}"
            )
    ...
    def scoped_handlers(self) -> Generator[None, Any, None]:
        ...
        Example:
            ...
            >>> from localagents.events.event_types import CrewKickoffStartedEvent
            >>> with localagents_event_bus.scoped_handlers():
            ...
    ...
    def shutdown(self, wait: bool = True) -> None:
        ...
        if wait:
            ...
            try:
                ...
                self._console.print(f"[LocalAgentsEventsBus] Error waiting for tasks: {e}")
        else:
...
localagents_event_bus: Final[LocalAgentsEventsBus] = LocalAgentsEventsBus()

atexit.register(localagents_event_bus.shutdown)
