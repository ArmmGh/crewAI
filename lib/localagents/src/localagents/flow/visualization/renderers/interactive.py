...
from localagents.flow.visualization.builder import calculate_execution_paths
from localagents.flow.visualization.types import FlowStructure
...
def render_interactive(
    ...
    for name, metadata in dag["nodes"].items():
        ...
        type_badge_bg: str = (
            LOCALAGENTS_ORANGE if node_type in ["start", "router"] else DARK_GRAY
        )
        ...
        if metadata.get("condition_type"):
            ...
            if condition == "AND":
                ...
                condition_color = LOCALAGENTS_ORANGE
            elif condition == "IF":
        ...
        if metadata.get("router_paths"):
            ...
            paths_items = "".join(
                [
                    f'<li style="margin: 3px 0;"><code style="background: rgba(255,90,80,0.08); padding: 2px 6px; border-radius: 3px; font-size: 10px; color: {LOCALAGENTS_ORANGE}; border: 1px solid rgba(255,90,80,0.2); font-weight: 600;">{p}</code></li>'
                    for p in paths
                ]
            )
            title_parts.append(f"""
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="localagents_flow_"))
    ...
    css_content = css_content.replace("'{{ LOCALAGENTS_ORANGE }}'", LOCALAGENTS_ORANGE)
    ...
    js_content = js_content.replace("{{ LOCALAGENTS_ORANGE }}", LOCALAGENTS_ORANGE)
    ...
    html_content = template.render(
        LOCALAGENTS_ORANGE=LOCALAGENTS_ORANGE,
        DARK_GRAY=DARK_GRAY,
        WHITE=WHITE,
        GRAY=GRAY,
        BG_DARK=BG_DARK,
