...
class NodeRenderer {
  ...
  applyShadow(ctx, node, glowIntensity = 0, nodeOpacity = 1.0) {
    if (glowIntensity > 0.001) {
      ...
      // Use LocalAgents orange for hover glow in both themes
      const glowR = 255;
      const glowG = 90;
      const glowB = 80;
...
class DrawerManager {
  ...
  renderConditionTree(condition, depth = 0) {
    ...
    if (condition.type === "AND" || condition.type === "OR") {
      ...
      const color = conditionType === "AND" ? "{{ LOCALAGENTS_ORANGE }}" : "var(--text-secondary)";
      ...
      if (stringChildren.length > 0) {
        ...
        children += grouped.map((group) => {
          if (group.items.length === 1) {
            ...
            const groupId = group.items.join(',');
            const groupColor = conditionType === "AND" ? "{{ LOCALAGENTS_ORANGE }}" : "var(--text-secondary)";
  ...
  renderMetadata(metadata) {
    ...
    if (metadata.condition_type) {
      const conditionColors = {
        AND: { color: "{{ LOCALAGENTS_ORANGE }}", bg: "rgba(255,90,80,0.12)" },
        IF: { color: "{{ LOCALAGENTS_ORANGE }}", bg: "rgba(255,90,80,0.18)" },
    ...
    if (metadata.router_paths && metadata.router_paths.length > 0) {
      ...
      metadataContent += `
                <div class="drawer-section">
                    ...
                    <ul class="drawer-list">
                        ${uniqueRouterPaths.map((p) => `<li><span class="drawer-code-link" data-node-id="${p}" style="color: {{ LOCALAGENTS_ORANGE }}; border-color: rgba(255,90,80,0.3);">${p}</span></li>`).join("")}
                    </ul>
                </div>
            `;
    }
