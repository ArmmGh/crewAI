...
from localagents.flow.constants import AND_CONDITION, OR_CONDITION
from localagents.flow.flow_context import current_flow_id, current_flow_request_id
from localagents.flow.flow_wrappers import (
    FlowCondition,
    FlowConditions,
    FlowMethod,
    ListenMethod,
...
from localagents.flow.input_provider import InputProvider
from localagents.flow.persistence.base import FlowPersistence
from localagents.flow.types import (
    FlowExecutionData,
    FlowMethodName,
    InputHistoryEntry,
    PendingListenerKey,
)
from localagents.flow.utils import (
...
if TYPE_CHECKING:
    ...
    from localagents.flow.async_feedback.types import PendingFeedbackContext
    from localagents.flow.human_feedback import HumanFeedbackResult
    from localagents.llms.base_llm import BaseLLM

from localagents.flow.visualization import build_flow_structure, render_interactive
from localagents.types.streaming import CrewStreamingOutput, FlowStreamingOutput
...
class Flow(Generic[T], metaclass=FlowMeta):
    ...
    def from_pending(
        ...
        **kwargs: Any,
        ...
        if persistence is None:
            from localagents.flow.persistence import SQLiteFlowPersistence
    ...
    async def resume_async(self, feedback: str = "") -> Any:
        ...
        from localagents.flow.human_feedback import _deserialize_llm_from_context
        ...
        if method is not None:
            ...
            if live_llm is not None:
                from localagents.llms.base_llm import BaseLLM as BaseLLMClass
        ...
        try:
            ...
            from localagents.flow.async_feedback.types import HumanFeedbackPending
    ...
    async def kickoff_async(
        ...
        try:
            ...
            if not self.suppress_flow_events:
                future = localagents_event_bus.emit(
                    self,
                    FlowStartedEvent(
                        type="flow_started",
                        flow_name=self.name or self.__class__.__name__,
    ...
    async def _execute_start_method(self, start_method_name: FlowMethodName) -> None:
        ...
        Note:
            ...
            - Automatically injects localagents_trigger_payload if available in flow inputs
        """
    ...
    def _inject_trigger_payload_for_start_method(
        ...
        def prepare_kwargs(
            *args: Any, **kwargs: Any
            ...
            trigger_payload = inputs.get("localagents_trigger_payload")

            sig = inspect.signature(original_method)
            accepts_trigger_payload = "localagents_trigger_payload" in sig.parameters

            if trigger_payload is not None and accepts_trigger_payload:
                kwargs["localagents_trigger_payload"] = trigger_payload
            elif trigger_payload is not None:
                self._log_flow_event(
                    f"Trigger payload available but {original_method.__name__} doesn't accept localagents_trigger_payload parameter"
                )
            return args, kwargs
    ...
    async def _execute_method(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            from localagents.flow.flow_context import current_flow_method_name
            ...
            if not self.suppress_flow_events:
                ...
                future = localagents_event_bus.emit(self, finished_event)
                if future:
                    self._event_futures.append(future)
    ...
    def _resolve_input_provider(self) -> InputProvider:
        ...
        """
        from localagents.flow.async_feedback.providers import ConsoleProvider
    ...
    def ask(
        ...
        from localagents.flow.flow_context import current_flow_method_name
        from localagents.flow.input_provider import InputResponse
    ...
    def _request_human_feedback(
        ...
        """
        from localagents.events.event_listener import event_listener
        from localagents.events.types.flow_events import (
    ...
    def plot(self, filename: str = "localagents_flow.html", show: bool = True) -> str:
        """Create interactive HTML visualization of Flow structure.

        Args:
            filename: Output HTML filename (default: "localagents_flow.html").
