...
from localagents.tools.tool_calling import InstructorToolCalling, ToolCalling
from localagents.utilities.agent_utils import (
    get_tool_names,
    render_text_description_and_args,
)
from localagents.utilities.converter import Converter
...
class ToolUsage:
    ...
    async def _ause(
        ...
        if self.agent:
            ...
            localagents_event_bus.emit(self, ToolUsageStartedEvent(**event_data))
    ...
    def on_tool_use_finished(
        ...
        if self.task:
            event_data["task_id"] = str(self.task.id)
            event_data["task_name"] = self.task.name or self.task.description
        localagents_event_bus.emit(self, ToolUsageFinishedEvent(**event_data))
