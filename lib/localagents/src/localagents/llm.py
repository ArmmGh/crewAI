...
from localagents.llms.base_llm import BaseLLM, get_current_call_id, llm_call_context
from localagents.llms.constants import (
    ANTHROPIC_MODELS,
    AZURE_MODELS,
    BEDROCK_MODELS,
    GEMINI_MODELS,
...
from localagents.utilities import InternalInstructor
from localagents.utilities.exceptions.context_window_exceeding_exception import (
    LLMContextLengthExceededError,
)
from localagents.utilities.logger_utils import suppress_warnings
from localagents.utilities.string_utils import sanitize_tool_name


try:
    from localagents_files import aformat_multimodal_content, format_multimodal_content
...
if TYPE_CHECKING:
    from localagents.agent.core import Agent
    from localagents.llms.hooks.base import BaseInterceptor
    from localagents.llms.providers.anthropic.completion import AnthropicThinkingConfig
...
class LLM(BaseLLM):
    ...
    def __new__(cls, model: str, is_litellm: bool = False, **kwargs: Any) -> LLM:
        ...
        if not LITELLM_AVAILABLE:
            ...
            error_msg = (
                ...
                f"uv add 'localagents[litellm]'\n"
                f"or\n"
                f"pip install litellm\n\n"
                f"For more details, see: "
                f"https://docs.localagents.com/en/learn/llm-connections"
            )
    ...
    def _get_native_provider(cls, provider: str) -> type | None:
        ...
        if provider == "openai":
            from localagents.llms.providers.openai.completion import OpenAICompletion

            return OpenAICompletion

        if provider == "anthropic" or provider == "claude":
            from localagents.llms.providers.anthropic.completion import (
                AnthropicCompletion,
            )
        ...
        if provider == "azure" or provider == "azure_openai":
            from localagents.llms.providers.azure.completion import AzureCompletion

            return AzureCompletion

        if provider == "google" or provider == "gemini":
            from localagents.llms.providers.gemini.completion import GeminiCompletion

            return GeminiCompletion

        if provider == "bedrock":
            from localagents.llms.providers.bedrock.completion import BedrockCompletion
