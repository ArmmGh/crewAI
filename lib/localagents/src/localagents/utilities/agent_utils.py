...
from localagents.agents.constants import FINAL_ANSWER_AND_PARSABLE_ACTION_ERROR_MESSAGE
...
from localagents.utilities.printer import ColoredText, Printer
...
if TYPE_CHECKING:
    ...
    from localagents.lite_agent import LiteAgent
...
def parse_tools(tools: list[BaseTool]) -> list[CrewStructuredTool]:
    ...
    for tool in tools:
        if isinstance(tool, LocalAgentsTool):
...
def handle_context_length(
    ...
    if respect_context_window:
        ...
        if verbose:
            printer.print(
                content="Context length exceeded. Consider using smaller text or RAG tools from localagents_tools.",
                color="red",
            )
        raise SystemExit(
            "Context length exceeded and user opted not to summarize. Consider using smaller text or RAG tools from localagents_tools."
        )
...
def load_agent_from_repository(from_repository: str) -> dict[str, Any]:
    ...
    if from_repository:
        ...
        if callable(_create_plus_client_hook):
            ...
            from localagents.cli.authentication.token import get_auth_token
        ...
        if response.status_code == 404:
            raise AgentRepositoryError(
                ...
                f"\nIf you are using the wrong organization, switch to the correct one using `localagents org switch <org_id>` command.",
            )
...
def execute_single_native_tool_call(
    ...
    *,
    ...
    from localagents.hooks.tool_hooks import (
        ToolCallHookContext,
...
def _setup_before_llm_call_hooks(
    ...
    if executor_context and executor_context.before_llm_call_hooks:
        from localagents.hooks.llm_hooks import LLMCallHookContext
