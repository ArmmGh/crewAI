...
from localagents.types.streaming import (
    CrewStreamingOutput,
    FlowStreamingOutput,
    StreamChunk,
    StreamChunkType,
...
def _unregister_handler(handler: Callable[[Any, BaseEvent], None]) -> None:
    ...
    with localagents_event_bus._rwlock.w_locked():
        handlers: frozenset[Callable[[Any, BaseEvent], None]] = (
            localagents_event_bus._sync_handlers.get(LLMStreamChunkEvent, frozenset())
        )
        localagents_event_bus._sync_handlers[LLMStreamChunkEvent] = handlers - {handler}
...
def create_streaming_state(
    ...
    localagents_event_bus.register_handler(LLMStreamChunkEvent, handler)
