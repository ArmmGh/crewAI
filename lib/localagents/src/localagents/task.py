...
from localagents.context import reset_current_task_id, set_current_task_id
from localagents.core.providers.content_processor import process_content
...
from localagents.security import Fingerprint, SecurityConfig
...
from localagents.utilities.config import process_config
from localagents.utilities.constants import NOT_SPECIFIED, _NotSpecified
from localagents.utilities.converter import Converter, convert_to_model
from localagents.utilities.file_store import (
    clear_task_files,
    get_all_files,
    store_task_files,
)
...
try:
    from localagents_files import FileInput, FilePath, get_supported_content_types
...
from localagents.utilities.guardrail import (
    process_guardrail,
)
from localagents.utilities.guardrail_types import (
    GuardrailCallable,
    GuardrailType,
    GuardrailsType,
)
...
from localagents.utilities.string_utils import interpolate_only
...
class Task(BaseModel):
    ...
    Attributes:
        ...
        allow_localagents_trigger_context: Optional flag to control localagents_trigger_payload injection.
    ...
    allow_localagents_trigger_context: bool | None = Field(
        default=None,
        description="Whether this task should append 'Trigger Payload: {localagents_trigger_payload}' to the task description when localagents_trigger_payload exists in crew inputs.",
    )
    ...
    async def _aexecute_core(
        ...
        try:
            ...
            localagents_event_bus.emit(self, TaskStartedEvent(context=context, task=self))
            ...
            self.end_time = datetime.datetime.now()
            localagents_event_bus.emit(self, TaskFailedEvent(error=str(e), task=self))
...
- Use ```language for code blocks"""
    ...
    def _save_file(self, result: dict[str, Any] | str | Any) -> None:
        ...
        Note:
            ...
            from the localagents_tools package:
                pip install 'localagents[tools]'
                from localagents_tools import FileWriterTool
        ...
        Raises:
            ...
            RuntimeError: If there is an error writing to the file. For cross-platform
                compatibility, especially on Windows, use FileWriterTool from localagents_tools
                package.
        """
        ...
        filewriter_recommendation = (
            ...
            "use FileWriterTool from localagents_tools package."
        )
    ...
    def _store_input_files(self) -> None:
        ...
        if not HAS_LOCALAGENTS_FILES or not self.input_files:
            return
