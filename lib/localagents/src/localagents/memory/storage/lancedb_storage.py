...
from localagents.memory.types import MemoryRecord, ScopeInfo
    ...
    def __init__(
        ...
        Args:
            path: Directory path for the LanceDB database. Defaults to
                  ``$LOCALAGENTS_STORAGE_DIR/memory`` if the env var is set,
        ...
        """
        if path is None:
            storage_dir = os.environ.get("LOCALAGENTS_STORAGE_DIR")
            if storage_dir:
