...
def test_tool_selection_error_event_direct():
    ...
    @localagents_event_bus.on(ToolSelectionErrorEvent)
...
def test_tool_validate_input_error_event():
    ...
    with (
        ...
        @localagents_event_bus.on(ToolValidateInputErrorEvent)
        def event_handler(source, event):
            with condition:
                received_events.append(event)
                condition.notify()
...
def test_tool_error_does_not_emit_finished_event():
    from localagents.tools.tool_calling import ToolCalling
