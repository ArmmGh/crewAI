...
from localagents.llm import CONTEXT_WINDOW_USAGE_RATIO, LLM
...
def test_validate_call_params_supported():
    ...
    with patch("localagents.llm.supports_response_schema", return_value=True):
        llm = LLM(
...
def test_validate_call_params_not_supported():
    ...
    with patch("localagents.llm.supports_response_schema", return_value=False):
        llm = LLM(
...
def test_context_window_validation():
    ...
    with pytest.raises(ValueError) as excinfo:
        with patch.dict(
            "localagents.llm.LLM_CONTEXT_WINDOW_SIZES",
...
def mock_emit() -> MagicMock:
    ...
    with patch.object(LocalAgentsEventsBus, "emit") as mock_emit:
        yield mock_emit
...
def test_native_provider_raises_error_when_supported_but_fails():
    ...
    with patch("localagents.llm.SUPPORTED_NATIVE_PROVIDERS", ["openai"]):
        with patch("localagents.llm.LLM._get_native_provider") as mock_get_native:
...
def test_native_provider_falls_back_to_litellm_when_not_in_supported_list():
    ...
    with patch("localagents.llm.SUPPORTED_NATIVE_PROVIDERS", ["openai", "anthropic"]):
