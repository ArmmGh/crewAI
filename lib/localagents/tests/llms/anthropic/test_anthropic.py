...
def test_anthropic_completion_module_is_imported():
    ...
    """
    module_name = "localagents.llms.providers.anthropic.completion"
...
def test_native_anthropic_raises_error_when_initialization_fails():
    ...
    with patch('localagents.llm.LLM._get_native_provider') as mock_get_provider:
...
def test_anthropic_raises_error_when_model_not_supported():
    ...
    with patch('localagents.llms.providers.anthropic.completion.Anthropic') as mock_anthropic_class:
...
def test_anthropic_tool_conversion():
    ...
    anthropic_tools = llm._convert_tools_for_interference(localagents_tools)
...
def test_tool_search_true_injects_bm25_and_defer_loading():
    ...
    params = llm._prepare_completion_params(
        formatted_messages, system_message, localagents_tools
    )
...
def test_tool_search_regex_config():
    ...
    from localagents.llms.providers.anthropic.completion import AnthropicToolSearchConfig
