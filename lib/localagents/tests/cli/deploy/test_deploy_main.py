...
from localagents.cli.utils import parse_toml


class TestDeployCommand(unittest.TestCase):
    @patch("localagents.cli.command.get_auth_token")
    @patch("localagents.cli.deploy.main.get_project_name")
    @patch("localagents.cli.command.PlusAPI")
    ...
    @patch("localagents.cli.deploy.main.DeployCommand._display_deployment_info")
    ...
    @patch("localagents.cli.deploy.main.fetch_and_json_env_file")
    @patch("localagents.cli.deploy.main.git.Repository.origin_url")
    ...
    def test_parse_toml_python_311_plus(self):
        ...
        python = "^3.11"
        localagents = { extras = ["tools"], version = ">=0.51.0,<1.0.0" }
        """
    ...
    @patch(
        ...
        dependencies = ["localagents"]
        """,
    )
    def test_get_project_name_python_310(self, mock_open):
        from localagents.cli.utils import get_project_name
    ...
    def test_get_localagents_version(self):
        from localagents.cli.version import get_localagents_version

        assert isinstance(get_localagents_version(), str)
