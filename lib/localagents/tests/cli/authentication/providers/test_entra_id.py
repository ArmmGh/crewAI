...
from localagents.cli.authentication.providers.entra_id import EntraIdProvider
...
class TestEntraIdProvider:
    ...
    def setup_method(self):
        self.valid_settings = Oauth2Settings(
            ...
            extra={
                "scope": "openid profile email api://localagents-cli-dev/read"
            }
        )
    ...
    def test_get_oauth_scopes(self):
        settings = Oauth2Settings(
            ...
            audience="test-audience",
            extra={
                "scope": "api://localagents-cli-dev/read"
            }
        )
        provider = EntraIdProvider(settings)
        assert provider.get_oauth_scopes() == ["openid", "profile", "email", "api://localagents-cli-dev/read"]
    ...
    def test_get_oauth_scopes_with_multiple_custom_scopes(self):
        settings = Oauth2Settings(  
            ...
            audience="test-audience",
            extra={
                "scope": "api://localagents-cli-dev/read api://localagents-cli-dev/write custom-scope1 custom-scope2"
            }
        )
        provider = EntraIdProvider(settings)
        assert provider.get_oauth_scopes() == ["openid", "profile", "email", "api://localagents-cli-dev/read", "api://localagents-cli-dev/write", "custom-scope1", "custom-scope2"]
