...
from localagents.cli.constants import (
    LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_AUDIENCE,
    LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_CLIENT_ID,
    LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN,
)
...
class TestAuthenticationCommand:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "workos",
                {
                    "device_code_url": f"https://{LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN}/oauth2/device_authorization",
                    "token_url": f"https://{LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN}/oauth2/token",
                    "client_id": LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_CLIENT_ID,
                    "audience": LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_AUDIENCE,
                    "domain": LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN,
                },
            ),
        ],
    )
    @patch("localagents.cli.authentication.main.AuthenticationCommand._get_device_code")
    @patch(
        "localagents.cli.authentication.main.AuthenticationCommand._display_auth_instructions"
    )
    @patch("localagents.cli.authentication.main.AuthenticationCommand._poll_for_token")
    @patch("localagents.cli.authentication.main.console.print")
    def test_login(
        self,
        mock_console_print,
        mock_poll,
    ...
    @patch("localagents.cli.authentication.main.webbrowser")
    @patch("localagents.cli.authentication.main.console.print")
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "workos",
                {
                    "jwks_url": f"https://{LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN}/oauth2/jwks",
                    "issuer": f"https://{LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN}",
                    "audience": LOCALAGENTS_ENTERPRISE_DEFAULT_OAUTH2_AUDIENCE,
                },
            ),
        ],
    ...
    @patch("localagents.cli.authentication.main.validate_jwt_token")
    @patch("localagents.cli.authentication.main.TokenManager.save_tokens")
    def test_validate_and_save_token(
        ...
        from localagents.cli.authentication.main import Oauth2Settings
        from localagents.cli.authentication.providers.workos import WorkosProvider
    ...
    @patch("localagents.cli.tools.main.ToolCommand")
    @patch("localagents.cli.authentication.main.Settings")
    @patch("localagents.cli.authentication.main.console.print")
    ...
    def test_login_to_tool_repository_error(
        ...
        expected_calls = [
            ...
            call(
                "Other features will work normally, but you may experience limitations with downloading and publishing tools.\nRun [bold]localagents login[/bold] to try logging in again.\n",
                style="yellow",
            ),
        ]
        mock_console_print.assert_has_calls(expected_calls)

    @patch("localagents.cli.authentication.main.httpx.post")
