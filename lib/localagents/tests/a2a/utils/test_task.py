...
from localagents.a2a.utils.task import cancel, cancellable, execute
...
class TestExecute:
    ...
    async def test_successful_execution(
        ...
        with (
            patch("localagents.a2a.utils.task.Task", return_value=mock_task),
            patch("localagents.a2a.utils.task.localagents_event_bus") as mock_bus,
        ):
...
class TestExecuteAndCancelIntegration:
    ...
    async def test_cancel_stops_running_execute(
        ...
        with (
            patch("localagents.a2a.utils.task.Task", return_value=mock_task),
            patch("localagents.a2a.utils.task.localagents_event_bus"),
        ):
