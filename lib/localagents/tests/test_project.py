...
from localagents.project import (
    CrewBase,
    after_kickoff,
    agent,
    before_kickoff,
...
def test_internal_crew_with_mcp():
    ...
    with (
        patch("localagents_tools.MCPServerAdapter", return_value=mock_adapter) as adapter_mock,
        patch("localagents.llm.LLM.__new__", return_value=mock_llm),
    ):
