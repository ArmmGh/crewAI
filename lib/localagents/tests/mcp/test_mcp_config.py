...
from localagents.mcp.config import MCPServerHTTP, MCPServerSSE, MCPServerStdio
...
def test_agent_with_stdio_mcp_config(mock_tool_definitions):
    ...
    with patch("localagents.mcp.tool_resolver.MCPClient") as mock_client_class:
...
def test_each_invocation_gets_fresh_client(mock_tool_definitions):
    ...
    with patch("localagents.mcp.tool_resolver.MCPClient", side_effect=_make_client):
        agent = Agent(
            role="Test Agent",
            goal="Test goal",
            backstory="Test backstory",
