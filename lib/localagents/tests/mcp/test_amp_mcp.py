...
from localagents.mcp.config import MCPServerHTTP, MCPServerSSE
...
class TestFetchAmpMCPConfigs:
    @patch("localagents.cli.plus_api.PlusAPI")
    @patch("localagents_tools.tools.localagents_platform_tools.misc.get_platform_integration_token", return_value="test-api-key")
    ...
    @patch("localagents_tools.tools.localagents_platform_tools.misc.get_platform_integration_token", side_effect=Exception("No token"))
...
class TestParseAmpRef:
    ...
    def test_legacy_prefix_slug(self):
        slug, tool = MCPToolResolver._parse_amp_ref("localagents-amp:notion")
        assert slug == "notion"
        assert tool is None

    def test_legacy_prefix_with_tool(self):
        slug, tool = MCPToolResolver._parse_amp_ref("localagents-amp:notion#search")
        assert slug == "notion"
        assert tool == "search"


class TestGetMCPToolsAmpIntegration:
    @patch("localagents.mcp.tool_resolver.MCPClient")
    ...
    @patch("localagents.mcp.tool_resolver.MCPClient")
    @patch.object(MCPToolResolver, "_fetch_amp_mcp_configs")
    def test_legacy_localagents_amp_prefix_still_works(
        self, mock_fetch, mock_client_class, agent, mock_tool_definitions
    ):
        mock_fetch.return_value = {
            "notion": {
        ...
        tools = agent.get_mcp_tools(["localagents-amp:notion"])
