...
from localagents.agents.crew_agent_executor import AgentFinish, CrewAgentExecutor
from localagents.cli.constants import DEFAULT_LLM_MODEL
from localagents.events.event_bus import localagents_event_bus
from localagents.events.types.tool_usage_events import ToolUsageFinishedEvent
from localagents.knowledge.knowledge import Knowledge
from localagents.knowledge.knowledge_config import KnowledgeConfig
from localagents.knowledge.source.base_knowledge_source import BaseKnowledgeSource
from localagents.knowledge.source.string_knowledge_source import StringKnowledgeSource
from localagents.llm import LLM
from localagents.llms.base_llm import BaseLLM
from localagents.process import Process
from localagents.tools.tool_calling import InstructorToolCalling
from localagents.tools.tool_usage import ToolUsage
from localagents.utilities.errors import AgentRepositoryError
import pytest

from localagents import Agent, Crew, Task
from localagents.agents.cache import CacheHandler
from localagents.tools import tool
from localagents.utilities import RPMController
...
def test_agent_human_input():
    from localagents.core.providers.human_input import SyncHumanInputProvider
...
@pytest.mark.vcr()
def test_task_allow_localagents_trigger_context():
    from localagents import Crew
    ...
    task = Task(
        ...
        agent=agent,
        allow_localagents_trigger_context=True,
    )
    crew = Crew(agents=[agent], tasks=[task])
    crew.kickoff({"localagents_trigger_payload": "Important context data"})
...
@pytest.mark.vcr()
def test_task_without_allow_localagents_trigger_context():
    from localagents import Crew
    ...
    task = Task(
        ...
        expected_output="Analysis report",
        agent=agent,
        allow_localagents_trigger_context=False,
    )
...
@pytest.mark.vcr()
def test_task_allow_localagents_trigger_context_no_payload():
    from localagents import Crew
...
@pytest.mark.vcr()
def test_do_not_allow_localagents_trigger_context_for_first_task_hierarchical():
    from localagents import Crew
    ...
    crew.kickoff({"localagents_trigger_payload": "Initial context data"})
...
@pytest.mark.vcr()
def test_ensure_first_task_allow_localagents_trigger_context_is_false_does_not_inject():
    from localagents import Crew
    ...
    crew = Crew(agents=[agent1, agent2], tasks=[first_task, second_task])
    crew.kickoff({"localagents_trigger_payload": "Context data"})
...
@patch("localagents.agent.core.CrewTrainingHandler")
...
def test_handle_context_length_exceeds_limit():
    ...
    from localagents.utilities.agent_utils import handle_context_length
    ...
    with patch("localagents.utilities.agent_utils.summarize_messages") as mock_summarize:
...
def test_handle_context_length_exceeds_limit_cli_no():
    ...
    with patch.object(
        ...
        with patch(
            "localagents.utilities.agent_utils.handle_context_length"
...
def test_agent_with_knowledge_sources():
    ...
    with patch("localagents.knowledge") as mock_knowledge:
...
def test_agent_with_knowledge_sources_with_query_limit_and_score_threshold():
    ...
    with (
        patch(
            "localagents.knowledge.storage.knowledge_storage.KnowledgeStorage"
        ) as mock_knowledge_storage,
        patch(
            "localagents.knowledge.source.base_knowledge_source.KnowledgeStorage"
        ) as mock_base_knowledge_storage,
        patch("localagents.rag.chromadb.client.ChromaDBClient") as mock_chromadb,
    ):
...
def test_agent_with_knowledge_sources_extensive_role():
    ...
    with (
        patch("localagents.knowledge") as mock_knowledge,
        patch(
            "localagents.knowledge.storage.knowledge_storage.KnowledgeStorage.save"
        ) as mock_save,
    ):
        mock_knowledge_instance = mock_knowledge.return_value
        mock_knowledge_instance.sources = [string_source]
...
def test_agent_with_knowledge_sources_works_with_copy():
    ...
    with patch(
        "localagents.knowledge.source.base_knowledge_source.BaseKnowledgeSource",
        autospec=True,
        ...
        with patch(
            ...
            from localagents.knowledge.storage.base_knowledge_storage import BaseKnowledgeStorage
...
def test_agent_with_knowledge_with_no_localagents_knowledge():
...
@pytest.mark.vcr()
def test_agent_with_only_localagents_knowledge():
    mock_knowledge = MagicMock(spec=Knowledge)
...
@pytest.mark.vcr()
@pytest.mark.skip(reason="Requires OpenRouter API key")
def test_agent_knowledege_with_localagents_knowledge():
...
def mock_get_auth_token():
    with patch(
        "localagents.cli.authentication.token.get_auth_token", return_value="test_token"
    ):
        yield


@patch("localagents.cli.plus_api.PlusAPI.get_agent")
...
def test_agent_from_repository_override_attributes(mock_get_agent, mock_get_auth_token):
    ...
    mock_get_response.json.return_value = {
        ...
        "backstory": "test backstory",
        "tools": [
            {"name": "SerperDevTool", "module": "localagents_tools", "init_params": {}}
        ],
    }
    mock_get_agent.return_value = mock_get_response
...
def test_agent_from_repository_with_invalid_tools(mock_get_agent, mock_get_auth_token):
    ...
    with pytest.raises(
        ...
        match="Tool DoesNotExist could not be loaded: module 'localagents_tools' has no attribute 'DoesNotExist'",
    ):
...
@patch("localagents.cli.plus_api.PlusAPI.get_agent")
@patch("localagents.utilities.agent_utils.Settings")
@patch("localagents.utilities.agent_utils.console")
...
def test_agent_from_repository_without_org_set(
    ...
    mock_console.print.assert_any_call(
        "No organization currently set. We recommend setting one before using: `localagents org switch <org_id>` command.",
        style="yellow",
    )
...
def test_agent_mcps_accepts_legacy_prefix_with_tool():
    agent = Agent(
        ...
        goal="Test MCP validation",
        backstory="Test agent",
        mcps=["localagents-amp:notion#get_page"],
    )
    assert agent.mcps == ["localagents-amp:notion#get_page"]
