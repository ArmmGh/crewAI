...
class TestAsyncAgentExecutor:
    ...
    async def test_ainvoke_loop_calls_aget_llm_response(
        ...
        with patch(
            "localagents.agents.crew_agent_executor.aget_llm_response",
            new_callable=AsyncMock,
    ...
    async def test_ainvoke_loop_handles_tool_execution(
        ...
        with patch(
            ...
            with patch(
                "localagents.agents.crew_agent_executor.aexecute_tool_and_check_finality",
                new_callable=AsyncMock,
    ...
    async def test_ainvoke_loop_respects_max_iterations(
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "localagents.agents.crew_agent_executor.handle_max_iterations_exceeded",
                    return_value=AgentFinish(
...
class TestInvokeStepCallback:
    ...
    def test_invoke_step_callback_with_async_callback(
        ...
        with patch("localagents.agents.crew_agent_executor.asyncio.run") as mock_run:
            executor._invoke_step_callback(answer)
...
class TestAsyncLLMResponseHelper:
    ...
    async def test_aget_llm_response_calls_acall(self) -> None:
        ...
        from localagents.utilities.agent_utils import aget_llm_response
