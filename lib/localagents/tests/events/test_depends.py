...
from localagents.events import Depends, localagents_event_bus
...
async def test_chain_of_dependencies():
    ...
    with localagents_event_bus.scoped_handlers():
        ...
        @localagents_event_bus.on(DependsTestEvent, depends_on=Depends(handler_a))
        def handler_b(source, event: DependsTestEvent):
            execution_order.append("handler_b")

        @localagents_event_bus.on(DependsTestEvent, depends_on=Depends(handler_b))
...
async def test_async_handler_with_dependency():
    ...
    with localagents_event_bus.scoped_handlers():
        ...
        @localagents_event_bus.on(DependsTestEvent, depends_on=Depends(sync_setup))
...
async def test_mixed_handlers_with_dependencies():
    ...
    with localagents_event_bus.scoped_handlers():
        ...
        @localagents_event_bus.on(DependsTestEvent, depends_on=Depends(setup))
...
async def test_circular_dependency_detection():
    ...
    from localagents.events.handler_graph import CircularDependencyError, build_execution_plan
...
async def test_aemit_ignores_dependencies():
    ...
    with localagents_event_bus.scoped_handlers():
        ...
        event = DependsTestEvent(value=1)
        await localagents_event_bus.aemit("test_source", event)
