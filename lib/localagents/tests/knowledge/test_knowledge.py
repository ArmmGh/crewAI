...
import pytest
from localagents.knowledge.source.crew_docling_source import CrewDoclingSource
from localagents.knowledge.source.csv_knowledge_source import CSVKnowledgeSource
from localagents.knowledge.source.excel_knowledge_source import ExcelKnowledgeSource
from localagents.knowledge.source.json_knowledge_source import JSONKnowledgeSource
...
from localagents.knowledge.source.text_file_knowledge_source import TextFileKnowledgeSource


@pytest.fixture(autouse=True)
def mock_vector_db():
    """Mock vector database operations."""
    with patch("localagents.knowledge.storage.knowledge_storage.KnowledgeStorage") as mock:
...
def test_pdf_knowledge_source(mock_vector_db):
    ...
    pdf_path = current_dir / "localagents_quickstart.pdf"
    ...
    mock_vector_db.query.return_value = [
        {"content": "localagents create crew latest-ai-development", "score": 0.9}
    ]
    ...
    # Assert that the correct information is retrieved
    assert any(
        "localagents create crew latest-ai-development" in result["content"].lower()
        for result in results
    )
    mock_vector_db.query.assert_called_once()
...
def test_hash_based_id_generation_without_doc_id(mock_vector_db):
    ...
    import hashlib
    import json
    from localagents.rag.chromadb.utils import _prepare_documents_for_chromadb
    from localagents.rag.types import BaseRecord
