...
from localagents.memory.types import (
    MemoryConfig,
    MemoryMatch,
    MemoryRecord,
    ScopeInfo,
...
def test_lancedb_save_search(lancedb_path: Path) -> None:
    from localagents.memory.storage.lancedb_storage import LanceDBStorage
...
def test_memory_scope_remember_recall(tmp_path: Path, mock_embedder: MagicMock) -> None:
    from localagents.memory.unified_memory import Memory
    from localagents.memory.memory_scope import MemoryScope
...
def test_memory_slice_recall(tmp_path: Path, mock_embedder: MagicMock) -> None:
    from localagents.memory.unified_memory import Memory
    from localagents.memory.memory_scope import MemorySlice
...
def test_flow_has_default_memory() -> None:
    ...
    from localagents.flow.flow import Flow
    from localagents.memory.unified_memory import Memory
...
def test_memory_extract_memories_returns_list_from_llm(tmp_path: Path) -> None:
    ...
    from localagents.memory.analyze import ExtractedMemories
    from localagents.memory.unified_memory import Memory
...
def test_analyze_for_save_llm_failure_returns_defaults() -> None:
    ...
    from localagents.memory.analyze import MemoryAnalysis, analyze_for_save
...
def test_extract_memories_llm_failure_returns_raw() -> None:
    ...
    from localagents.memory.analyze import extract_memories_from_content
...
def test_analyze_query_llm_failure_returns_defaults() -> None:
    ...
    from localagents.memory.analyze import QueryAnalysis, analyze_query
...
def test_batch_consolidation_deduplicates_against_storage(
    ...
    from localagents.memory.unified_memory import Memory
    from localagents.memory.analyze import ConsolidationPlan
    ...
    # Pre-insert
    from localagents.memory.types import MemoryRecord
...
def test_parallel_analyze_runs_concurrent_calls(tmp_path: Path) -> None:
    ...
    from localagents.memory.unified_memory import Memory
    from localagents.memory.analyze import MemoryAnalysis, ExtractedMetadata
