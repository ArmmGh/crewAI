...
from localagents.telemetry import Telemetry
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("OTEL_SDK_DISABLED", "TRUE", False),
        ("LOCALAGENTS_DISABLE_TELEMETRY", "true", False),
        ("LOCALAGENTS_DISABLE_TELEMETRY", "TRUE", False),
        ("OTEL_SDK_DISABLED", "false", True),
        ("LOCALAGENTS_DISABLE_TELEMETRY", "false", True),
    ],
)
def test_telemetry_environment_variables(env_var, value, expected_ready):
    ...
    with patch.dict(os.environ, env_overrides):
        with patch("localagents.telemetry.telemetry.TracerProvider"):
            telemetry = Telemetry()
...
@patch("localagents.telemetry.telemetry.logger.error")
@patch(
...
def test_telemetry_fails_due_connect_timeout(export_mock, logger_mock):
    ...
    with patch.dict(
        os.environ, {"LOCALAGENTS_DISABLE_TELEMETRY": "false", "OTEL_SDK_DISABLED": "false"}
    ):
        telemetry = Telemetry()
...
def test_no_signal_handler_traceback_in_non_main_thread():
    ...
    Regression test for https://github.com/localAgentsInc/localAgents/issues/4289
    """
    ...
    def init_in_thread():
        try:
            ...
            with (
                patch.dict(
                    os.environ,
                    {"LOCALAGENTS_DISABLE_TELEMETRY": "false", "OTEL_SDK_DISABLED": "false"},
                ),
                patch("localagents.telemetry.telemetry.TracerProvider"),
                patch("signal.signal") as mock_signal,
                patch("localagents.telemetry.telemetry.logger") as mock_logger,
            ):
                Telemetry()
