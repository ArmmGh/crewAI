...
def wait_for_event_handlers(timeout: float = 5.0) -> None:
    ...
    loop = getattr(localagents_event_bus, "_loop", None)
    ...
    localagents_event_bus._sync_executor.shutdown(wait=True)
    localagents_event_bus._sync_executor = ThreadPoolExecutor(
        max_workers=10,
