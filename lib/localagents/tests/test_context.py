...
import pytest
from localagents.context import (
...
class TestPlatformIntegrationToken:
    ...
    @patch.dict(os.environ, {"LOCALAGENTS_PLATFORM_INTEGRATION_TOKEN": "env-token-456"})
    ...
    @patch.dict(os.environ, {"LOCALAGENTS_PLATFORM_INTEGRATION_TOKEN": "env-token"})
    ...
    @patch.dict(os.environ, {"LOCALAGENTS_PLATFORM_INTEGRATION_TOKEN": "env-backup"})
    ...
    @patch.dict(os.environ, {"LOCALAGENTS_PLATFORM_INTEGRATION_TOKEN": ""})
    def test_empty_env_var(self):
        assert _platform_integration_token.get() is None
        assert get_platform_integration_token() == ""

    @patch("localagents.context.os.getenv")
