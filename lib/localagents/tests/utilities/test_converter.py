...
def test_convert_to_model_with_invalid_json() -> None:
    ...
    with patch("localagents.utilities.converter.handle_partial_json") as mock_handle:
...
def test_handle_partial_json_with_invalid_partial(mock_agent: Mock) -> None:
    ...
    with patch("localagents.utilities.converter.convert_with_instructions") as mock_convert:
...
@patch("localagents.utilities.converter.create_converter")
@patch("localagents.utilities.converter.get_conversion_instructions")
...
def test_convert_with_instructions_failure(
    ...
    result = "Some text to convert"
    with patch("localagents.utilities.converter.Printer") as mock_printer:
        output = convert_with_instructions(result, SimpleModel, False, mock_agent)
