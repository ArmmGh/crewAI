...
def test_concurrent_emit_from_multiple_threads():
    ...
    with localagents_event_bus.scoped_handlers():
        ...
        @localagents_event_bus.on(ThreadSafetyTestEvent)
        ...
        def emit_events(thread_id: int) -> None:
            for i in range(events_per_thread):
                ...
                localagents_event_bus.emit(f"source_{thread_id}", event)
...
def test_concurrent_handler_registration():
    ...
    with localagents_event_bus.scoped_handlers():
        ...
        def register_handler(handler_id: int) -> None:
            localagents_event_bus.register_handler(
        ...
        localagents_event_bus.emit("test_source", event)
    ...
    with localagents_event_bus.scoped_handlers():
        ...
        def emit_continuously() -> None:
            for i in range(50):
                ...
                localagents_event_bus.emit("emitter", event)
                time.sleep(0.001)
...
def test_stress_test_rapid_emit():
    ...
    with localagents_event_bus.scoped_handlers():
        ...
        for i in range(num_events):
            ...
            localagents_event_bus.emit("rapid_source", event)
...
def test_multiple_event_types_concurrent():
    ...
    with localagents_event_bus.scoped_handlers():
        ...
        @localagents_event_bus.on(EventTypeA)
        def handler_a(source: object, event: BaseEvent) -> None:
            with lock:
                received_a.append(event)

        @localagents_event_bus.on(EventTypeB)
        ...
        def emit_type_a() -> None:
            for i in range(50):
                localagents_event_bus.emit("source_a", EventTypeA(type=f"type_a_{i}"))

        def emit_type_b() -> None:
            for i in range(50):
                localagents_event_bus.emit("source_b", EventTypeB(type=f"type_b_{i}"))
