...
def test_create_llm_with_none_uses_default_model() -> None:
    with patch.dict(os.environ, {"OPENAI_API_KEY": "fake-key"}, clear=True):
        with patch("localagents.utilities.llm_utils.DEFAULT_LLM_MODEL", DEFAULT_LLM_MODEL):
...
def test_create_llm_anthropic_missing_dependency() -> None:
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "fake-key"}, clear=True):
        with patch("localagents.llm.LLM.__new__", side_effect=ImportError('Anthropic native provider not available, to install: uv add "localagents[anthropic]"')):
            with pytest.raises(ImportError) as exc_info:
                create_llm(llm_value="anthropic/claude-3-sonnet")

            assert "Anthropic native provider not available, to install: uv add \"localagents[anthropic]\"" in str(exc_info.value)
