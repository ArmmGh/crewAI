...
def test_crew_emits_start_kickoff_event(
    ...
    with patch("localagents.events.event_listener.Telemetry", return_value=mock_telemetry):
...
def test_agent_emits_execution_started_and_completed_events(base_agent, base_task):
    ...
    @localagents_event_bus.on(AgentExecutionStartedEvent)
    def handle_agent_start(source, event):
        with condition:
            started_events.append(event)
            condition.notify()

    @localagents_event_bus.on(AgentExecutionCompletedEvent)
...
def test_agent_emits_execution_error_event(base_agent, base_task):
    ...
    @localagents_event_bus.on(AgentExecutionErrorEvent)
    def handle_agent_start(source, event):
        received_events.append(event)
        event_received.set()
...
def test_register_handler_adds_new_handler(base_agent, base_task):
    ...
    localagents_event_bus.register_handler(CrewKickoffStartedEvent, custom_handler)
...
def test_multiple_handlers_for_same_event(base_agent, base_task):
    ...
    localagents_event_bus.register_handler(CrewKickoffStartedEvent, handler_1)
    localagents_event_bus.register_handler(CrewKickoffStartedEvent, handler_2)
...
def test_llm_emits_call_failed_event():
    ...
    with patch(
        "localagents.llms.providers.openai.completion.OpenAICompletion._handle_completion"
...
def test_streaming_fallback_to_non_streaming():
    ...
    def mock_call(messages, tools=None, callbacks=None, available_functions=None):
        ...
        localagents_event_bus.emit(llm, event=LLMStreamChunkEvent(chunk="Test chunk 1", response_id="Id", call_id="test-call-id"))
        localagents_event_bus.emit(llm, event=LLMStreamChunkEvent(chunk="Test chunk 2", response_id="Id", call_id="test-call-id"))
...
def test_streaming_empty_response_handling():
    ...
    def mock_call(messages, tools=None, callbacks=None, available_functions=None):
        ...
        for _ in range(3):
            localagents_event_bus.emit(llm, event=LLMStreamChunkEvent(chunk="", response_id="id", call_id="test-call-id"))
