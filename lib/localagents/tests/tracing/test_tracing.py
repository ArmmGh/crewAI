...
from localagents.events.listeners.tracing.first_time_trace_handler import (
    FirstTimeTraceHandler,
)
from localagents.events.listeners.tracing.trace_batch_manager import (
    TraceBatch,
    TraceBatchManager,
)
...
from localagents.events.listeners.tracing.types import TraceEvent
from localagents.flow.flow import Flow, start
...
class TestTraceListenerSetup:
    ...
    def mock_user_data_file_io(self):
        ...
        with patch(
            "localagents.events.listeners.tracing.utils._load_user_data",
            return_value={},
        ):
            yield
    ...
    def mock_auth_token(self):
        ...
        with (
            patch(
                "localagents.cli.authentication.token.get_auth_token",
                return_value="mock_token_12345",
            ),
            patch(
                "localagents.events.listeners.tracing.trace_listener.get_auth_token",
                return_value="mock_token_12345",
            ),
            patch(
                "localagents.events.listeners.tracing.trace_batch_manager.get_auth_token",
                return_value="mock_token_12345",
            ),
        ):
            yield
    ...
    def reset_tracing_singletons(self):
        ...
        from localagents.events.listeners.tracing.utils import _tracing_enabled
        ...
        with localagents_event_bus._rwlock.w_locked():
            localagents_event_bus._sync_handlers = {}
            localagents_event_bus._async_handlers = {}
            localagents_event_bus._handler_dependencies = {}
            localagents_event_bus._execution_plan_cache = {}
    ...
    def test_trace_listener_collects_crew_events(self):
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "LOCALAGENTS_DISABLE_TELEMETRY": "false",
                "LOCALAGENTS_DISABLE_TRACKING": "false",
    ...
    def test_batch_manager_finalizes_batch_clears_buffer(self):
        ...
        with patch.dict(
            ...
            trace_listener = None
            with localagents_event_bus._rwlock.r_locked():
                for handler_set in localagents_event_bus._sync_handlers.values():
                ...
                if not trace_listener:
                    for handler_set in localagents_event_bus._async_handlers.values():
                        for handler in handler_set:
                            if hasattr(handler, "__self__") and isinstance(
                                handler.__self__, TraceCollectionListener
                            ):
    ...
    def test_trace_listener_disabled_when_env_false(self):
        ...
        with patch.dict(os.environ, {"LOCALAGENTS_TRACING_ENABLED": "false"}):
            agent = Agent(
                role="Test Agent",
                goal="Test goal",
                backstory="Test backstory",
            ...
            trace_handlers = []
            with localagents_event_bus._rwlock.r_locked():
                for handlers in localagents_event_bus._sync_handlers.values():
                    for handler in handlers:
                        if hasattr(handler, "__self__") and isinstance(
                            handler.__self__, TraceCollectionListener
                        ):
                            trace_handlers.append(handler)
                for handlers in localagents_event_bus._async_handlers.values():
                    for handler in handlers:
                        if hasattr(handler, "__self__") and isinstance(
                            handler.__self__, TraceCollectionListener
                        ):
    ...
    def test_trace_listener_ephemeral_batch(self):
        ...
        with (
            ...
            patch(
                "localagents.events.listeners.tracing.trace_listener.TraceCollectionListener._check_authenticated",
                return_value=False,
            ),
        ):
            agent = Agent(
    ...
    def test_first_time_user_trace_collection_with_timeout(self, mock_plus_api_calls):
        ...
        with (
            patch.dict(
                os.environ,
                {
                    "LOCALAGENTS_TRACING_ENABLED": "false",
                    "LOCALAGENTS_DISABLE_TELEMETRY": "false",
                    "LOCALAGENTS_DISABLE_TRACKING": "false",
                    "OTEL_SDK_DISABLED": "false",
                },
            ...
            patch(
                "localagents.events.listeners.tracing.utils._is_test_environment",
                return_value=False,
            ),
            patch(
                "localagents.events.listeners.tracing.utils.should_auto_collect_first_time_traces",
                return_value=True,
            ),
            patch(
                "localagents.events.listeners.tracing.utils.is_first_execution",
                return_value=True,
            ),
            patch(
                "localagents.events.listeners.tracing.first_time_trace_handler.prompt_user_for_trace_viewing",
                return_value=False,
            ) as mock_prompt,
            patch(
                "localagents.events.listeners.tracing.first_time_trace_handler.mark_first_execution_completed"
    ...
    def test_first_time_user_trace_collection_user_accepts(self, mock_plus_api_calls):
        ...
        with (
            ...
            trace_listener.batch_manager.ephemeral_trace_url = (
                "https://localagents.com/trace/mock-id"
            )
    ...
    def test_trace_batch_marked_as_failed_on_finalize_error(self):
        ...
        with patch("localagents.events.listeners.tracing.trace_batch_manager.is_tracing_enabled_in_context", return_value=True):
    ...
    def test_ephemeral_batch_includes_anon_id(self):
        ...
        with (
            patch(
                "localagents.events.listeners.tracing.trace_batch_manager.is_tracing_enabled_in_context",
                return_value=True,
            ),
            patch(
                "localagents.events.listeners.tracing.trace_batch_manager.get_user_id",
                return_value=fake_user_id,
            ),
            patch(
                "localagents.events.listeners.tracing.trace_batch_manager.should_auto_collect_first_time_traces",
                return_value=False,
            ),
        ):
            batch_manager = TraceBatchManager()
...
class TestInitializeBackendBatchRetry:
    ...
    def test_retries_on_none_response_then_succeeds(self):
        ...
        with (
            ...
            patch("localagents.events.listeners.tracing.trace_batch_manager.time.sleep") as mock_sleep,
        ):
            bm._initialize_backend_batch(
                user_context={"privacy_level": "standard"},
                execution_metadata={"execution_type": "crew"},
    ...
    def test_retries_on_5xx_then_succeeds(self):
        ...
        with (
            ...
            patch("localagents.events.listeners.tracing.trace_batch_manager.time.sleep"),
        ):
            bm._initialize_backend_batch(
                user_context={"privacy_level": "standard"},
                execution_metadata={"execution_type": "crew"},
