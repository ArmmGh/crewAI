...
from localagents_files.core.constants import (
    MAX_CONCURRENCY,
    MULTIPART_CHUNKSIZE,
    MULTIPART_THRESHOLD,
)
from localagents_files.core.sources import FileBytes, FilePath
...
class BedrockFileUploader(FileUploader):
    ...
    def __init__(
        ...
        Args:
            bucket_name: S3 bucket name. If not provided, uses
                LOCALAGENTS_BEDROCK_S3_BUCKET environment variable.
            bucket_owner: Optional bucket owner account ID for cross-account access.
                Uses LOCALAGENTS_BEDROCK_S3_BUCKET_OWNER environment variable if not provided.
            prefix: S3 key prefix for uploaded files (default: "localagents-files").
        ...
        """
        self._bucket_name = bucket_name or os.environ.get("LOCALAGENTS_BEDROCK_S3_BUCKET")
        self._bucket_owner = bucket_owner or os.environ.get(
            "LOCALAGENTS_BEDROCK_S3_BUCKET_OWNER"
        )
        self._prefix = prefix
    ...
    def bucket_name(self) -> str:
        ...
        if not self._bucket_name:
            raise ValueError(
                "S3 bucket name not configured. Set LOCALAGENTS_BEDROCK_S3_BUCKET "
