...
def get_uploader(
    ...
    if "bedrock" in provider_lower or "aws" in provider_lower:
        ...
        if (
            not os.environ.get("LOCALAGENTS_BEDROCK_S3_BUCKET")
            and "bucket_name" not in kwargs
        ):
            logger.debug(
                "Bedrock S3 uploader not configured. "
                "Set LOCALAGENTS_BEDROCK_S3_BUCKET environment variable to enable."
            )
            raise
        try:
            from localagents_files.uploaders.bedrock import BedrockFileUploader
            ...
            return BedrockFileUploader(
                ...
                prefix=kwargs.get("prefix", "localagents-files"),
