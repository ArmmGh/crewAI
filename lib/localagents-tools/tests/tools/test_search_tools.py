...
from localagents_tools.tools import (
    CSVSearchTool,
    CodeDocsSearchTool,
    DOCXSearchTool,
    DirectorySearchTool,
...
def test_website_search_tool(mock_adapter):
    ...
    website = "https://localagents.com"
    search_query = "what is localagents?"
    tool = WebsiteSearchTool(website=website, adapter=mock_adapter)
    result = tool._run(search_query=search_query)

    mock_adapter.query.assert_called_once_with(
        "what is localagents?", similarity_threshold=0.6, limit=5
    )
...
def test_youtube_channel_search_tool(mock_adapter):
    ...
    youtube_channel_handle = "@localagents"
...
def test_code_docs_search_tool(mock_adapter):
    ...
    docs_url = "https://localagents.com/any-docs-url"
...
def test_github_search_tool(mock_adapter):
    ...
    tool = GithubSearchTool(
        gh_token="test_token",
        github_repo="localagents/localagents",
        content_types=["code"],
        adapter=mock_adapter,
    )
    result = tool._run(search_query="tell me about localagents repo")
    assert "repo description" in result
    mock_adapter.add.assert_called_once_with(
        "https://github.com/localagents/localagents",
        data_type=DataType.GITHUB,
        metadata={"content_types": ["code"], "gh_token": "test_token"},
    )
    mock_adapter.query.assert_called_once_with(
        "tell me about localagents repo", similarity_threshold=0.6, limit=5
    )
    ...
    result = tool._run(
        ...
        search_query="tell me about localagents repo",
    )
    assert "repo description" in result
    mock_adapter.add.assert_called_once_with(
        "https://github.com/localagents/localagents",
