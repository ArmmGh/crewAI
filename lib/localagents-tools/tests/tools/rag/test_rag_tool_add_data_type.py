...
def rag_tool(mock_rag_client: MagicMock) -> RagTool:
    ...
    with (
        patch(
            "localagents_tools.adapters.localagents_rag_adapter.get_rag_client",
            return_value=mock_rag_client,
        ),
        patch(
            "localagents_tools.adapters.localagents_rag_adapter.create_client",
            return_value=mock_rag_client,
        ),
    ):
        return RagTool()
...
class TestDataTypeStringValues:
    ...
    def test_pdf_file_string(
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "localagents_tools.adapters.localagents_rag_adapter.DataType.get_loader"
            ) as mock_loader:
