...
from localagents_tools import SingleStoreSearchTool
from localagents_tools.tools.singlestore_search_tool import SingleStoreSearchToolSchema
import pytest
...
def docker_server_url() -> Generator[str, None, None]:
    ...
    try:
        ...
        conn = sdb.connect()
        curr = conn.cursor()
        curr.execute("CREATE DATABASE test_localagents")
        curr.close()
        conn.close()
...
def clean_db_url(docker_server_url) -> Generator[str, None, None]:
    ...
    try:
        conn = connect(host=docker_server_url, database="test_localagents")
        curr = conn.cursor()
...
def sample_table_setup(clean_db_url):
    ...
    conn = connect(host=clean_db_url, database="test_localagents")
    curr = conn.cursor()
...
class TestSingleStoreSearchTool:
    ...
    def test_tool_creation_with_connection_url(self, sample_table_setup):
        ...
        tool = SingleStoreSearchTool(host=f"{sample_table_setup}/test_localagents")
    ...
    def test_tool_creation_with_empty_database(self, clean_db_url):
        ...
        with pytest.raises(ValueError, match="No tables found in the database"):
            SingleStoreSearchTool(host=clean_db_url, database="test_localagents")

    def test_description_generation(self, sample_table_setup):
        """Test that tool description is properly generated with table info."""

        tool = SingleStoreSearchTool(host=sample_table_setup, database="test_localagents")
    ...
    def test_query_validation_select_allowed(self, sample_table_setup):
        ...
        tool = SingleStoreSearchTool(database="test_localagents")
