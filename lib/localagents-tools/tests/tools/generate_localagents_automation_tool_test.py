...
from localagents_tools.tools.generate_localagents_automation_tool.generate_localagents_automation_tool import (
    ...
    GenerateLocalagentsAutomationToolSchema,
)
import pytest
import requests
...
def mock_env():
    with patch.dict(os.environ, {"LOCALAGENTS_PERSONAL_ACCESS_TOKEN": "test_token"}):
        os.environ.pop("LOCALAGENTS_PLUS_URL", None)
        yield


@pytest.fixture
def tool():
    return GenerateLocalagentsAutomationTool()


@pytest.fixture
def custom_url_tool():
    with patch.dict(os.environ, {"LOCALAGENTS_PLUS_URL": "https://custom.localagents.com"}):
        return GenerateLocalagentsAutomationTool()


def test_default_initialization(tool):
    assert tool.localagents_enterprise_url == "https://app.localagents.com"
...
def test_custom_base_url_from_environment(custom_url_tool):
    assert custom_url_tool.localagents_enterprise_url == "https://custom.localagents.com"
...
def test_valid_prompt_only():
    schema = GenerateLocalagentsAutomationToolSchema(
...
def test_empty_prompt_validation():
    schema = GenerateLocalagentsAutomationToolSchema(prompt="")
...
def test_successful_generation_without_org_id(mock_post, tool):
    ...
    mock_response.json.return_value = {
        "url": "https://app.localagents.com/studio/project-123"
    }
    ...
    assert (
        result
        == "Generated LocalAI Studio project URL: https://app.localagents.com/studio/project-123"
    )
    mock_post.assert_called_once_with(
        "https://app.localagents.com/localagents_plus/api/v1/studio",
        headers={
...
def test_successful_generation_with_org_id(mock_post, tool):
    mock_response = MagicMock()
    mock_response.json.return_value = {
        "url": "https://app.localagents.com/studio/project-456"
    }
    mock_post.return_value = mock_response
    ...
    assert (
        result
        == "Generated LocalAI Studio project URL: https://app.localagents.com/studio/project-456"
    )
    mock_post.assert_called_once_with(
        "https://app.localagents.com/localagents_plus/api/v1/studio",
        headers={
            ...
            "Content-Type": "application/json",
            "Accept": "application/json",
            "X-Localagents-Organization-Id": "org-456",
        },
        json={"prompt": "Create automation"},
    )
...
def test_custom_base_url_usage(mock_post, custom_url_tool):
    mock_response = MagicMock()
    mock_response.json.return_value = {
        "url": "https://custom.localagents.com/studio/project-789"
    }
    mock_post.return_value = mock_response
    ...
    mock_post.assert_called_once_with(
        "https://custom.localagents.com/localagents_plus/api/v1/studio",
        headers={
            "Authorization": "Bearer test_token",
            "Content-Type": "application/json",
            "Accept": "application/json",
...
def test_authorization_header_construction(tool):
    ...
    assert "X-Localagents-Organization-Id" not in headers
...
def test_authorization_header_with_org_id(tool):
    ...
    assert headers["Authorization"] == "Bearer test_token"
    assert headers["X-Localagents-Organization-Id"] == "org-123"
