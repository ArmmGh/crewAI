...
## Description
The MultiOnTool, integrated within the localagents_tools package, empowers LocalAI agents with the capability to navigate and interact with the web through natural language instructions. Leveraging the Multion API, this tool facilitates seamless web browsing, making it an essential asset for projects requiring dynamic web data interaction.

## Installation
Ensure the `localagents[tools]` package is installed in your environment to use the MultiOnTool. If it's not already installed, you can add it using the command below:
```shell
...
from localagents_tools import MultiOnTool
...
1. **Install LocalAI**: Confirm that the `localagents[tools]` package is installed in your Python environment.
