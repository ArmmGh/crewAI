...
## Installation
To use the GithubSearchTool, first ensure the localagents_tools package is installed in your Python environment:

```shell
pip install 'localagents[tools]'
```

This command installs the necessary package to run the GithubSearchTool along with any other tools included in the localagents_tools package.

## Example
Here’s how you can use the GithubSearchTool to perform semantic searches within a GitHub repository:
```python
from localagents_tools import GithubSearchTool
