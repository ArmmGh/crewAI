...
from localagents_tools.rag.base_loader import BaseLoader, LoaderResult
...
class DOCXLoader(BaseLoader):
    def load(self, source_content: SourceContent, **kwargs: Any) -> LoaderResult:  # type: ignore[override]
        try:
            ...
            raise ImportError(
                "python-docx is required for DOCX loading. Install with: 'uv pip install python-docx' or pip install localagents-tools[rag]"
            ) from e
    ...
    def _download_from_url(url: str, kwargs: dict[str, Any]) -> str:
        headers = kwargs.get(
            "headers",
            {
                ...
                "User-Agent": "Mozilla/5.0 (compatible; localagents-tools DOCXLoader)",
            },
        )
