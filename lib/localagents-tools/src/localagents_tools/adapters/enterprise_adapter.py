...
class EnterpriseActionKitToolAdapter:
    ...
    def _set_enterprise_action_token(self, enterprise_action_token: str | None) -> None:
        if enterprise_action_token and not enterprise_action_token.startswith("PK_"):
            warnings.warn(
                "Legacy token detected, please consider using the new Enterprise Action Auth token. Check out our docs for more information https://docs.localagents.com/en/enterprise/features/integrations.",
                DeprecationWarning,
                stacklevel=2,
            )

        token = enterprise_action_token or os.environ.get(
            "LOCALAGENTS_ENTERPRISE_TOOLS_TOKEN"
        )
