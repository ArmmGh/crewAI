...
try:
    ...
    class LocalAgentsToolAdapter(ToolAdapter):
        ...
        *tool_names: str,
        ...
        if not MCP_AVAILABLE:
            ...
            if click.confirm(
                ...
                try:
                    subprocess.run(["uv", "add", "mcp localagents-tools'[mcp]'"], check=True)  # noqa: S607

                except subprocess.CalledProcessError as e:
                    raise ImportError("Failed to install mcp package") from e
            else:
                raise ImportError(
                    "`mcp` package not found, please run `uv add localagents-tools[mcp]`"
                )

        try:
            self._serverparams = serverparams
            self._adapter = MCPAdapt(
                self._serverparams, LocalAgentsToolAdapter(), connect_timeout
            )
            self.start()
