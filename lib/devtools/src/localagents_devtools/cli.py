...
from localagents_devtools.docs_check import docs_check
from localagents_devtools.prompts import RELEASE_NOTES_PROMPT, TRANSLATE_RELEASE_NOTES_PROMPT
...
def update_changelog(
    ...
    entry = (
        ...
        f"\n"
        f"  [{link_text}]"
        f"(https://github.com/localAgentsInc/localAgents/releases/tag/{version})\n"
        f"\n"
        f"{indented_notes}\n"
        f"\n"
        f"</Update>"
...
def _pin_localagents_deps(content: str, version: str) -> str:
    """Replace localagents dependency version pins in a pyproject.toml string.
    ...
    return re.sub(
        r'"localagents(\[tools\])?(==|>=)[^"]*"',
        lambda m: f'"localagents{(m.group(1) or "")!s}=={version}"',
        content,
    )


def update_template_dependencies(templates_dir: Path, new_version: str) -> list[Path]:
    """Update localagents dependency versions in CLI template pyproject.toml files.
    ...
    for pyproject in templates_dir.rglob("pyproject.toml"):
        ...
        new_content = _pin_localagents_deps(content, new_version)
        if new_content != content:
...
def get_github_contributors(commit_range: str) -> list[str]:
    ...
    try:
        ...
        github_repo = g.get_repo("localAgentsInc/localAgents")
...
def _update_all_versions(
    ...
    templates_dir = lib_dir / "localagents" / "src" / "localagents" / "cli" / "templates"
...
_ENTERPRISE_LOCALAGENTS_DEP_PATH: Final[str | None] = os.getenv("ENTERPRISE_LOCALAGENTS_DEP_PATH")
...
def _update_enterprise_localagents_dep(pyproject_path: Path, version: str) -> bool:
    """Update the localagents[tools] pin in an enterprise pyproject.toml.

    Args:
        pyproject_path: Path to the pyproject.toml file.
        version: New localagents version string.
    ...
    new_content = _pin_localagents_deps(content, version)
    if new_content != content:
...
_DEPLOYMENT_TEST_REPO: Final[str] = "localAgentsInc/crew_deployment_test"
...
def _update_deployment_test_repo(version: str, is_prerelease: bool) -> None:
    """Update the deployment test repo to pin the new localagents version.

    Clones the repo, updates the localagents[tools] pin in pyproject.toml,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        new_content = re.sub(
            r'"localagents\[tools\]==[^"]+"',
            f'"localagents[tools]=={version}"',
            content,
        )
        if new_content == content:
            console.print(
                "[yellow]Warning:[/yellow] No localagents[tools] pin found to update"
            )
            return
        pyproject.write_text(new_content)
        console.print(f"[green]✓[/green] Updated localagents[tools] pin to {version}")
        ...
        run_command(
            ["git", "commit", "-m", f"chore: bump localagents to {version}"],
            cwd=repo_dir,
        )
...
def _release_enterprise(version: str, is_prerelease: bool, dry_run: bool) -> None:
    ...
    ENTERPRISE_LOCALAGENTS_DEP_PATH to be validated before calling.
    ...
    if dry_run:
        ...
        console.print(
            f"[dim][DRY RUN][/dim] Would update localagents[tools] dep in "
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # --- update localagents[tools] pin ---
        enterprise_pyproject = repo_dir / enterprise_dep_path
        if _update_enterprise_localagents_dep(enterprise_pyproject, version):
            console.print(
                f"[green]✓[/green] Updated localagents[tools] dep in {enterprise_dep_path}"
            )

        _wait_for_pypi("localagents", version)
...
def release(
    ...
    try:
        ...
        if not skip_enterprise or skip_to_enterprise:
            ...
            if not _ENTERPRISE_LOCALAGENTS_DEP_PATH:
                missing.append("ENTERPRISE_LOCALAGENTS_DEP_PATH")
            if missing:
                console.print(
