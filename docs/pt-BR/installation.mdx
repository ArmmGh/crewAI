---
title: Instalação
description: Comece a usar o LocalAI - Instale, configure e crie seu primeiro crew de IA
icon: wrench
mode: "wide"
---
...
LocalAI requer `Python >=3.10 e <3.14`. Veja como verificar sua versão:
...
LocalAI utiliza o `uv` como ferramenta de gerenciamento de dependências e pacotes. Ele simplifica a configuração e execução do projeto, oferecendo uma experiência fluida.
...
<Steps>
    ...
    <Step title="Instale o LocalAI 🚀">
      - Execute o seguinte comando para instalar o CLI do `localagents`:
        ```shell
      ...
      - Para verificar se o `localagents` está instalado, execute:
        ```shell
        uv tool list
        ```
      - Você deverá ver algo assim:
        ...
        - localagents
        ```
      - Caso precise atualizar o `localagents`, execute:
        ```shell
...
<Steps>
  <Step title="Gerar Scaffolding do Projeto">
    - Execute o comando CLI do `localagents`:
      ```shell
...
<Note type="info">
Para equipes e organizações, o LocalAI oferece opções de implantação corporativa que eliminam a complexidade da configuração:

### LocalAI AMP (SaaS)

- Zero instalação necessária - basta se cadastrar gratuitamente em [app.localagents.com](https://app.localagents.com)
- Atualizações e manutenção automáticas
- Infraestrutura e escalabilidade gerenciadas
- Construa crews sem código

### LocalAI Factory (Auto-Hospedado)
...
<Card title="Explore as Opções Enterprise" icon="building" href="https://localagents.com/enterprise">
  Saiba mais sobre as soluções enterprise do LocalAI e agende uma demonstração
</Card>
</Note>
...
<CardGroup cols={2}>
  ...
  >
    Siga nosso guia de início rápido para criar seu primeiro agente LocalAI e
