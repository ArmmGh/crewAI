...
1. Você deve ter uma conta na LocalAI, experimente o [plano gratuito](https://app.localagents.com).
2. Você precisa estar autenticado usando o CLI da LocalAI.
3. Se você tiver mais de uma organização, garanta que alternou para a organização correta usando o comando do CLI:

```bash
localagents org switch <org_id>
```
...
Para alternar entre organizações ou ver sua organização atual, use o CLI da LocalAI:
