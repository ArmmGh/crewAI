...
O Enterprise Event Streaming permite que você receba atualizações em tempo real via webhook sobre suas crews e flows implantados no LocalAI AMP, como chamadas de modelo, uso de ferramentas e etapas do flow.
...
A estrutura do objeto `data` varia conforme o tipo de evento. Consulte a [lista de eventos](https://github.com/localAIInc/localAI/tree/main/src/localagents/utilities/events) no GitHub.
...
O LocalAI oferece suporte a eventos do sistema e eventos personalizados no Enterprise Event Streaming. Esses eventos são enviados para o endpoint do seu webhook configurado durante a execução das crews e flows.
...
Os nomes dos eventos correspondem ao event bus interno. Veja o [código fonte no GitHub](https://github.com/localAIInc/localAI/tree/main/src/localagents/utilities/events) para a lista completa.
