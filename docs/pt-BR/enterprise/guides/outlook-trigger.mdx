...
Teste sua integração de trigger do Outlook localmente usando a CLI da LocalAI:
...
O comando `localagents triggers run` executará sua crew com um payload completo do Outlook, permitindo que você teste sua lógica de parsing antes do deployment.

<Warning>
  Use `localagents triggers run microsoft_outlook/email_received` (não `localagents run`)
...
- Teste localmente com `localagents triggers run microsoft_outlook/email_received` para ver a estrutura exata do payload
