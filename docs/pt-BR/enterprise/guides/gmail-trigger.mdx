...
Teste sua integração de trigger do Gmail localmente usando a CLI da LocalAI:
...
localagents triggers run gmail/new_email_received
```

O comando `localagents triggers run` executará sua crew com um payload completo do Gmail, permitindo que você teste sua lógica de parsing antes do deployment.

<Warning>
  Use `localagents triggers run gmail/new_email_received` (não `localagents run`) para
...
- Teste localmente com `localagents triggers run gmail/new_email_received` para ver a estrutura exata do payload
- Check the execution logs and confirm the payload is passed as `localagents_trigger_payload`
