...
result = crew.kickoff({
    "localagents_trigger_payload": teams_payload,
})
print(result.raw)
```
...
Teste sua integração de trigger do Microsoft Teams localmente usando a CLI da LocalAI:
...
localagents triggers run microsoft_teams/teams_message_created
```

O comando `localagents triggers run` executará sua crew com um payload completo do Teams, permitindo que você teste sua lógica de parsing antes do deployment.

<Warning>
  Use `localagents triggers run microsoft_teams/teams_message_created` (não `localagents
...
- Teste localmente com `localagents triggers run microsoft_teams/teams_message_created` para ver a estrutura exata do payload
