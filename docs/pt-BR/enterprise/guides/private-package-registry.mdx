...
description: "Instale pacotes Python privados de registros PyPI autenticados no LocalAI AMP"
icon: "lock"
mode: "wide"
---

<Note>
  Este guia aborda como configurar seu projeto LocalAI para instalar pacotes Python
  de registros PyPI privados (Azure DevOps Artifacts, GitHub Packages, GitLab, AWS CodeArtifact, etc.)
  ao implantar no LocalAI AMP.
</Note>
...
O LocalAI AMP usa [UV](https://docs.astral.sh/uv/) para resolução e instalação de dependências.
...
[project]
dependencies = [
    "localagents[tools]>=0.100.1,<1.0.0",
...
<Warning>
  Essas variáveis de ambiente **devem** ser adicionadas pelas configurações de **Variáveis de Ambiente** do LocalAI AMP —
...
As credenciais do registro privado devem ser configuradas como variáveis de ambiente no LocalAI AMP.
Você tem duas opções:

<Tabs>
  <Tab title="Interface Web">
    1. Faça login no [LocalAI AMP](https://app.localagents.com)
  ...
  <Tab title="Implantação via CLI">
    Adicione as variáveis ao seu arquivo `.env` local antes de executar `localagents deploy create`.
...
Quando o LocalAI AMP faz o build da sua automação, o fluxo de resolução funciona assim:
