...
<Steps>
    ...
    <Step title="Configure a Conexão Enterprise do LocalAI">
        4. Em outra aba, abra `LocalAI AMP > LLM Connections`. Dê um nome à sua LLM Connection, selecione Azure como provedor e escolha o mesmo modelo que você selecionou no Azure.
    ...
    <Step title="Defina Configurações Padrão">
        7. Em `LocalAI AMP > Settings > Defaults > Crew Studio LLM Settings`, defina a nova LLM Connection e o modelo como padrão.
    </Step>
    ...
    <Step title="Configure o Acesso à Rede">
        8. Certifique-se das configurações de acesso à rede:
            ...
            - Certifique-se de que a opção `Allow access from all networks` está habilitada. Se essa configuração estiver restrita, o LocalAI pode ser impedido de acessar seu endpoint do Azure OpenAI.
    </Step>
...
- Certifique-se de que o acesso à rede está configurado para permitir conexões do LocalAI
- Confirme se o modelo da implantação corresponde ao que você configurou no LocalAI
