...
Teste sua integração de trigger do OneDrive localmente usando a CLI da LocalAI:
...
O comando `localagents triggers run` executará sua crew com um payload completo do OneDrive, permitindo que você teste sua lógica de parsing antes do deployment.

<Warning>
  Use `localagents triggers run microsoft_onedrive/file_changed` (não `localagents run`)
...
- Teste localmente com `localagents triggers run microsoft_onedrive/file_changed` para ver a estrutura exata do payload
