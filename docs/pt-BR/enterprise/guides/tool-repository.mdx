...
O Repositório de Ferramentas é um gerenciador de pacotes para ferramentas da LocalAI. Ele permite que usuários publiquem, instalem e gerenciem ferramentas que se integram com crews e flows da LocalAI.

As ferramentas podem ser:

- **Privadas**: acessíveis apenas dentro da sua organização (padrão)
- **Públicas**: acessíveis a todos os usuários LocalAI se publicadas com a flag `--public`
...
- Uma conta [LocalAI AMP](https://app.localagents.com)
- [LocalAI CLI](/pt-BR/concepts/cli#cli) instalada
...
- Permissões de acesso para publicar ou instalar ferramentas em sua organização LocalAI AMP
...
```bash
localagents tool install <nome-da-ferramenta>
```
...
```bash
localagents tool create <nome-da-ferramenta>
```
...
1. Acesse o [LocalAI AMP](https://app.localagents.com)
