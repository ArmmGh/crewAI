...
description: "Implante seu Crew ou Flow no LocalAI AMP"
icon: "rocket"
mode: "wide"
---
...
<Note>
  ...
  implantá-lo na plataforma LocalAI AMP. Este guia cobre múltiplos métodos de
...
<Info>
  **Crews vs Flows**: Ambos os tipos de projeto podem ser implantados como "automações" no LocalAI AMP.
...
## Opção 1: Implantar Usando o LocalAI CLI
...
<Steps>
  <Step title="Instale o LocalAI CLI">
    Se ainda não tiver, instale o LocalAI CLI:
    ...
    <Tip>
    A CLI vem com o pacote principal LocalAI, mas o extra `[tools]` garante todas as dependências de implantação.
    </Tip>

  </Step>

  <Step title="Autentique-se na Plataforma Enterprise">
    Primeiro, você precisa autenticar sua CLI com a plataforma LocalAI AMP:

    ```bash
    # Se já possui uma conta LocalAI AMP, ou deseja criar uma:
    localagents login
    ```
...
O LocalAI CLI oferece vários comandos para gerenciar suas implantações:
...
localagents deploy remove <deployment_id>
```

## Opção 2: Implantar Diretamente pela Interface Web

Você também pode implantar seus Crews ou Flows diretamente pela interface web do LocalAI AMP conectando sua conta do GitHub. Esta abordagem não requer utilizar a CLI na sua máquina local. A plataforma detecta automaticamente o tipo do seu projeto e trata o build adequadamente.
...
Você precisa enviar seu crew para um repositório do GitHub. Caso ainda não tenha criado um crew, você pode [seguir este tutorial](/pt-BR/quickstart).
  ...
  <Step title="Conectando o GitHub ao LocalAI AMP">

    1. Faça login em [LocalAI AMP](https://app.localagents.com)
...
Para implantações automatizadas em pipelines CI/CD, você pode usar a API do LocalAI para acionar reimplantações de crews existentes. Isso é particularmente útil para GitHub Actions, Jenkins ou outros workflows de automação.

<Steps>
  <Step title="Obtenha Seu Token de Acesso Pessoal">

    Navegue até as configurações da sua conta LocalAI AMP para gerar um token de API:

    1. Acesse [app.localagents.com](https://app.localagents.com)
  ...
  <Step title="Encontre o UUID da Sua Automação">
    ...
    1. Acesse **Automations** no seu dashboard LocalAI AMP
  ...
  <Step title="Acione a Reimplantação via API">
    ...
    #   "public_url": "https://your-crew-deployment.localagents.com",
  ...
  <Step title="Exemplo de Integração com GitHub Actions">
    ...
    <Tip>
    Adicione `LOCALAGENTS_PAT` e `LOCALAGENTS_AUTOMATION_UUID` como secrets do repositório. Para implantações de PR, adicione um label "deploy" para acionar o workflow.
    </Tip>
...
2. **Interface Web**: Acesse [app.localagents.com](https://app.localagents.com) para visualizar:
...
**Solução**: Verifique se a seção `[tool.localagents]` corresponde ao tipo do seu projeto:
...
<Card title="Precisa de Ajuda?" icon="headset" href="mailto:support@localagents.com">
