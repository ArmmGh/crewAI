...
description: "Certifique-se de que seu Crew ou Flow está pronto para implantação no LocalAI AMP"
icon: "clipboard-check"
mode: "wide"
---

<Note>
  Antes de implantar no LocalAI AMP, é crucial verificar se seu projeto está estruturado corretamente.
...
No LocalAI AMP, **automações** é o termo geral para projetos de IA Agêntica implantáveis. Uma automação pode ser:
...
| **Comando CLI de criação** | `localagents create crew name` | `localagents create flow name` |
...
Quando você executa `localagents create crew my_crew`, você obtém esta estrutura:
...
Quando você executa `localagents create flow my_flow`, você obtém esta estrutura:
...
Seu `pyproject.toml` deve incluir a seção `[tool.localagents]` correta:
...
LocalAI usa `uv` para gerenciamento de dependências. O arquivo `uv.lock` garante builds reproduzíveis e é **obrigatório** para implantação.
...
grep -A2 "\[tool.localagents\]" pyproject.toml
...
<Card title="Deploy para AMP" icon="rocket" href="/pt-BR/enterprise/guides/deploy-to-amp">
  Siga o guia de implantação para implantar seu Crew ou Flow no LocalAI AMP usando
