...
description: "Conecte seus próprios servidores MCP ao LocalAI AMP com acesso público, autenticação por token ou OAuth 2.0"
icon: "plug"
mode: "wide"
---

O LocalAI AMP suporta a conexão com qualquer servidor MCP que implemente o [Model Context Protocol](https://modelcontextprotocol.io/). Você pode conectar servidores públicos que não exigem autenticação, servidores protegidos por chave de API ou token bearer, e servidores que utilizam OAuth 2.0 para acesso delegado seguro.

## Pré-requisitos

<CardGroup cols={2}>
  <Card title="Conta LocalAI AMP" icon="user">
    Você precisa de uma conta ativa no [LocalAI AMP](https://app.localagents.com).
  </Card>
...
<Steps>
    <Step title="Acesse Tools & Integrations">
        Navegue até **Tools & Integrations** no menu lateral esquerdo do LocalAI AMP e selecione a aba **Connections**.
    </Step>
...
Use este método para servidores MCP que requerem autorização OAuth 2.0. O LocalAI gerenciará todo o fluxo OAuth, incluindo a renovação de tokens.
...
<Steps>
    ...
    <Step title="Crie e autorize">
        Clique em **Create MCP Server**. Você será redirecionado ao seu provedor OAuth para autorizar o acesso. Uma vez autorizado, o LocalAI armazenará os tokens e os renovará automaticamente conforme necessário.
    </Step>
</Steps>
...
- **Atribuir ferramentas a agentes** nas suas crews, assim como qualquer outra ferramenta LocalAI.
