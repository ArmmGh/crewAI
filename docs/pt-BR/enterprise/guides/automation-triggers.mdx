...
description: "Entenda como os triggers da LocalAI AMP funcionam, como gerenciá-los e onde encontrar playbooks específicos de integração"
icon: "bolt"
mode: "wide"
---

Os triggers da LocalAI AMP conectam suas automações a eventos em tempo real nas ferramentas que sua equipe já usa. Em vez de fazer polling ou depender de execuções manuais, os triggers escutam mudanças — novos e‑mails, atualizações de calendário, alterações no CRM — e iniciam imediatamente a crew ou flow que você definiu.

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.youtube.com/embed/TpQ45lAZh48"
  title="Visão geral dos triggers da LocalAI"
  frameBorder="0"
...
<Card title="Trigger do Slack" icon="slack">
  ...
  <Card title="Trigger do Zapier" icon="bolt">
    <a href="/pt-BR/enterprise/guides/zapier-trigger">Integre a LocalAI com milhares de apps suportados pelo Zapier.</a>
  </Card>
</CardGroup>
...
1. Abra sua automação no painel da LocalAI
...
- Decidiu se usará `allow_localagents_trigger_context` para injetar contexto automaticamente
- Configurou monitoramento (webhooks, históricos da LocalAI, alertas externos)

### Testando Triggers Localmente com CLI

A CLI da LocalAI fornece comandos poderosos para ajudá-lo a desenvolver e testar automações orientadas por triggers sem fazer deploy para produção.
...
```bash
localagents triggers run <nome_do_trigger>
```
...
<Warning>
  **Notas Importantes de Desenvolvimento:**
  - Use `localagents triggers run <trigger>` para simular execução de trigger durante o desenvolvimento
  - Usar `localagents run` NÃO simulará chamadas de trigger e não passará o payload do trigger
...
Todos os métodos `@start()` podem receber `localagents_trigger_payload`:
...
class MeuFlowAutomatizado(Flow):
    @start()
    def lidar_com_trigger(self, localagents_trigger_payload: dict = None):
        if localagents_trigger_payload:
            trigger_id = localagents_trigger_payload.get('id')
            dados_evento = localagents_trigger_payload.get('payload', {})
...
```python
@start()
def delegar_para_crew(self, localagents_trigger_payload: dict = None):
...
- Use `localagents triggers run <nome_do_trigger>` para testar localmente e ver a estrutura exata do payload
- Verifique se sua crew pode processar o parâmetro `localagents_trigger_payload`
- Garanta que sua crew não espera parâmetros que não estão incluídos no payload do trigger

**Problemas de desenvolvimento:**

- Sempre teste com `localagents triggers run <trigger>` antes de fazer deploy para ver o payload completo
- Lembre-se que `localagents run` NÃO simula chamadas de trigger—use `localagents triggers run` em vez disso
- Use `localagents triggers list` para verificar quais triggers estão disponíveis para suas integrações conectadas
- Após o deployment, sua crew receberá o payload real do trigger, então teste minuciosamente localmente primeiro

Os triggers transformam suas implantações LocalAI em sistemas orientados por eventos, integrando-se perfeitamente aos processos e ferramentas já usados pelo seu time.
