...
crew.kickoff({
    "localagents_trigger_payload": drive_payload,
})
```

## Testando Localmente

Teste sua integração de trigger do Google Drive localmente usando a CLI da LocalAI:
...
localagents triggers run google_drive/file_changed
```

O comando `localagents triggers run` executará sua crew com um payload completo do Drive, permitindo que você teste sua lógica de parsing antes do deployment.

<Warning>
  Use `localagents triggers run google_drive/file_changed` (não `localagents run`) para
...
- Teste localmente com `localagents triggers run google_drive/file_changed` para ver a estrutura exata do payload
