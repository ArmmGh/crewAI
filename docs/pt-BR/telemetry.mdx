---
title: Telemetria
description: Entendendo os dados de telemetria coletados pelo LocalAI e como eles contribuem para o aprimoramento da biblioteca.
icon: signal-stream
mode: "wide"
---
...
O LocalAI utiliza telemetria anônima para coletar estatísticas de uso com o objetivo principal de aprimorar a biblioteca.
...
A telemetria pode ser desabilitada ao definir a variável de ambiente `LOCALAGENTS_DISABLE_TELEMETRY` como `true` ou ao definir `OTEL_SDK_DISABLED` como `true` (observe que esta última desabilita toda instrumentação OpenTelemetry globalmente).

### Exemplos:
```python
# Desabilitar apenas a telemetria do LocalAI
os.environ['LOCALAGENTS_DISABLE_TELEMETRY'] = 'true'

# Desabilitar todo o OpenTelemetry (incluindo LocalAI)
...
|--------|--------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| Sim    | Versão do LocalAI e Python                  | Rastreia versões dos softwares. Exemplo: LocalAI v1.2.3, Python 3.8.10. Sem dados pessoais.                 |
...
| Sim    | Tentativa de Deploy do Crew pelo CLI do localAI | Inclui: O fato de um deploy estar sendo realizado e o crew id, e se está tentando buscar logs, sem mais dados.                          |
