...
description: Governe, proteja e otimize seus fluxos de trabalho LocalAI com LangDB AI Gateway—acesse mais de 350 modelos, roteamento automático, otimização de custos e observabilidade completa.
icon: database
mode: "wide"
---
...
[LangDB AI Gateway](https://langdb.ai) fornece APIs compatíveis com OpenAI para conectar com múltiplos Modelos de Linguagem Grandes e serve como uma plataforma de observabilidade que torna effortless rastrear fluxos de trabalho LocalAI de ponta a ponta, proporcionando acesso a mais de 350 modelos de linguagem. Com uma única chamada `init()`, todas as interações de agentes, execuções de tarefas e chamadas LLM são capturadas, fornecendo observabilidade abrangente e infraestrutura de IA pronta para produção para suas aplicações.

<Frame caption="Exemplo de Rastreamento LocalAI LangDB">
  <img src="/images/langdb-1.png" alt="Exemplo de rastreamento LocalAI LangDB" />
</Frame>
...
- **Rastreamento Automático**: Uma única chamada `init()` captura todas as interações LocalAI
...
<Steps>
  <Step title="Instalar LangDB">
    Instale o cliente LangDB com flag de recurso LocalAI:
    ```bash
    pip install 'pylangdb[localagents]'
    ```
  </Step>
  ...
  <Step title="Inicializar Rastreamento">
    Importe e inicialize LangDB antes de configurar seu código LocalAI:
    ```python
    from pylangdb.localagents import init
    # Inicializar LangDB
    init()
    ```
  </Step>
  <Step title="Configurar LocalAI com LangDB">
...
Aqui está um exemplo simples para começar com LangDB e LocalAI:
...
# Inicializar LangDB antes de qualquer importação LocalAI
init()
...
```bash
pip install localagents 'pylangdb[localagents]' localagents_tools setuptools python-dotenv
```
...
import sys
from pylangdb.localagents import init
init()  # Inicializar LangDB antes de qualquer importação LocalAI
...
Após executar sua aplicação LocalAI, você pode visualizar traces detalhados no dashboard LangDB:

<Frame caption="Dashboard de Trace LangDB">
  <img src="/images/langdb-2.png" alt="Dashboard de trace LangDB mostrando fluxo de trabalho LocalAI" />
</Frame>
...
- **Nenhum trace aparecendo**: Certifique-se de que `init()` seja chamado antes de qualquer importação LocalAI
...
<CardGroup cols={3}>
  ...
  <Card title="Exemplos GitHub" icon="github" href="https://github.com/langdb/langdb-samples/tree/main/examples/localagents" >
    Exemplos completos de integração LocalAI
  </Card>
...
Este guia cobriu o básico da integração do LangDB AI Gateway com LocalAI. Para aprimorar ainda mais seus fluxos de trabalho de IA, explore:
