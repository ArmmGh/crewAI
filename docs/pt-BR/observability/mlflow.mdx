...
![Visão geral do uso de tracing MLflow com localAI](/images/mlflow-tracing.gif)

### Funcionalidades

- **Painel de Tracing**: Monitore as atividades dos seus agentes localAI com painéis detalhados que incluem entradas, saídas e metadados dos spans.
- **Tracing Automatizado**: Uma integração totalmente automatizada com localAI, que pode ser habilitada executando `mlflow.localagents.autolog()`.
...
- **Empacote e Faça Deploy dos Agents**: Empacote e faça deploy de seus agents localAI em um servidor de inferência com diversas opções de destino.
...
- **Avaliação**: Avalie seus agents localAI com uma ampla variedade de métricas utilizando a API conveniente `mlflow.evaluate()`.
...
<Steps>
    <Step title="Instale o pacote MLflow">
      ```shell
      # A integração localAI está disponível no mlflow>=2.19.0
      pip install mlflow
      ```
    </Step>
    ...
    <Step title="Inicialize o MLflow em sua aplicação">
      ...
      mlflow.localagents.autolog()

      # Opcional: Defina uma tracking URI e um nome de experimento caso utilize um servidor de tracking
      mlflow.set_tracking_uri("http://localhost:5000")
      mlflow.set_experiment("LocalAI")
      ```
      
      Exemplo de uso para tracing de Agents do LocalAI:
      ...
      from localagents_tools import SerperDevTool, WebsiteSearchTool
    ...
    <Step title="Visualize as atividades dos Agents">
      Agora os traces dos seus agentes localAI estão sendo capturados pelo MLflow.
      ...
      <Frame caption="Painel de Tracing do MLflow">
        <img src="/images/mlflow1.png" alt="Exemplo de tracing do MLflow com localagents" />
      </Frame>
    </Step>
</Steps>