...
Maxim AI oferece monitoramento completo de agentes, avaliação e observabilidade para suas aplicações LocalAI. Com a integração de uma linha do Maxim, você pode facilmente rastrear e analisar interações dos agentes, métricas de desempenho e muito mais.
...
# Instrumente o LocalAI com apenas uma linha
instrument_localagents(logger)
```

### 4. Crie e execute sua aplicação LocalAI normalmente
...
É isso! Todas as interações dos seus agentes LocalAI agora serão registradas e estarão disponíveis em seu painel Maxim.
...
Após executar sua aplicação LocalAI:
...
- Certifique-se de que você **chamou `instrument_localagents()`** ***antes*** de executar sua crew. Isso inicializa corretamente os hooks de logging.
- Defina `debug=True` na chamada do `instrument_localagents()` para expor erros internos:
...
- Verifique cuidadosamente se `instrument_localagents()` foi chamado **antes** de criar ou executar agentes. Isso pode parecer óbvio, mas é um erro comum.
