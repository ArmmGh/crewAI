---
title: MultiOn Tool
description: O `MultiOnTool` permite que agentes LocalAI naveguem e interajam com a web por meio de instruções em linguagem natural.
icon: globe
mode: "wide"
---
...
1. **Instale o LocalAI**: Certifique-se de que o pacote `localagents[tools]` esteja instalado em seu ambiente Python.
...
O `MultiOnTool` é implementado como uma subclasse de `BaseTool` do LocalAI. Ele envolve o cliente MultiOn para fornecer capacidades de navegação web:
...
O `MultiOnTool` oferece uma maneira poderosa de integrar capacidades de navegação web em agentes LocalAI. Ao permitir que agentes interajam com sites por meio de instruções em linguagem natural, amplia significativamente as possibilidades para tarefas baseadas na web, desde coleta de dados e pesquisa até interações automatizadas com serviços online.