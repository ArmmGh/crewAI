...
O pacote `localagents_tools`, que incluirá o PGSearchTool assim que for lançado, pode ser instalado usando o comando abaixo:
...
<Note>
    O PGSearchTool ainda não está disponível na versão atual do pacote `localagents_tools`. Este comando de instalação será atualizado assim que a ferramenta for lançada.
</Note>
