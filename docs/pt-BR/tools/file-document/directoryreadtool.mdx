...
Para utilizar o DirectoryReadTool em seu projeto, instale o pacote `localagents_tools`. Se este pacote ainda não faz parte do seu ambiente, você pode instalá-lo usando o pip com o comando abaixo:

```shell
pip install 'localagents[tools]'
```

Esse comando instala a versão mais recente do pacote `localagents_tools`, permitindo o acesso ao DirectoryReadTool, entre outras utilidades.
