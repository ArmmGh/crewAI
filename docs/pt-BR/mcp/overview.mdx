---
title: "Servidores MCP como Ferramentas no LocalAI"
description: "Aprenda como integrar servidores MCP como ferramentas nos seus agentes LocalAI usando a biblioteca `localagents-tools`."
icon: plug
mode: "wide"
---
...
O LocalAI oferece **duas abordagens** para integração MCP:
...
Para casos de uso avançados que requerem gerenciamento manual de conexão, a biblioteca `localagents-tools` fornece a classe `MCPServerAdapter`.
...
Assista a este tutorial em vídeo para um guia abrangente sobre a integração do MCP com o LocalAI:
...
Antes de começar a usar MCP com `localagents-tools`, é necessário instalar a dependência extra `mcp` do `localagents-tools` com o seguinte comando:
...
A classe `MCPServerAdapter` da `localagents-tools` é a principal forma de conectar-se a um servidor MCP e disponibilizar suas ferramentas aos seus agentes LocalAI. Ela suporta diferentes mecanismos de transporte e simplifica o gerenciamento de conexões.
...
Confira este repositório para demonstrações completas e exemplos de integração MCP com LocalAI! 👇
...
- **Primitivas Suportadas**: Atualmente, o `MCPServerAdapter` suporta principalmente a adaptação de `tools` MCP.
  Outras primitivas MCP como `prompts` ou `resources` não são integradas diretamente como componentes LocalAI através deste adaptador por enquanto.
