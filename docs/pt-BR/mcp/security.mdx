...
description: Saiba mais sobre as principais melhores práticas de segurança ao integrar servidores MCP com seus agentes LocalAI.
icon: lock
mode: "wide"
---
...
<Warning>
O aspecto mais crítico da segurança MCP é a **confiança**. Você deve **apenas** conectar seus agentes LocalAI a servidores MCP nos quais confie plenamente.
</Warning>

Ao integrar serviços externos como servidores MCP (Model Context Protocol) aos seus agentes LocalAI, a segurança é fundamental.
...
1. Quando seu agente LocalAI se conecta a um servidor MCP, ele normalmente solicita uma lista de ferramentas disponíveis.
...
O transporte Stdio (Entrada/Saída Padrão) é tipicamente usado para servidores MCP locais, rodando na mesma máquina que sua aplicação LocalAI.
...
**Implicação para Usuários LocalAI:**
...
- **Criptografe Dados em Trânsito**: Sempre use HTTPS (HTTP Seguro) para URLs de servidores MCP remotos. Isso criptografa a comunicação entre sua aplicação LocalAI e o servidor MCP, protegendo contra escuta e ataques Man-in-the-Middle (MitM). O `MCPServerAdapter` respeitará o esquema (`http` ou `https`) fornecido na URL.
...
- **Verifique a Identidade**: Se o servidor MCP fornece ferramentas sensíveis ou acesso a dados privados, ele DEVE implementar mecanismos de autenticação robustos para verificar a identidade do cliente (sua aplicação LocalAI). Isso pode envolver chaves de API, tokens OAuth ou outros métodos padrão.
...
- **Atenção do Lado do Cliente**: Embora a validação no servidor seja fundamental, como usuário LocalAI, fique atento aos dados que seus agentes são configurados para enviar a ferramentas MCP, especialmente ao interagir com servidores MCP novos ou menos confiáveis.
...
- **Re-tentativas do Lado do Cliente**: Implemente lógica de repetição sensata em suas tarefas LocalAI se problemas de rede transitórios ou limites de taxa do servidor forem esperados, mas evite re-tentativas agressivas que possam aumentar a carga do servidor.

## 4. Conselhos para Implementação de Servidor MCP Seguro (Para Desenvolvedores)

Se você está desenvolvendo um servidor MCP ao qual agentes LocalAI possam se conectar, considere estas melhores práticas além dos pontos acima:
...
Ao entender essas considerações de segurança e implementar as melhores práticas, você pode aproveitar com segurança o poder dos servidores MCP em seus projetos LocalAI.
