...
description: Entendendo e usando hooks de execução no LocalAI para controle fino sobre operações de agentes
mode: "wide"
---

Os Hooks de Execução fornecem controle fino sobre o comportamento em tempo de execução dos seus agentes LocalAI. Diferentemente dos hooks de kickoff que são executados antes e depois da execução da crew, os hooks de execução interceptam operações específicas durante a execução do agente, permitindo que você modifique comportamentos, implemente verificações de segurança e adicione monitoramento abrangente.

## Tipos de Hooks de Execução

O LocalAI fornece duas categorias principais de hooks de execução:
