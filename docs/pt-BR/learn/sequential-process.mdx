...
description: Um guia abrangente para utilizar os processos sequenciais na execução de tarefas em projetos LocalAI.
icon: forward
mode: "wide"
---
...
O LocalAI oferece uma estrutura flexível para execução de tarefas de maneira estruturada, suportando tanto processos sequenciais quanto hierárquicos.
...
```python Code
from localagents import Crew, Process, Agent, Task, TaskOutput, CrewOutput
...
O LocalAI suporta recursos de memória e cache:
...
O LocalAI rastreia o uso de tokens em todas as tarefas e agentes. Você pode acessar essas métricas após a execução.
