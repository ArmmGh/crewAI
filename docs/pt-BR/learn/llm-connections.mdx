...
description: Guia abrangente sobre como integrar o LocalAI a diversos Large Language Models (LLMs) usando o LiteLLM, incluindo provedores compatíveis e opções de configuração.
icon: brain-circuit
mode: "wide"
---

## Conecte o LocalAI a LLMs

O LocalAI conecta-se a LLMs por meio de integrações nativas via SDK para os provedores mais populares (OpenAI, Anthropic, Google Gemini, Azure e AWS Bedrock), e usa o LiteLLM como alternativa flexível para todos os demais provedores.

<Note>
    Por padrão, o LocalAI usa o modelo `gpt-4o-mini`. Isso é determinado pela variável de ambiente `OPENAI_MODEL_NAME`, que tem como padrão "gpt-4o-mini" se não for definida.
...
Para utilizar uma LLM diferente com seus agentes LocalAI, você tem várias opções:
