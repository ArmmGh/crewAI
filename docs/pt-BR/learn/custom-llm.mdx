...
description: Aprenda a criar implementações personalizadas de LLM no LocalAI.
icon: code
mode: "wide"
---
...
O LocalAI oferece suporte a implementações personalizadas de LLM por meio da classe abstrata `BaseLLM`. Isso permite integrar qualquer provedor de LLM que não tenha suporte nativo no LiteLLM ou implementar mecanismos de autenticação personalizados.
...
Este guia cobre o essencial para implementar LLMs personalizados no LocalAI.