...
description: "Framework estratégico para escolher o LLM certo para seus agentes LocalAI e escrever definições eficazes de tarefas e agentes"
icon: "brain-circuit"
mode: "wide"
---

## A Abordagem LocalAI para Seleção de LLMs
...
As implementações LocalAI mais sofisticadas empregam múltiplos modelos estrategicamente, designando-os conforme as funções e necessidades dos agentes. Assim, é possível otimizar desempenho e custos usando o modelo mais adequado para cada tipo de tarefa.
...
<Tabs>
  <Tab title="Manager LLM">
    O manager LLM desempenha papel central em fluxos hierárquicos LocalAI, coordenando agentes e tarefas. Este modelo precisa se destacar em delegação, priorização de tarefas e manutenção de contexto em várias operações simultâneas.
...
Definir bem as tarefas é frequentemente mais importante do que a seleção do modelo no resultado gerado pelos agentes LocalAI. Tarefas bem formuladas orientam claramente mesmo modelos simples a terem bom desempenho. Já tarefas mal definidas prejudicam até os modelos mais avançados.
...
Em vez de repetir o framework estratégico, segue um checklist tático para implementar as decisões de seleção de LLM em LocalAI:
...
<Steps>
  ...
  <Step title="Valide com Testes Empresariais" icon="test-tube">
    **Após colocar os agentes em produção:**
    - Use [LocalAI AMP platform](https://app.localagents.com) para testar seleções de modelo A/B
...
## Armadilhas Comuns na Seleção de Modelos LocalAI
...
<AccordionGroup>
  <Accordion title="A Armadilha do 'Um Modelo Serve para Tudo'" icon="triangle-exclamation">
    ...
    **Solução LocalAI**: Configure modelos específicos por agente:
    ```python
  ...
  <Accordion title="Ignorar Hierarquia de LLM entre Crew e Agente" icon="shuffle">
    **O problema**: Não entender como funciona a hierarquia LLM da LocalAI — configurações conflitam entre crew, manager e agentes.

    **Exemplo real**: Configurar crew com Claude, mas agentes com GPT, gerando comportamento inconsistente e trocas desnecessárias.

    **Solução LocalAI**: Planeje a hierarquia estrategicamente:
    ```python
    crew = Crew(
  ...
  <Accordion title="Incompatibilidade para Function Calling" icon="screwdriver-wrench">
    ...
    **Solução LocalAI**: Priorize desempenho em function calling para agentes que usam ferramentas:
    ```python
  ...
  <Accordion title="Otimização Prematura sem Teste" icon="gear">
    **O problema**: Decidir configurações complexas de modelo com base em hipóteses não validadas nos fluxos e tarefas reais LocalAI.
    ...
    **Solução LocalAI**: Comece simples e otimize baseado em dados reais:
    ```python
    # Comece assim
  ...
  <Accordion title="Ignorar Limites de Contexto e Memória" icon="brain">
    **O problema**: Não considerar como janela de contexto dos modelos interage com memória e compartilhamento de contexto entre agentes LocalAI.
    ...
    **Solução LocalAI**: Alinhe capacidades de contexto ao padrão de comunicação da crew.
...
Para equipes sérias sobre otimização, a **plataforma LocalAI AMP** oferece testes sofisticados que vão além do CLI. Ela permite avaliação completa para decisões orientadas por dados na estratégia de LLM.
...
Acesse [app.localagents.com](https://app.localagents.com) para começar!
...
As tabelas abaixo mostram uma amostra dos modelos de maior destaque em cada categoria, junto de orientação sobre aplicação em agentes LocalAI:
...
<Tabs>
  <Tab title="Raciocínio & Planejamento">
    **Melhores para LLMs Manager e Análises Complexas**
    ...
    | Modelo | Score de Inteligência | Custo ($/M tokens) | Velocidade | Melhor Uso em LocalAI |
    |:------|:---------------------|:-------------------|:-----------|:--------------------|
  ...
  <Tab title="Codificação & Técnica">
    **Melhores para Desenvolvimento e Workflows com Ferramentas**
    ...
    | Modelo | Performance em Coding | Tool Use Score | Custo ($/M tokens) | Melhor Uso em LocalAI |
    |:--------|:---------------------|:--------------|:-------------------|:--------------------|
  ...
  <Tab title="Velocidade & Eficiência">
    **Melhores para Operações em Massa e Aplicações em Tempo Real**
    ...
    | Modelo | Velocidade (tokens/s) | Latência (TTFT) | Custo ($/M tokens) | Melhor Uso em LocalAI |
    |:-------|:---------------------|:----------------|:-------------------|:---------------------|
  ...
  <Tab title="Performance Equilibrada">
    **Melhores Modelos Coringa para Crews Diversos**
    ...
    | Modelo | Score Global | Versatilidade | Custo ($/M tokens) | Melhor Uso em LocalAI |
    |:------------|:--------------|:-------------|:-------------------|:--------------------|
