---
title: 'LLMs'
description: 'Um guia abrangente para configurar e usar Modelos de Linguagem de Grande Escala (LLMs) em seus projetos LocalAI'
icon: 'microchip-ai'
mode: "wide"
---
...
O LocalAI integra-se com múltiplos provedores de LLM através do LiteLLM, oferecendo flexibilidade para você escolher o modelo certo para o seu caso de uso específico. Este guia irá ajudá-lo a entender como configurar e usar diferentes provedores de LLM em seus projetos LocalAI.

## O que são LLMs?

Modelos de Linguagem de Grande Escala (LLMs) são a inteligência central por trás dos agentes LocalAI. Eles permitem que os agentes compreendam o contexto, tomem decisões e gerem respostas semelhantes às humanas. Veja o que você precisa saber:

<CardGroup cols={2}>
  <Card title="Noções Básicas de LLM" icon="brain">
    Modelos de Linguagem de Grande Escala são sistemas de IA treinados em grandes volumes de dados textuais. Eles potencializam a inteligência dos agentes LocalAI, permitindo compreender e gerar textos de voz humana.
  </Card>
...
Existem diferentes locais no código do LocalAI onde você pode especificar o modelo a ser utilizado. Após definir o modelo usado, será necessário fornecer a configuração (como uma chave de API) para cada provedor de modelo. Veja a seção de [exemplos de configuração de provedores](#provider-configuration-examples) para seu provedor.

<Tabs>
  <Tab title="1. Variáveis de Ambiente">
    A maneira mais simples de começar. Defina o modelo diretamente em seu ambiente, usando um arquivo `.env` ou no código do seu aplicativo. Se você utilizou `localagents create` para iniciar seu projeto, já estará configurado.
...
<Info>
  O LocalAI oferece integrações nativas via SDK para OpenAI, Anthropic, Google (Gemini API), Azure e AWS Bedrock — sem necessidade de instalação extra além dos extras específicos do provedor (ex.: `uv add "localagents[openai]"`).
...
O LocalAI suporta uma grande variedade de provedores de LLM, cada um com recursos, métodos de autenticação e capacidades de modelo únicos.
...
<AccordionGroup>
  <Accordion title="OpenAI">
    ...
    Exemplo de uso em seu projeto LocalAI:
    ```python Code
    from localagents import LLM
  ...
  <Accordion title="Meta-Llama">
    ...
    A API está disponível através da [Meta Llama API](https://llama.developer.meta.com?utm_source=partner-localagents&utm_medium=website).
    Defina as seguintes variáveis de ambiente no seu arquivo `.env`:
  ...
  <Accordion title="Local NVIDIA NIM Deployed using WSL2">
    ...
    3. Configure seus modelos locais localagents:
...
O LocalAI suporta respostas em streaming de LLMs, permitindo que sua aplicação receba e processe saídas em tempo real assim que são geradas.
...
<Tabs>
  ...
  <Tab title="Manipulação de Eventos">
    O LocalAI emite eventos para cada chunk recebido durante o streaming:
...
O LocalAI suporta respostas estruturadas de LLMs permitindo que você defina um `response_format` usando um modelo Pydantic. Isso permite que o framework automaticamente faça o parsing e valide a saída, facilitando a integração da resposta em sua aplicação sem pós-processamento manual.
...
<AccordionGroup>
  <Accordion title="Gestão da Janela de Contexto">
    O LocalAI inclui recursos inteligentes para gerenciamento de contexto:

    ```python
    from localagents import LLM

    # O LocalAI automaticamente gerencia:
  ...
  <Accordion title="Descartar Parâmetros Adicionais">
    O LocalAI usa Litellm internamente para chamadas LLM, permitindo descartar parâmetros adicionais desnecessários para seu caso de uso. Isso pode simplificar seu código e reduzir a complexidade da configuração do LLM.
