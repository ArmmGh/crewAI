---
title: CLI
description: Aprenda a usar o CLI do LocalAI para interagir com o LocalAI.
icon: terminal
mode: "wide"
---

<Warning>
  A partir da versão 0.140.0, a plataforma LocalAI AMP iniciou um processo de
  ...
  anteriores da biblioteca `localagents`.
</Warning>

## Visão Geral

O CLI do LocalAI fornece um conjunto de comandos para interagir com o LocalAI, permitindo que você crie, treine, execute e gerencie crews & flows.

## Instalação

Para usar o CLI do LocalAI, certifique-se de que o LocalAI está instalado:
...
A estrutura básica de um comando CLI do LocalAI é:
...
Mostre a versão instalada do LocalAI.

```shell Terminal
localagents version [OPTIONS]
```

- `--tools`: (Opcional) Mostra a versão instalada das ferramentas do LocalAI
...
n_iterations = 2
inputs = {"topic": "Treinamento LocalAI"}
...
<Note>
  A partir da versão 0.103.0, o comando `localagents run` pode ser usado para
...
<Note>
  ...
  projeto LocalAI está configurado. Alguns comandos podem exigir configuração ou
  ajustes adicionais dentro da estrutura do seu projeto.
</Note>

### 9. Chat

A partir da versão `0.98.0`, ao rodar o comando `localagents chat`, você inicia uma sessão interativa com seu crew. O assistente de IA irá guiá-lo solicitando as entradas necessárias para executar o crew. Uma vez que todas as entradas são fornecidas, o crew executará suas tarefas.
...
<Note>
Garanta que você execute estes comandos a partir do diretório raiz do seu projeto LocalAI.
</Note>
<Note>
...
Implemente o crew ou flow no [LocalAI AMP](https://app.localagents.com).

- **Autenticação**: Você precisa estar autenticado para implementar no LocalAI AMP.
...
Gerencie suas organizações no LocalAI AMP.
...
<Note>
  Você deve estar autenticado no LocalAI AMP para usar estes comandos de
...
- **Implantar o Crew**: Depois de autenticado, você pode implantar seu crew ou flow no LocalAI AMP.

  ```shell Terminal
  localagents deploy push
  ```

  - Inicia o processo de deployment na plataforma LocalAI AMP.
...
- **Deletar um deployment**: Você pode deletar um deployment com:
  ...
  Isto exclui o deployment da plataforma LocalAI AMP.

- **Comando de Ajuda**: Você pode obter ajuda sobre o CLI com:
  ```shell Terminal
  localagents deploy --help
  ```
  Isto exibe a mensagem de ajuda para o CLI LocalAI Deploy.

Assista ao vídeo tutorial para uma demonstração passo-a-passo de implantação do seu crew no [LocalAI AMP](http://app.localagents.com) usando o CLI.
...
Ao executar o comando `localagents create crew`, o CLI primeiro mostrará os 5 provedores de LLM mais comuns e pedirá para você selecionar um.
...
Gerencie as configurações do CLI para LocalAI.

```shell Terminal
localagents config [COMANDO] [OPÇÕES]
```
...
```shell Terminal
localagents config set <chave> <valor>
```
...
- `enterprise_base_url`: URL base da instância LocalAI AMP
...
| :------------------ | :--------------------- | :------------------------------------------------------------- |
| enterprise_base_url | https://app.localagents.com | URL base da instância LocalAI AMP                               |
...
| oauth2_domain       | login.localagents.com       | Domínio do provedor OAuth2 (ex.: your-org.auth0.com)           |

Definir a URL base do enterprise:

```shell Terminal
localagents config set enterprise_base_url https://minha-empresa.localagents.com
```
...
```shell Terminal
localagents config set oauth2_domain minha-empresa.auth0.com
```
...
<Note>
  As configurações são armazenadas em `~/.config/localagents/settings.json`. Algumas
