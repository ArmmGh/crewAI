...
description: Melhores práticas para construir aplicações de IA prontas para produção com LocalAI
icon: server
mode: "wide"
---
...
Ao construir aplicações de IA de produção com LocalAI, **recomendamos começar com um Flow**.
...
3.  **Observabilidade**: Flows fornecem uma estrutura clara que facilita o rastreamento da execução, a depuração de problemas e o monitoramento do desempenho. Recomendamos o uso do [LocalAI Tracing](/pt-BR/observability/tracing) para insights detalhados. Basta executar `localagents login` para habilitar recursos de observabilidade gratuitos.

## A Arquitetura

Uma aplicação LocalAI de produção típica se parece com isso:
...
Aproveite as primitivas de controle do LocalAI para adicionar robustez e controle aos seus Crews.
...
### LocalAI Enterprise
A maneira mais fácil de implantar seu Flow é usando o LocalAI Enterprise. Ele lida com a infraestrutura, autenticação e monitoramento para você.
