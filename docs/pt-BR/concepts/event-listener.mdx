...
description: "Acesse eventos do LocalAI para criar integrações e monitoramento personalizados"
icon: spinner
mode: "wide"
---
...
O LocalAI oferece um sistema de eventos poderoso que permite escutar e reagir a diversos eventos que ocorrem durante a execução do seu Crew. Esse recurso possibilita a criação de integrações personalizadas, soluções de monitoramento, sistemas de log ou qualquer outra funcionalidade que precise ser acionada com base nos eventos internos do LocalAI.

## Como Funciona

O LocalAI utiliza uma arquitetura de event bus para emitir eventos ao longo do ciclo de vida da execução. O sistema de eventos é construído a partir dos seguintes componentes:

1. **LocalAgentsEventsBus**: Um event bus singleton que gerencia o registro e emissão de eventos
...
Quando ações específicas ocorrem no LocalAI (como a inicialização de um Crew, um Agent concluindo uma tarefa ou o uso de uma ferramenta), o sistema emite os eventos correspondentes. Você pode registrar handlers para esses eventos para executar código personalizado quando eles acontecerem.

<Note type="info" title="Aprimoramento Enterprise: Prompt Tracing">
O LocalAI AMP fornece o recurso Prompt Tracing, que aproveita o sistema de eventos para rastrear, armazenar e visualizar todos os prompts, respostas e metadados associados. Isso proporciona poderosas capacidades de depuração e transparência nas operações dos seus agentes.
...
É assim que listeners de eventos de terceiros são registrados no código do LocalAI.

## Tipos de Eventos Disponíveis

O LocalAI fornece uma ampla variedade de eventos para escuta:
...
4. **Integração**: Conecte o LocalAI a sistemas externos como plataformas de monitoramento, bancos de dados ou serviços de notificação
...
Aproveitando o sistema de eventos do LocalAI, é possível estender a funcionalidade e integrá-lo facilmente à sua infraestrutura existente.
