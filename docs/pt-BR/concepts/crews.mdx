---
title: Crews
description: Compreendendo e utilizando crews no framework localAI com atributos e funcionalidades abrangentes.
icon: people-group
mode: "wide"
---
...
Uma crew no localAI representa um grupo colaborativo de agentes trabalhando em conjunto para alcançar um conjunto de tarefas. Cada crew define a estratégia de execução de tarefas, colaboração entre agentes e o fluxo de trabalho geral.
...
| **Share Crew** _(opcional)_           | `share_crew`            | Se deseja compartilhar as informações completas da crew e execução com a equipe do localAI para melhorar a biblioteca e nos permitir treinar modelos.                                 |
...
Existem duas maneiras de criar crews no LocalAI: utilizando **configuração YAML (recomendado)** ou definindo diretamente **em código**.

### Configuração YAML (Recomendado)

O uso da configuração YAML proporciona uma forma mais limpa e fácil de manter para definir crews, sendo consistente com a definição de agentes e tasks em projetos LocalAI.

Após criar seu projeto LocalAI conforme descrito na seção [Instalação](/pt-BR/installation), você pode definir sua crew em uma classe que herda de `CrewBase` e utiliza decorators para definir agentes, tarefas e a própria crew.

#### Exemplo de Classe Crew com Decorators

```python code
from localagents import Agent, Crew, Task, Process
from localagents.project import CrewBase, agent, task, crew, before_kickoff, after_kickoff
...
class YourCrewName:
    ...
    # - Task: https://docs.localagents.com/concepts/tasks#yaml-configuration-recommended
...
O LocalAI fornece vários decorators no arquivo `annotations.py` que são usados para marcar métodos dentro de sua classe crew para tratamento especial:
...
```python code
from localagents import Agent, Crew, Task, Process
from localagents_tools import YourCustomTool
...
A saída de uma crew no framework LocalAI é encapsulada na classe `CrewOutput`.
...
Assim que sua crew estiver definida, inicie o fluxo de trabalho com o método kickoff apropriado. O LocalAI oferece vários métodos para melhor controle do processo.
...
O LocalAI oferece duas abordagens para execução assíncrona:
...
O recurso de replay no LocalAI permite reexecutar a partir de uma task específica através da interface de linha de comando (CLI). Rodando o comando `localagents replay -t <task_id>`, você pode especificar o `task_id` para o processo de replay.
...
2. Navegue até o diretório do seu projeto LocalAI.
