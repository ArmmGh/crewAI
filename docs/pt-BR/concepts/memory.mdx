---
title: Memória
description: Aproveitando o sistema de memória unificado no LocalAI para aprimorar as capacidades dos agentes.
icon: database
mode: "wide"
---
...
Isso evita o acúmulo de duplicatas. Por exemplo, se você salvar "LocalAI garante operação confiável" três vezes, a consolidação reconhece as duplicatas e mantém apenas um registro.
...
memory.remember_many([
    ...
    "LocalAI supports complex workflows.",  # descartado pelo dedup intra-batch
])
```
...
- **Padrão**: LanceDB, armazenado em `./.localagents/memory` (ou `$LOCALAGENTS_STORAGE_DIR/memory` se a variável de ambiente estiver definida, ou o caminho que você passar como `storage="path/to/dir"`).
- **Backend customizado**: Implemente o protocolo `StorageBackend` (veja `localagents.memory.storage.backend`) e passe uma instância para `Memory(storage=your_backend)`.
...
- Garanta que o caminho de armazenamento seja gravável (padrão `./.localagents/memory`). Passe `storage="./your_path"` para usar outro diretório, ou defina a variável de ambiente `LOCALAGENTS_STORAGE_DIR`.
...
```bash
localagents memory                              # Abre o navegador TUI
localagents memory --storage-path ./my_memory   # Apontar para um diretório específico
```
