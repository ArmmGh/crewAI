---
title: Ferramentas
description: Compreendendo e aproveitando ferramentas dentro do framework LocalAI para colaboração e execução de tarefas por agentes.
...
As ferramentas do LocalAI capacitam agentes com habilidades que vão desde busca na web e análise de dados até colaboração e delegação de tarefas entre colegas de trabalho.
Esta documentação descreve como criar, integrar e aproveitar essas ferramentas dentro do framework LocalAI, incluindo um novo foco em ferramentas de colaboração.

## O que é uma Ferramenta?

Uma ferramenta no LocalAI é uma habilidade ou função que os agentes podem utilizar para executar diversas ações.
Isso inclui ferramentas do [LocalAI Toolkit](https://github.com/joaomdmoura/localagents-tools) e [LangChain Tools](https://python.langchain.com/docs/integrations/tools),
...
O LocalAI AMP oferece um Repositório de Ferramentas abrangente, com integrações pré-construídas para sistemas empresariais e APIs comuns. Implemente agentes com ferramentas corporativas em minutos em vez de dias.
...
## Utilizando Ferramentas LocalAI

Para aprimorar as capacidades de seus agentes com as ferramentas do LocalAI, comece instalando nosso pacote extra de ferramentas:
...
# Importando ferramentas do localAI
...
## Ferramentas LocalAI Disponíveis
...
Existem duas formas principais de criar uma ferramenta LocalAI:
...
O LocalAI suporta ferramentas assíncronas, permitindo que você implemente ferramentas que realizam operações não bloqueantes, como requisições de rede, I/O de arquivos ou outras operações async sem bloquear o fluxo principal de execução.
...
O framework LocalAI lida automaticamente com a execução de ferramentas síncronas e assíncronas, então você não precisa se preocupar com diferenças na chamada.
...
Ferramentas são fundamentais para expandir as capacidades dos agentes LocalAI, permitindo que assumam uma ampla gama de tarefas e colaborem de forma eficiente.
Ao construir soluções com LocalAI, aproveite tanto ferramentas existentes quanto personalizadas para potencializar seus agentes e ampliar o ecossistema de IA. Considere utilizar tratamento de erros,
