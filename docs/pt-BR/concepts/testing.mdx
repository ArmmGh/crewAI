---
title: Testes
description: Saiba como testar sua LocalAI Crew e avaliar seu desempenho.
icon: vial
mode: "wide"
---
...
Testar é uma parte crucial do processo de desenvolvimento, sendo essencial para garantir que sua crew está performando conforme o esperado. Com o localAI, você pode facilmente testar sua crew e avaliar seu desempenho utilizando as funcionalidades de teste integradas.

### Utilizando o Recurso de Teste

Adicionamos o comando de CLI `localagents test` para facilitar o teste da sua crew. Esse comando executará sua crew por um número especificado de iterações e fornecerá métricas de desempenho detalhadas. Os parâmetros são `n_iterations` e `model`, ambos opcionais e com valores padrão de 2 e `gpt-4o-mini`, respectivamente. Por enquanto, o único provedor disponível é a OpenAI.
...
```bash
localagents test --n_iterations 5 --model gpt-4o
```

ou usando as formas abreviadas:

```bash
localagents test -n 5 -m gpt-4o
```

Ao executar o comando `localagents test`, a crew será executada pelo número especificado de iterações, e as métricas de desempenho serão exibidas ao final da execução.
