...
description: Saiba como usar o sistema de impressão digital da LocalAI para identificar e rastrear componentes de forma única durante todo o seu ciclo de vida.
icon: fingerprint
mode: "wide"
---
...
As impressões digitais na LocalAI fornecem uma maneira de identificar e rastrear componentes de forma única durante todo o seu ciclo de vida. Cada `Agent`, `Crew` e `Task` recebe automaticamente uma impressão digital única quando criado, que não pode ser sobrescrita manualmente.
...
Uma impressão digital é uma instância da classe `Fingerprint` do módulo `localagents.security`. Cada impressão digital contém:
