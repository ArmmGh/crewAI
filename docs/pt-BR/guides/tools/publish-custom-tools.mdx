...
description: Como construir, empacotar e publicar suas próprias ferramentas compatíveis com LocalAI no PyPI para que qualquer usuário do LocalAI possa instalá-las e usá-las.
icon: box-open
mode: "wide"
---
...
O sistema de ferramentas do LocalAI foi projetado para ser extensível. Se você construiu uma ferramenta que pode beneficiar outros, pode empacotá-la como uma biblioteca Python independente, publicá-la no PyPI e disponibilizá-la para qualquer usuário do LocalAI — sem necessidade de PR para o repositório do LocalAI.
...
Toda ferramenta LocalAI deve satisfazer uma das duas interfaces:

### Opção 1: Subclassificar `BaseTool`

Subclassifique `localagents.tools.BaseTool` e implemente o método `_run`. Defina `name`, `description` e, opcionalmente, um `args_schema` para validação de entrada.
...
Para ferramentas mais simples, o decorador `@tool` transforma uma função em uma ferramenta LocalAI. A função **deve** ter uma docstring (usada como descrição da ferramenta) e anotações de tipo.
...
Defina um modelo Pydantic como seu `args_schema` para obter validação automática de entrada e mensagens de erro claras. Se não fornecer um, o LocalAI irá inferi-lo da assinatura do seu método `_run`.
...
```
localagents-geolocate/
├── pyproject.toml
├── LICENSE
├── README.md
└── src/
    └── localagents_geolocate/
        ├── __init__.py
        └── tools.py
```
...
```toml
[project]
name = "localagents-geolocate"
version = "0.1.0"
description = "Uma ferramenta LocalAI para geolocalizar endereços."
...
Declare `localagents` como dependência para que os usuários obtenham automaticamente uma versão compatível.
...
```python
from localagents_geolocate.tools import GeolocateTool
...
- **Nome do pacote**: Use o prefixo `localagents-` (ex.: `localagents-geolocate`). Isso torna sua ferramenta fácil de encontrar no PyPI.
- **Nome do módulo**: Use underscores (ex.: `localagents_geolocate`).
...
from localagents_geolocate import GeolocateTool
...
```bash
pip install localagents-geolocate
```

Ou com uv:

```bash
uv add localagents-geolocate
```
