---
title: تكامل Portkey
description: كيفية استخدام Portkey مع LocalAI
icon: key
mode: "wide"
---
...
يعزز Portkey إمكانيات LocalAI بميزات جاهزة للإنتاج، محولاً طواقم الوكلاء التجريبية إلى أنظمة متينة من خلال توفير:
...
<Step title="إعداد LocalAI مع Portkey">
التكامل بسيط - ما عليك سوى تحديث إعداد LLM في تكوين LocalAI الخاص بك:
...
يوفر Portkey مراقبة شاملة لوكلاء LocalAI، مما يساعدك على فهم ما يحدث بالضبط أثناء كل عملية تنفيذ.
...
أضف بيانات وصفية مخصصة لتكوين LLM في LocalAI لتمكين تصفية وتقسيم قوية:
...
من السهل تفعيل الاحتياط في إعداد LocalAI الخاص بك باستخدام تكوين Portkey:
...
### 3. إدارة الموجهات في LocalAI

يساعدك استوديو هندسة الموجهات من Portkey في إنشاء وإدارة وتحسين الموجهات المستخدمة في وكلاء LocalAI. بدلاً من ترميز الموجهات أو التعليمات بشكل ثابت، استخدم API عرض الموجهات من Portkey لجلب وتطبيق موجهاتك المُصدَرة ديناميكياً.
...
تجعل هذه البيئة المرئية من الأسهل صياغة موجهات فعالة لكل خطوة في سير عمل وكلاء LocalAI.
  </Tab>
...
تضمن حواجز الحماية أن وكلاء LocalAI يعملون بأمان ويستجيبون بشكل مناسب في جميع الحالات.

**لماذا تستخدم حواجز الحماية؟**

قد يواجه وكلاء LocalAI أوضاع فشل مختلفة:
...
تتبع المستخدمين الفرديين عبر وكلاء LocalAI باستخدام نظام البيانات الوصفية في Portkey.
...
طبّق التخزين المؤقت لجعل وكلاء LocalAI أكثر كفاءة وفعالية من حيث التكلفة:
...
يدعم LocalAI مزودي LLM متعددين، ويوسّع Portkey هذه القدرة من خلال توفير الوصول إلى أكثر من 200 نموذج LLM عبر واجهة موحدة. يمكنك التبديل بسهولة بين نماذج مختلفة دون تغيير منطق الوكيل الأساسي:
...
## إعداد حوكمة المؤسسة لـ LocalAI

**لماذا حوكمة المؤسسة؟**
إذا كنت تستخدم LocalAI داخل مؤسستك، فأنت بحاجة إلى مراعاة عدة جوانب حوكمة:
...
<Step title="الاتصال بـ LocalAI">
بعد إعداد مفتاح API لـ Portkey مع التكوين المرفق، اربطه بوكلاء LocalAI:
...
```
  ...
  <Accordion title="الخطوة 4: النشر والمراقبة">
    ...
    بعد توزيع مفاتيح API على أعضاء فريقك، يصبح إعداد LocalAI الجاهز للمؤسسة جاهزاً للعمل. يمكن لكل عضو في الفريق الآن استخدام مفاتيح API المخصصة له مع مستويات وصول وضوابط ميزانية مناسبة.
...
**تكامل LocalAI الخاص بك يتضمن الآن:**
...
<AccordionGroup>
  <Accordion title="كيف يعزز Portkey إمكانيات LocalAI؟">
    يضيف Portkey جاهزية الإنتاج لـ LocalAI من خلال مراقبة شاملة (تتبعات وسجلات ومقاييس) وميزات موثوقية (احتياط وإعادة محاولة وتخزين مؤقت) والوصول إلى أكثر من 200 نموذج LLM عبر واجهة موحدة. هذا يسهّل تصحيح الأخطاء وتحسين وتوسيع تطبيقات الوكلاء.
  </Accordion>

  <Accordion title="هل يمكنني استخدام Portkey مع تطبيقات LocalAI الحالية؟">
    نعم! يتكامل Portkey بسلاسة مع تطبيقات LocalAI الحالية. ما عليك سوى تحديث كود تكوين LLM بالنسخة المُمكّنة من Portkey. يبقى باقي كود الوكيل والطاقم دون تغيير.
  </Accordion>

  <Accordion title="هل يعمل Portkey مع جميع ميزات LocalAI؟">
    يدعم Portkey جميع ميزات LocalAI، بما في ذلك الوكلاء والأدوات وسير العمل مع تدخل بشري وجميع أنواع عمليات المهام (تسلسلي وهرمي وغيرها). يضيف المراقبة والموثوقية دون تقييد أي من وظائف الإطار.
  </Accordion>
...
<CardGroup cols="3">
  <Card title="وثائق LocalAI" icon="book" href="https://docs.localagents.com/">
    <p>وثائق LocalAI الرسمية</p>
  </Card>
