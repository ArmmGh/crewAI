...
يوفر Maxim AI مراقبة شاملة للوكلاء وتقييماً ومراقبة لتطبيقات LocalAI. مع تكامل Maxim بسطر واحد، يمكنك بسهولة تتبع وتحليل تفاعلات الوكلاء ومقاييس الأداء والمزيد.

## الميزات

### إدارة المطالبات

تمكّنك قدرات إدارة المطالبات في Maxim من إنشاء وتنظيم وتحسين المطالبات لوكلاء LocalAI. بدلاً من ترميز التعليمات مباشرة، استفد من SDK الخاص بـ Maxim لاسترداد وتطبيق مطالبات مُدارة بالإصدارات ديناميكياً.
...
يوفر Maxim AI مراقبة وتقييماً شاملاً لوكلاء LocalAI، مما يساعدك في فهم ما يحدث بالضبط أثناء كل تنفيذ.
...
### 4. إنشاء وتشغيل تطبيق LocalAI كالمعتاد
...
هذا كل شيء! سيتم الآن تسجيل جميع تفاعلات وكلاء LocalAI وستكون متاحة في لوحة معلومات Maxim.
...
- تأكد من استدعاء **`instrument_localagents()`** **_قبل_** تشغيل طاقمك
- عيّن `debug=True` في استدعاء `instrument_localagents()` لإظهار أي أخطاء داخلية:

  ```python
  instrument_localagents(logger, debug=True)
  ```
- أعدّ وكلاءك مع `verbose=True` لالتقاط سجلات مفصلة
- تحقق مرة أخرى من أن `instrument_localagents()` يُستدعى **قبل** إنشاء أو تنفيذ الوكلاء

## الموارد

<CardGroup cols="3">
  <Card title="وثائق LocalAI" icon="book" href="https://docs.localagents.com/">
    وثائق LocalAI الرسمية
  </Card>
