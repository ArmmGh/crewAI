...
description: تعرّف على كيفية استخدام واجهة سطر أوامر LocalAI للتفاعل مع LocalAI.
icon: terminal
mode: "wide"
---

<Warning>
  منذ الإصدار 0.140.0، بدأ LocalAI AMP عملية نقل مزود تسجيل الدخول.
  ...
  من تسجيل الدخول مع الإصدارات القديمة من مكتبة `localagents`.
</Warning>

## نظرة عامة

توفر واجهة سطر أوامر LocalAI مجموعة من الأوامر للتفاعل مع LocalAI، مما يتيح لك إنشاء وتدريب وتشغيل وإدارة الأطقم والتدفقات.

## التثبيت

لاستخدام واجهة سطر أوامر LocalAI، تأكد من تثبيت LocalAI:
...
الهيكل الأساسي لأمر LocalAI CLI هو:
...
عرض الإصدار المثبت من LocalAI.

```shell Terminal
localagents version [OPTIONS]
```

- `--tools`: (اختياري) عرض الإصدار المثبت من أدوات LocalAI
...
<Note>
  بدءًا من الإصدار 0.103.0، يمكن استخدام أمر `localagents run` لتشغيل
...
بدءًا من الإصدار `0.98.0`، عند تشغيل أمر `localagents chat`، تبدأ جلسة تفاعلية مع طاقمك. سيرشدك المساعد الذكي بطلب المدخلات اللازمة لتنفيذ الطاقم. بمجرد توفير جميع المدخلات، سينفذ الطاقم مهامه.
...
نشر الطاقم أو التدفق إلى [LocalAI AMP](https://app.localagents.com).

- **المصادقة**: تحتاج لتكون مصادقًا للنشر إلى LocalAI AMP.
...
المصادقة مع LocalAI AMP باستخدام تدفق رمز الجهاز الآمن.
...
إدارة إعدادات تهيئة CLI لـ LocalAI.
...
- أضف `LOCALAGENTS_TRACING_ENABLED=true` إلى ملف `.env`، أو
- شغّل `localagents traces enable`

**لتعطيل التتبع**، استخدم أيًا من هذه الطرق:
- عيّن `tracing=False` في كود الطاقم/التدفق، أو
- أزل أو عيّن `false` لمتغير `LOCALAGENTS_TRACING_ENABLED`، أو
- شغّل `localagents traces disable`
</Note>

<Tip>
  يتعامل LocalAI CLI مع المصادقة لمستودع الأدوات تلقائيًا عند
  إضافة حزم إلى مشروعك. فقط أضف `localagents` قبل أي أمر `uv`
  لاستخدامه. مثلًا `localagents uv add requests`.
</Tip>

<Note>
  تُخزن إعدادات التهيئة في `~/.config/localagents/settings.json`. بعض
