...
description: 'دليل شامل لإعداد واستخدام نماذج اللغة الكبيرة (LLMs) في مشاريع LocalAI'
icon: 'microchip-ai'
mode: "wide"
---
...
يتكامل LocalAI مع مزودي LLM متعددين من خلال حزم SDK الأصلية للمزودين، مما يمنحك المرونة لاختيار النموذج المناسب لحالة الاستخدام الخاصة بك. سيساعدك هذا الدليل على فهم كيفية إعداد واستخدام مزودي LLM المختلفين في مشاريع LocalAI.


## ما هي نماذج اللغة الكبيرة؟

نماذج اللغة الكبيرة (LLMs) هي الذكاء الأساسي وراء وكلاء LocalAI. تمكّن الوكلاء من فهم السياق واتخاذ القرارات وتوليد استجابات شبيهة بالبشر. إليك ما تحتاج معرفته:

<CardGroup cols={2}>
  <Card title="أساسيات LLM" icon="brain">
    نماذج اللغة الكبيرة هي أنظمة ذكاء اصطناعي مدربة على كميات هائلة من البيانات النصية. تدعم ذكاء وكلاء LocalAI، مما يمكّنهم من فهم وتوليد نصوص شبيهة بالبشر.
  </Card>
...
هناك أماكن مختلفة في كود LocalAI حيث يمكنك تحديد النموذج المُستخدم. بمجرد تحديد النموذج، ستحتاج إلى توفير الإعداد (مثل مفتاح API) لكل مزود نموذج تستخدمه. انظر قسم [أمثلة إعداد المزودين](#أمثلة-إعداد-المزودين) لمزودك.

<Tabs>
  <Tab title="1. متغيرات البيئة">
    أبسط طريقة للبدء. عيّن النموذج في بيئتك مباشرة، من خلال ملف `.env` أو في كود تطبيقك. إذا استخدمت `localagents create` لبدء مشروعك، سيكون مُعيّنًا بالفعل.
...
<Info>
  يوفر LocalAI تكاملات SDK أصلية لـ OpenAI و Anthropic و Google (Gemini API) و Azure و AWS Bedrock -- لا حاجة لتثبيت إضافي بخلاف الملحقات الخاصة بالمزود (مثل `uv add "localagents[openai]"`).
...
يدعم LocalAI العديد من مزودي LLM، كل منهم يقدم ميزات فريدة وطرق مصادقة وقدرات نماذج.
في هذا القسم، ستجد أمثلة مفصلة تساعدك في اختيار وإعداد وتحسين LLM الأنسب لاحتياجات مشروعك.

<AccordionGroup>
  <Accordion title="OpenAI">
    يوفر LocalAI تكاملًا أصليًا مع OpenAI من خلال OpenAI Python SDK.
  ...
  <Accordion title="Meta-Llama">
    ...
    الـ API متاحة عبر [Meta Llama API](https://llama.developer.meta.com?utm_source=partner-localagents&utm_medium=website).
    ...
    مثال الاستخدام في مشروع LocalAI:
    ```python Code
    from localagents import LLM
  ...
  <Accordion title="Anthropic">
    يوفر LocalAI تكاملًا أصليًا مع Anthropic من خلال Anthropic Python SDK.
    ...
    **التفكير الموسّع (Claude Sonnet 4 وما بعده):**
    ...
    يدعم LocalAI ميزة التفكير الموسّع من Anthropic، التي تتيح لـ Claude التفكير في المشكلات بطريقة أكثر شبهًا بالبشر قبل الاستجابة. مفيد بشكل خاص لمهام الاستدلال والتحليل وحل المشكلات المعقدة.
  ...
  <Accordion title="Google (Gemini API)">
    يوفر LocalAI تكاملًا أصليًا مع Google Gemini من خلال Google Gen AI Python SDK.
  ...
  <Accordion title="Azure">
    يوفر LocalAI تكاملًا أصليًا مع Azure AI Inference و Azure OpenAI من خلال Azure AI Inference Python SDK.
  ...
  <Accordion title="AWS Bedrock">
    يوفر LocalAI تكاملًا أصليًا مع AWS Bedrock من خلال boto3 SDK باستخدام Converse API.
  ...
  <Accordion title="Local NVIDIA NIM Deployed using WSL2">
    ...
    3. أعدّ نماذج localagents المحلية:
...
يدعم LocalAI بث الاستجابات من LLMs، مما يتيح لتطبيقك تلقي ومعالجة المخرجات في الوقت الفعلي فور توليدها.
...
<Tabs>
  ...
  <Tab title="معالجة الأحداث">
    يُصدر LocalAI أحداثًا لكل جزء يتم تلقيه أثناء البث:
  ...
  <Tab title="تتبع Agent والمهمة">
    جميع أحداث LLM في LocalAI تتضمن معلومات Agent والمهمة، مما يتيح لك تتبع وتصفية تفاعلات LLM بواسطة وكلاء أو مهام محددة:
...
يدعم LocalAI استدعاءات LLM غير المتزامنة لأداء وتزامن محسّنين في سير عمل الذكاء الاصطناعي. تتيح لك الاستدعاءات غير المتزامنة تشغيل طلبات LLM متعددة بشكل متزامن دون حجب، مما يجعلها مثالية لتطبيقات الإنتاجية العالية وعمليات الوكلاء المتوازية.
...
يدعم LocalAI الاستجابات المهيكلة من استدعاءات LLM من خلال السماح لك بتحديد `response_format` باستخدام نموذج Pydantic. يمكّن هذا الإطار من تحليل المخرجات والتحقق منها تلقائيًا، مما يسهّل دمج الاستجابة في تطبيقك دون معالجة لاحقة يدوية.
...
<AccordionGroup>
  <Accordion title="إدارة نافذة السياق">
    يتضمن LocalAI ميزات إدارة سياق ذكية:
  ...
  <Accordion title="إسقاط المعاملات الإضافية">
    يستخدم LocalAI داخليًا حزم SDK أصلية لاستدعاءات LLM، مما يتيح لك إسقاط معاملات إضافية غير مطلوبة لحالة الاستخدام الخاصة بك. يمكن أن يساعد هذا في تبسيط كودك وتقليل تعقيد إعداد LLM.
  ...
  <Accordion title="معترضات النقل">
    يوفر LocalAI معترضات رسائل لعدة مزودين، مما يتيح لك الربط بدورات الطلب/الاستجابة على مستوى طبقة النقل.
