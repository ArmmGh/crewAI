---
title: المعرفة
description: ما هي المعرفة في LocalAI وكيفية استخدامها.
icon: book
mode: "wide"
---
...
المعرفة في LocalAI هي نظام قوي يتيح لوكلاء الذكاء الاصطناعي الوصول إلى مصادر المعلومات الخارجية واستخدامها أثناء مهامهم.
...
يوفر LocalAI تجريدًا لعميل RAG محايد بالنسبة للمزود لمتاجر المتجهات. المزود الافتراضي هو ChromaDB، ويتم دعم Qdrant أيضًا. يمكنك التبديل بين المزودين باستخدام أدوات الإعداد.
...
يدعم LocalAI أنواعًا متعددة من مصادر المعرفة جاهزة للاستخدام:
...
<Info>
**فهم مستويات المعرفة**: يدعم LocalAI المعرفة على مستوى كل من Agent و Crew. يوضح هذا القسم بالضبط كيف يعمل كل منهما، ومتى يتم تهيئتهما، ويعالج المفاهيم الخاطئة الشائعة حول التبعيات.
</Info>
...
<Info>
**فهم تخزين المعرفة**: يخزّن LocalAI مصادر المعرفة تلقائيًا في مجلدات خاصة بالمنصة باستخدام ChromaDB للتخزين المتجهي. فهم هذه المواقع والإعدادات الافتراضية يساعد في النشر في بيئة الإنتاج وتصحيح الأخطاء وإدارة التخزين.
</Info>

### أين يخزّن LocalAI ملفات المعرفة

بشكل افتراضي، يستخدم LocalAI نفس نظام التخزين مثل الذاكرة، حيث يخزّن المعرفة في مجلدات خاصة بالمنصة:
...
لرؤية المكان الذي يخزّن فيه LocalAI ملفات المعرفة بالضبط:
...
<Info>
**مزود التضمين الافتراضي**: يستخدم LocalAI افتراضيًا تضمينات OpenAI (`text-embedding-3-small`) لتخزين المعرفة، حتى عند استخدام مزودي LLM مختلفين. يمكنك تخصيص هذا بسهولة ليتوافق مع إعدادك.
</Info>
...
ينفذ LocalAI آلية إعادة صياغة استعلام ذكية لتحسين استرجاع المعرفة. عندما يحتاج وكيل إلى البحث في مصادر المعرفة، يتم تحويل موجّه المهمة الخام تلقائيًا إلى استعلام بحث أكثر فعالية.
...
يُصدر LocalAI أحداثًا أثناء عملية استرجاع المعرفة يمكنك الاستماع إليها باستخدام نظام الأحداث. تتيح لك هذه الأحداث مراقبة وتصحيح أخطاء وتحليل كيفية استرجاع المعرفة واستخدامها بواسطة وكلائك.
...
يتيح لك LocalAI إنشاء مصادر معرفة مخصصة لأي نوع من البيانات عن طريق توسيع فئة `BaseKnowledgeSource`. لنقم بإنشاء مثال عملي يجلب ويعالج مقالات أخبار الفضاء.
...
إذا كنت بحاجة إلى مسح المعرفة المخزّنة في LocalAI، يمكنك استخدام أمر `localagents reset-memories` مع خيار `--knowledge`.
...
<AccordionGroup>
  ...
  <Accordion title="المعرفة لمرة واحدة">
    - مع هيكل الملفات النموذجي الذي يوفره LocalAI، يتم تضمين مصادر المعرفة في كل مرة يتم فيها تشغيل kickoff.
    ...
    - رابط للمشكلة للحصول على فكرة كاملة [Github Issue](https://github.com/localAIInc/localAI/issues/2755)
  </Accordion>
  ...
  <Accordion title="أفضل ممارسات الإنتاج">
    - عيّن `LOCALAGENTS_STORAGE_DIR` إلى موقع معروف في الإنتاج
