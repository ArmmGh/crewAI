---
title: الذاكرة
description: الاستفادة من نظام الذاكرة الموحد في LocalAI لتعزيز قدرات الوكلاء.
icon: database
mode: "wide"
---
...
يوفر LocalAI **نظام ذاكرة موحد** -- فئة `Memory` واحدة تستبدل أنواع الذاكرة المنفصلة (قصيرة المدى، طويلة المدى، ذاكرة الكيانات، والخارجية) بواجهة برمجة تطبيقات ذكية واحدة. تستخدم الذاكرة LLM لتحليل المحتوى عند الحفظ (استنتاج النطاق والفئات والأهمية) وتدعم الاسترجاع متعدد العمق مع تسجيل مركب يمزج بين التشابه الدلالي والحداثة والأهمية.
...
```python
from localagents import Crew, Agent, Task, Process, Memory
...
```python
from localagents import Agent, Memory
...
هذا يمنع تراكم النسخ المكررة. على سبيل المثال، إذا حفظت "LocalAI ensures reliable operation" ثلاث مرات، يتعرف التوحيد على النسخ المكررة ويحتفظ بسجل واحد فقط.
...
memory.remember_many([
    "LocalAI supports complex workflows.",
    "Python is a great language.",
    "LocalAI supports complex workflows.",  # dropped by intra-batch dedup
])
```
...
```python
from localagents import Memory, LLM
...
- **الافتراضي**: LanceDB، مخزّن تحت `./.localagents/memory` (أو `$LOCALAGENTS_STORAGE_DIR/memory` إذا تم تعيين متغير البيئة، أو المسار الذي تمرره كـ `storage="path/to/dir"`).
- **واجهة مخصصة**: نفّذ بروتوكول `StorageBackend` (انظر `localagents.memory.storage.backend`) ومرّر مثيلًا إلى `Memory(storage=your_backend)`.
...
```python
from localagents.events import BaseEventListener, MemoryQueryCompletedEvent
...
**الذاكرة لا تستمر؟**
- تأكد من أن مسار التخزين قابل للكتابة (الافتراضي `./.localagents/memory`). مرّر `storage="./your_path"` لاستخدام مجلد مختلف، أو عيّن متغير البيئة `LOCALAGENTS_STORAGE_DIR`.
...
```bash
localagents memory                              # Opens the TUI browser
localagents memory --storage-path ./my_memory   # Point to a specific directory
```
