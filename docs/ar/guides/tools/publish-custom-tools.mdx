...
description: كيفية بناء وتعبئة ونشر أدواتك الخاصة المتوافقة مع LocalAI على PyPI ليتمكن أي مستخدم LocalAI من تثبيتها واستخدامها.
icon: box-open
mode: "wide"
---
...
نظام الأدوات في LocalAI مصمم للتوسيع. إذا بنيت أداة يمكن أن تفيد الآخرين، يمكنك تعبئتها كمكتبة Python مستقلة ونشرها على PyPI وإتاحتها لأي مستخدم LocalAI — دون الحاجة لطلب سحب إلى مستودع LocalAI.
...
كل أداة LocalAI يجب أن تستوفي إحدى الواجهتين:

### الخيار 1: وراثة `BaseTool`

ورث من `localagents.tools.BaseTool` وطبّق طريقة `_run`. عرّف `name` و`description` واختياريًا `args_schema` للتحقق من المدخلات.
...
للأدوات الأبسط، يحوّل مزخرف `@tool` دالة إلى أداة LocalAI. يجب أن تحتوي الدالة على سلسلة توثيق (تُستخدم كوصف الأداة) وتعليقات أنواع.
