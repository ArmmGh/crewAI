---
title: التثبيت
description: ابدأ مع LocalAI - التثبيت والتهيئة وبناء أول فريق AI
icon: wrench
mode: "wide"
---
...
يتطلب LocalAI إصدار `Python >=3.10 and <3.14`. إليك كيفية التحقق من إصدارك:
...
يتطلب LocalAI 0.175.0 إصدار `openai >= 1.13.3`. إذا كنت تدير التبعيات بنفسك، تأكد من أن بيئتك تستوفي هذا الشرط لتجنب مشاكل الاستيراد/التشغيل.

</Note>

يستخدم LocalAI أداة `uv` لإدارة التبعيات والحزم. وهي تبسّط إعداد المشروع وتنفيذه وتوفر تجربة سلسة.
...
<Steps>
    ...
    <Step title="تثبيت LocalAI">
      - شغّل الأمر التالي لتثبيت واجهة سطر أوامر `localagents`:
        ```shell
      ...
      - للتحقق من تثبيت `localagents`، شغّل:
        ```shell
        uv tool list
        ```
      - يجب أن ترى شيئًا مثل:
        ...
        - localagents
        ```
      - إذا كنت بحاجة لتحديث `localagents`، شغّل:
        ```shell
...
<Note type="info">
للفرق والمؤسسات، يوفر LocalAI خيارات نشر مؤسسية تزيل تعقيد الإعداد:

### LocalAI AMP (SaaS)

- لا يتطلب أي تثبيت - فقط سجّل مجانًا على [app.localagents.com](https://app.localagents.com)
- تحديثات وصيانة تلقائية
- بنية تحتية مُدارة وقابلة للتوسع
- بناء Crews بدون كتابة كود

### LocalAI Factory (استضافة ذاتية)

- نشر بالحاويات على بنيتك التحتية
- يدعم أي مزود سحابي بما في ذلك النشر المحلي
- تكامل مع أنظمة الأمان الحالية

<Card title="استكشف خيارات المؤسسات" icon="building" href="https://localagents.com/enterprise">
  تعرّف على عروض LocalAI للمؤسسات وجدول عرضًا توضيحيًا
</Card>
</Note>
...
<CardGroup cols={2}>
  <Card title="ابنِ أول Agent لك" icon="code" href="/ar/quickstart">
    اتبع دليل البداية السريعة لإنشاء أول Agent في LocalAI والحصول على تجربة عملية.
  </Card>
  <Card
  ...
  >
    تواصل مع مطورين آخرين واحصل على المساعدة وشارك تجاربك مع LocalAI.
  </Card>
</CardGroup>
