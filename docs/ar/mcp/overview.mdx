---
title: "خوادم MCP كأدوات في LocalAI"
description: "تعلم كيفية دمج خوادم MCP كأدوات في وكلاء LocalAI باستخدام مكتبة `localagents-tools`."
icon: plug
mode: "wide"
---
...
مثالية لخوادم HTTPS البعيدة وتكاملات MCP المتصلة من كتالوج LocalAI:
...
لحالات الاستخدام المتقدمة التي تتطلب إدارة اتصال يدوية، توفر مكتبة `localagents-tools` فئة `MCPServerAdapter`.
...
شاهد هذا الفيديو التعليمي للحصول على دليل شامل حول تكامل MCP مع LocalAI:
...
يتطلب تكامل LocalAI MCP مكتبة `mcp`:
...
اربط خوادم MCP من كتالوج LocalAI أو أحضر خوادمك الخاصة. بمجرد الاتصال في حسابك، أشر إليها بالمعرف المختصر:
...
- **تكامل شفاف**: يعمل بسلاسة مع ميزات LocalAI الموجودة
...
للسيناريوهات المعقدة التي تتطلب إدارة اتصال يدوية، استخدم فئة `MCPServerAdapter` من `localagents-tools`. استخدام مدير سياق Python (تعليمة `with`) هو النهج الموصى به لأنه يتعامل تلقائياً مع بدء وإيقاف الاتصال بخادم MCP.
...
تحقق من هذا المستودع للحصول على عروض وأمثلة كاملة لتكامل MCP مع LocalAI!
...
- **الأوليات المدعومة**: حالياً، يدعم `MCPServerAdapter` بشكل أساسي تكييف `أدوات` MCP.
  لا يتم دمج أوليات MCP الأخرى مثل `prompts` أو `resources` مباشرة كمكونات LocalAI من خلال هذا المحول في هذا الوقت.
