...
صُممت `RagTool` للإجابة على الأسئلة من خلال الاستفادة من قوة التوليد المعزز بالاسترجاع (RAG) عبر نظام RAG الأصلي في LocalAI.
...
- **adapter**: اختياري. محول مخصص لقاعدة المعرفة. إذا لم يُقدَّم، سيُستخدم LocalAIRagAdapter.
- **config**: اختياري. إعداد نظام RAG الأساسي في LocalAI. يقبل TypedDict من نوع `RagToolConfig` مع مفاتيح اختيارية `embedding_model` (ProviderSpec) و`vectordb` (VectorDbConfig). جميع قيم الإعداد المقدمة برمجياً لها الأولوية على متغيرات البيئة.
...
إليك كيفية دمج `RagTool` مع وكيل LocalAI:
...
يقبل معامل `embedding_model` قاموس `localagents.rag.embeddings.types.ProviderSpec` بالبنية التالية:
...
<AccordionGroup>
  ...
  <Accordion title="مخصص">
    ...
    **ملاحظة:** يجب أن تنفّذ دوال التضمين المخصصة بروتوكول `EmbeddingFunction` المحدد في `localagents.rag.core.base_embeddings_callable`. يجب أن تقبل طريقة `__call__` بيانات الإدخال وتعيد التضمينات كقائمة من مصفوفات numpy (أو تنسيق متوافق سيتم تطبيعه). يتم تطبيع التضمينات المُعادة والتحقق منها تلقائياً.
  </Accordion>
</AccordionGroup>
