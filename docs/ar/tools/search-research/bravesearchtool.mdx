...
تقدم LocalAI عائلة من أدوات بحث Brave، كل منها يستهدف نقطة نهاية محددة في [واجهة برمجة تطبيقات Brave Search](https://brave.com/search/api/). بدلاً من أداة واحدة شاملة، يمكنك اختيار الأداة التي تتطابق بدقة مع نوع النتائج التي يحتاجها وكيلك:
...
1. **تثبيت الحزمة** - تأكد من تثبيت `localagents[tools]` في بيئة Python الخاصة بك.
...
```python Code
from localagents_tools import BraveWebSearchTool

tool = BraveWebSearchTool()
results = tool.run(q="LocalAI agent framework")
print(results)
```
...
```python Code
from localagents_tools import BraveNewsSearchTool
...
```python Code
from localagents_tools import BraveImageSearchTool
...
```python Code
from localagents_tools import BraveVideoSearchTool
...
إليك كيفية تزويد وكيل LocalAI بأدوات بحث Brave متعددة:

```python Code
from localagents import Agent
from localagents.project import agent
from localagents_tools import BraveWebSearchTool, BraveNewsSearchTool
...
توفر مجموعة أدوات بحث Brave لوكلاء LocalAI وصولاً مرناً ومحدداً بنقطة النهاية إلى واجهة برمجة تطبيقات Brave Search. سواء كنت تحتاج إلى صفحات ويب أو أخبار عاجلة أو صور أو مقاطع فيديو، هناك أداة مخصصة مع معاملات مُتحقق منها ومرونة مدمجة. اختر الأداة المناسبة لحالة استخدامك، وارجع إلى [وثائق واجهة Brave Search API](https://brave.com/search/api/) للحصول على التفاصيل الكاملة حول المعاملات المتاحة وصيغ الاستجابة.
