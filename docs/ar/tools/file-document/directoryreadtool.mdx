...
لاستخدام DirectoryReadTool في مشروعك، قم بتثبيت حزمة `localagents_tools`. إذا لم تكن هذه الحزمة جزءاً من بيئتك بعد، يمكنك تثبيتها باستخدام pip بالأمر التالي:

```shell
pip install 'localagents[tools]'
```

يُثبّت هذا الأمر أحدث إصدار من حزمة `localagents_tools`، مما يمنح الوصول إلى DirectoryReadTool بالإضافة إلى أدوات مساعدة أخرى.
