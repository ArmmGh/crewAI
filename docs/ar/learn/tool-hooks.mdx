...
description: تعلم كيفية استخدام خطافات استدعاء الأدوات لاعتراض وتعديل والتحكم في تنفيذ الأدوات في LocalAI
mode: "wide"
---
...
```python
from localagents.hooks import register_before_tool_call_hook, register_after_tool_call_hook
...
```python
from localagents.hooks import before_tool_call, after_tool_call
...
```python
from localagents.hooks import ToolCallHookContext, BeforeToolCallHookType, AfterToolCallHookType
...
توفر خطافات استدعاء الأدوات إمكانيات قوية للتحكم في تنفيذ الأدوات ومراقبتها في LocalAI. استخدمها لتنفيذ حواجز السلامة وبوابات الموافقة والتحقق من المدخلات وتنقية النتائج والتسجيل والتحليلات. مع معالجة الأخطاء المناسبة وأمان الأنواع، تُمكّن الخطافات أنظمة وكلاء آمنة وجاهزة للإنتاج مع مراقبة شاملة.
