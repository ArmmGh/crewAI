...
يوفر LocalAI القدرة على إعادة التشغيل من مهمة محددة من آخر تشغيل للطاقم. هذه الميزة مفيدة بشكل خاص عندما تكون قد أنهيت تشغيلاً وقد ترغب في إعادة محاولة مهام معينة أو لا تحتاج إلى إعادة جلب البيانات ووكلاؤك لديهم بالفعل السياق المحفوظ من تنفيذ التشغيل، لذا تحتاج فقط إلى إعادة تشغيل المهام التي تريدها.
...
<Steps>
   ...
   <Step title="انتقل إلى المجلد الذي يقع فيه مشروع LocalAI الخاص بك."></Step>
...
<Note>
    تأكد من أن `localagents` مثبت ومُعد بشكل صحيح في بيئة التطوير الخاصة بك.
</Note>
   ...
   <Step title="نفّذ أمر إعادة التشغيل ضمن كتلة try-except للتعامل مع الأخطاء المحتملة.">
      ...
      <CodeGroup>
          ```python Code
            ...
            """
            task_id = '<task_id>'
            inputs = {"topic": "LocalAI Training"}  # This is optional; you can pass in the inputs you want to replay; otherwise, it uses the previous kickoff's inputs.
            try:
...
مع التحسينات المذكورة أعلاه والوظائف المفصلة، أصبحت إعادة تشغيل مهام محددة في LocalAI أكثر كفاءة ومتانة.
