...
description: تعلم كيفية استخدام خطافات استدعاء LLM لاعتراض وتعديل والتحكم في تفاعلات نماذج اللغة في LocalAI
mode: "wide"
---
...
```python
from localagents.hooks import register_before_llm_call_hook, register_after_llm_call_hook
...
```python
from localagents.hooks import before_llm_call, after_llm_call
...
```python
from localagents.hooks import LLMCallHookContext, BeforeLLMCallHookType, AfterLLMCallHookType
...
توفر خطافات استدعاء LLM إمكانيات قوية للتحكم في تفاعلات نماذج اللغة ومراقبتها في LocalAI. استخدمها لتنفيذ حواجز الأمان وبوابات الموافقة والتسجيل وتتبع التكاليف وتنقية الاستجابات. مع معالجة الأخطاء المناسبة وأمان الأنواع، تُمكّن الخطافات أنظمة وكلاء قوية وجاهزة للإنتاج.
