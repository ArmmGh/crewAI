...
1. انتقل إلى <Link href="https://app.localagents.com/localagents_plus/connectors">Integrations</Link>
2. انقر على <b>Connect</b> في الخدمة المطلوبة
3. أكمل تدفق OAuth وامنح النطاقات
4. انسخ رمز Enterprise من <Link href="https://app.localagents.com/localagents_plus/settings/integrations">Integration Settings</Link>
...
لاستخدام التكاملات محلياً، تحتاج إلى تثبيت أحدث حزمة `localagents-tools`.
...
<Note>
  ...
  `LOCALAGENTS_PLATFORM_INTEGRATION_TOKEN` برمز Enterprise الخاص بك.
</Note>
...
أنشئ أدوات مخصصة محلياً، وانشرها في مستودع أدوات LocalAI AMP واستخدمها في وكلائك.

{" "}
<Tip>
  قبل تشغيل الأوامر أدناه، تأكد من تسجيل الدخول إلى حساب LocalAI AMP
  بتشغيل هذا الأمر: ```bash localagents login ```
</Tip>
...
<Steps>
  <Step title="الإنشاء">
    أنشئ أداة جديدة محلياً. ```bash localagents tool create your-tool ```
  </Step>
  <Step title="النشر">
    انشر الأداة في مستودع أدوات LocalAI AMP. ```bash localagents tool
    publish ```
  </Step>
  <Step title="التثبيت">
    ثبّت الأداة من مستودع أدوات LocalAI AMP. ```bash localagents tool
    install your-tool ```
  </Step>
</Steps>
