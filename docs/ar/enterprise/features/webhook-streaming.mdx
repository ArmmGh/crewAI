...
يتيح لك بث أحداث Enterprise تلقي تحديثات webhook في الوقت الفعلي حول طواقمك وتدفقاتك المنشورة على LocalAI AMP، مثل استدعاءات النماذج واستخدام الأدوات وخطوات التدفق.
...
يختلف هيكل كائن `data` حسب نوع الحدث. راجع [قائمة الأحداث](https://github.com/localAIInc/localAI/tree/main/lib/localagents/src/localagents/events/types) على GitHub.

نظراً لأن الطلبات تُرسل عبر HTTP، لا يمكن ضمان ترتيب الأحداث. إذا كنت تحتاج الترتيب، استخدم حقل `timestamp`.

## الأحداث المدعومة

يدعم LocalAI كلاً من أحداث النظام والأحداث المخصصة في بث أحداث Enterprise. تُرسل هذه الأحداث إلى نقطة نهاية webhook المُهيأة أثناء تنفيذ الطاقم والتدفق.
...
<CardGroup>
  <Card
    title="GitHub"
    icon="github"
    href="https://github.com/localAIInc/localAI/tree/main/src/localagents/utilities/events"
  >
