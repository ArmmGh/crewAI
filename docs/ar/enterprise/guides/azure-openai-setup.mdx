...
<Steps>
    ...
    <Step title="تهيئة اتصال LocalAI AMP">
        4. في علامة تبويب أخرى، افتح `LocalAI AMP > LLM Connections`. سمِّ اتصال LLM، واختر Azure كمزود، واختر نفس النموذج الذي اخترته في Azure.
    ...
    <Step title="ضبط التهيئة الافتراضية">
        7. في `LocalAI AMP > Settings > Defaults > Crew Studio LLM Settings`، عيّن اتصال LLM والنموذج الجديدين كافتراضيين.
    </Step>
    ...
    <Step title="تهيئة الوصول إلى الشبكة">
        8. تأكد من إعدادات الوصول إلى الشبكة:
            ...
            - تأكد من تفعيل `Allow access from all networks`. إذا كان هذا الإعداد مقيداً، فقد يُحظر وصول LocalAI إلى نقطة نهاية Azure OpenAI.
    </Step>
...
- تأكد من تهيئة الوصول إلى الشبكة للسماح باتصالات LocalAI
- تأكد من أن نموذج النشر يتطابق مع ما هيأته في LocalAI
