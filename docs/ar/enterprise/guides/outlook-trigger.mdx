...
اختبر تكامل مشغل Outlook محلياً باستخدام LocalAI CLI:
...
سينفذ أمر `localagents triggers run` طاقمك بحمولة Outlook كاملة، مما يتيح لك اختبار منطق التحليل قبل النشر.

<Warning>
  استخدم `localagents triggers run microsoft_outlook/email_received` (وليس `localagents run`)
...
- اختبر محلياً بـ `localagents triggers run microsoft_outlook/email_received` لرؤية هيكل الحمولة بالضبط
