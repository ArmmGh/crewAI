...
اختبر تكامل مشغل Gmail محلياً باستخدام LocalAI CLI:
...
سينفذ أمر `localagents triggers run` طاقمك بحمولة Gmail كاملة، مما يتيح لك اختبار منطق التحليل قبل النشر.

<Warning>
  استخدم `localagents triggers run gmail/new_email_received` (وليس `localagents run`) لمحاكاة
...
- اختبر محلياً بـ `localagents triggers run gmail/new_email_received` لرؤية هيكل الحمولة بالضبط
- تحقق من سجلات التنفيذ وتأكد من تمرير الحمولة كـ `localagents_trigger_payload`
