...
اختبر تكامل مشغل Google Drive محلياً باستخدام LocalAI CLI:
...
سينفذ أمر `localagents triggers run` طاقمك بحمولة Drive كاملة، مما يتيح لك اختبار منطق التحليل قبل النشر.

<Warning>
  استخدم `localagents triggers run google_drive/file_changed` (وليس `localagents run`) لمحاكاة
...
- اختبر محلياً بـ `localagents triggers run google_drive/file_changed` لرؤية هيكل الحمولة بالضبط
