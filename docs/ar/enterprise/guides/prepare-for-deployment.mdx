---
title: "التحضير للنشر"
description: "تأكد من جاهزية طاقمك أو تدفقك للنشر على LocalAI AMP"
icon: "clipboard-check"
mode: "wide"
---

<Note>
  قبل النشر على LocalAI AMP، من الضروري التحقق من صحة بنية مشروعك.
...
في LocalAI AMP، **الأتمتات** هو المصطلح الشامل لمشاريع الذكاء الاصطناعي الوكيل القابلة للنشر. يمكن أن تكون الأتمتة إما:
...
| **أمر CLI للإنشاء** | `localagents create crew name` | `localagents create flow name` |
...
عند تشغيل `localagents create crew my_crew`، تحصل على هذه البنية:
...
عند تشغيل `localagents create flow my_flow`، تحصل على هذه البنية:
...
يجب أن يتضمن `pyproject.toml` قسم `[tool.localagents]` الصحيح:
...
يستخدم LocalAI `uv` لإدارة الاعتماديات. يضمن ملف `uv.lock` بناءً قابلاً للتكرار وهو **مطلوب** للنشر.
...
<Card title="النشر على AMP" icon="rocket" href="/ar/enterprise/guides/deploy-to-amp">
  اتبع دليل النشر لنشر طاقمك أو تدفقك على LocalAI AMP باستخدام
