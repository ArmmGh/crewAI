...
description: "تثبيت حزم Python الخاصة من سجلات PyPI المصادق عليها في LocalAI AMP"
icon: "lock"
mode: "wide"
---

<Note>
  يغطي هذا الدليل كيفية تهيئة مشروع LocalAI لتثبيت حزم Python
  من سجلات PyPI الخاصة (Azure DevOps Artifacts، GitHub Packages، GitLab، AWS CodeArtifact، إلخ.)
  عند النشر على LocalAI AMP.
</Note>
...
يستخدم LocalAI AMP [UV](https://docs.astral.sh/uv/) لحل وتثبيت الاعتماديات.
...
<Warning>
  هذه المتغيرات **يجب** إضافتها عبر إعدادات **Environment Variables** في LocalAI AMP —
...
يجب تهيئة بيانات اعتماد السجل الخاص كمتغيرات بيئة في LocalAI AMP.
لديك خياران:
...
<Tabs>
  ...
  <Tab title="النشر عبر CLI">
    أضف المتغيرات إلى ملف `.env` المحلي قبل تشغيل `localagents deploy create`.
...
عندما يبني LocalAI AMP أتمتتك، يعمل تدفق الحل هكذا:
