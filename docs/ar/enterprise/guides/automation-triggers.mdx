...
description: "فهم كيفية عمل مشغلات LocalAI AMP وكيفية إدارتها وأين تجد أدلة التكامل الخاصة بكل خدمة"
icon: "face-smile"
mode: "wide"
---

تربط مشغلات LocalAI AMP أتمتاتك بالأحداث الفورية عبر الأدوات التي تستخدمها فرقك بالفعل. بدلاً من الاستعلام المتكرر عن الأنظمة أو الاعتماد على التشغيل اليدوي، تستمع المشغلات للتغييرات — رسائل بريد إلكتروني جديدة، تحديثات التقويم، تغييرات حالة CRM — وتطلق فوراً الطاقم أو التدفق الذي تحدده.
...
<Card title="مشغل Salesforce" icon="salesforce">
  <a href="/ar/enterprise/guides/salesforce-trigger">
    اربط عمليات Salesforce بـ LocalAI لأتمتة CRM.
  </a>
</Card>
...
<Card title="مشغل Slack" icon="slack">
  ...
  <Card title="مشغل Zapier" icon="bolt">
    <a href="/ar/enterprise/guides/zapier-trigger">اربط LocalAI بآلاف التطبيقات المدعومة من Zapier.</a>
  </Card>
</CardGroup>
...
1. انتقل إلى عملية النشر في لوحة تحكم LocalAI
...
- تحديد ما إذا كنت ستمرر سياق المشغل تلقائياً باستخدام `allow_localagents_trigger_context`
- إعداد المراقبة — سجلات webhook وسجل تنفيذ LocalAI والتنبيهات الخارجية الاختيارية

### اختبار المشغلات محلياً باستخدام CLI

يوفر LocalAI CLI أوامر قوية لمساعدتك في تطوير واختبار الأتمتات المدفوعة بالمشغلات دون النشر في الإنتاج.
...
<Warning>
  **ملاحظات تطوير مهمة:**
  - استخدم `localagents triggers run <trigger>` لمحاكاة تنفيذ المشغل أثناء التطوير
  - استخدام `localagents run` لن يحاكي استدعاءات المشغل ولن يمرر حمولة المشغل
...
سيتلقى الطاقم تلقائياً حمولة المشغل ويمكنه الوصول إليها عبر آليات سياق LocalAI القياسية.

<Note>
  يمكن أن تتضمن مدخلات الطاقم والتدفق `localagents_trigger_payload`. يحقن LocalAI
  هذه الحمولة تلقائياً: - المهام: تُلحق بوصف المهمة الأولى افتراضياً ("Trigger Payload: {localagents_trigger_payload}") - التحكم
  عبر `allow_localagents_trigger_context`: عيّن `True` للحقن دائماً، `False` لعدم
  الحقن أبداً - التدفقات: أي دالة `@start()` تقبل معامل
  `localagents_trigger_payload` ستستلمه
</Note>
...
جميع دوال `@start()` في تدفقاتك ستقبل معاملاً إضافياً يسمى `localagents_trigger_payload`:
...
- استخدم `localagents triggers run <trigger_name>` للاختبار محلياً ورؤية هيكل الحمولة بالضبط
- تحقق من أن طاقمك يمكنه التعامل مع معامل `localagents_trigger_payload`
- تأكد من أن طاقمك لا يتوقع معاملات غير مضمنة في حمولة المشغل

**مشاكل التطوير:**

- اختبر دائماً باستخدام `localagents triggers run <trigger>` قبل النشر لرؤية الحمولة الكاملة
- تذكر أن `localagents run` لا يحاكي استدعاءات المشغل — استخدم `localagents triggers run` بدلاً من ذلك
- استخدم `localagents triggers list` للتحقق من المشغلات المتاحة لتكاملاتك المتصلة
- بعد النشر، سيتلقى طاقمك حمولة المشغل الفعلية، لذا اختبر بدقة محلياً أولاً

تحوّل مشغلات الأتمتة عمليات نشر LocalAI إلى أنظمة استجابة مدفوعة بالأحداث يمكنها التكامل بسلاسة مع عمليات عملك وأدواتك الحالية.
