...
اختبر تكامل مشغل Microsoft Teams محلياً باستخدام LocalAI CLI:
...
سينفذ أمر `localagents triggers run` طاقمك بحمولة Teams كاملة، مما يتيح لك اختبار منطق التحليل قبل النشر.

<Warning>
  استخدم `localagents triggers run microsoft_teams/teams_message_created` (وليس `localagents
...
- اختبر محلياً بـ `localagents triggers run microsoft_teams/teams_message_created` لرؤية هيكل الحمولة بالضبط
