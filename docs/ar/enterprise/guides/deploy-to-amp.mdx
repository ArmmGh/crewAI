---
title: "النشر على AMP"
description: "انشر طاقمك أو تدفقك على LocalAI AMP"
icon: "rocket"
mode: "wide"
---
...
<Note>
  ...
  نشره على منصة LocalAI AMP. يغطي هذا الدليل طرق نشر متعددة
...
<Steps>
  <Step title="تثبيت LocalAI CLI">
    إذا لم تكن قد فعلت بالفعل، ثبّت LocalAI CLI:
    ...
    <Tip>
    يأتي CLI مع حزمة LocalAI الرئيسية، لكن الإضافة `[tools]` تضمن حصولك على جميع اعتماديات النشر.
    </Tip>

  </Step>

  <Step title="المصادقة مع منصة Enterprise">
    أولاً، تحتاج لمصادقة CLI مع منصة LocalAI AMP:

    ```bash
    # إذا كان لديك حساب LocalAI AMP بالفعل، أو تريد إنشاء واحد:
    localagents login
    ```
...
يقدم LocalAI CLI عدة أوامر لإدارة عمليات النشر:
...
يمكنك أيضاً نشر طواقمك أو تدفقاتك مباشرة عبر واجهة ويب LocalAI AMP بربط حساب GitHub. لا يتطلب هذا النهج استخدام CLI على جهازك المحلي. تكتشف المنصة تلقائياً نوع مشروعك وتتعامل مع البناء بشكل مناسب.
...
تحتاج لدفع طاقمك إلى مستودع GitHub. إذا لم تكن قد أنشأت طاقماً بعد، يمكنك [اتباع هذا الدليل](/ar/quickstart).
  ...
  <Step title="ربط GitHub بـ LocalAI AMP">

    1. سجّل الدخول إلى [LocalAI AMP](https://app.localagents.com)
...
لعمليات النشر الآلية في خطوط أنابيب CI/CD، يمكنك استخدام LocalAI API لتشغيل إعادة نشر الطواقم الحالية. هذا مفيد بشكل خاص لـ GitHub Actions وJenkins أو سير عمل الأتمتة الأخرى.

<Steps>
  <Step title="الحصول على رمز الوصول الشخصي">

    انتقل إلى إعدادات حساب LocalAI AMP لإنشاء رمز API:

    1. انتقل إلى [app.localagents.com](https://app.localagents.com)
  ...
  <Step title="إيجاد UUID الأتمتة">
    ...
    1. انتقل إلى **Automations** في لوحة تحكم LocalAI AMP
  ...
  <Step title="مثال تكامل GitHub Actions">
    ...
    <Tip>
    أضف `LOCALAGENTS_PAT` و`LOCALAGENTS_AUTOMATION_UUID` كأسرار مستودع. لعمليات نشر PR، أضف تسمية "deploy" لتشغيل سير العمل.
    </Tip>
...
2. **واجهة الويب**: زر [app.localagents.com](https://app.localagents.com) للوصول إلى:
...
**الحل**: تحقق من أن قسم `[tool.localagents]` يتطابق مع نوع مشروعك:
