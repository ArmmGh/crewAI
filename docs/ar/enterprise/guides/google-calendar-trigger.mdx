...
استخدم `localagents_trigger_payload` تماماً كما يتم تسليمه من المشغل حتى يتمكن الطاقم من استخراج الحقول المناسبة.

## الاختبار المحلي

اختبر تكامل مشغل Google Calendar محلياً باستخدام LocalAI CLI:
...
سينفذ أمر `localagents triggers run` طاقمك بحمولة Calendar كاملة، مما يتيح لك اختبار منطق التحليل قبل النشر.

<Warning>
  استخدم `localagents triggers run google_calendar/event_changed` (وليس `localagents run`) لمحاكاة
...
- اختبر محلياً بـ `localagents triggers run google_calendar/event_changed` لرؤية هيكل الحمولة بالضبط
