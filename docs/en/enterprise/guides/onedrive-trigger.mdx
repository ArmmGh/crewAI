...
Test your OneDrive trigger integration locally using the LocalAI CLI:
...
The `localagents triggers run` command will execute your crew with a complete OneDrive payload, allowing you to test your parsing logic before deployment.

<Warning>
  Use `localagents triggers run microsoft_onedrive/file_changed` (not `localagents run`)
...
- Test locally with `localagents triggers run microsoft_onedrive/file_changed` to see the exact payload structure
