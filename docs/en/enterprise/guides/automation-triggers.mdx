...
description: "Understand how LocalAI AMP triggers work, how to manage them, and where to find integration-specific playbooks"
icon: "face-smile"
mode: "wide"
---

LocalAI AMP triggers connect your automations to real-time events across the tools your teams already use. Instead of polling systems or relying on manual kickoffs, triggers listen for changes—new emails, calendar updates, CRM status changes—and immediately launch the crew or flow you specify.
...
<Card title="Salesforce Trigger" icon="salesforce">
  <a href="/en/enterprise/guides/salesforce-trigger">
    Connect Salesforce processes to LocalAI for CRM automation.
  </a>
</Card>
...
<Card title="Slack Trigger" icon="slack">
  ...
  <Card title="Zapier Trigger" icon="bolt">
    <a href="/en/enterprise/guides/zapier-trigger">Bridge LocalAI with thousands of Zapier-supported apps.</a>
  </Card>
</CardGroup>
...
1. Navigate to your deployment in the LocalAI dashboard
...
- Decide whether to pass trigger context automatically using `allow_localagents_trigger_context`
- Set up monitoring—webhook logs, LocalAI execution history, and optional external alerting

### Testing Triggers Locally with CLI

The LocalAI CLI provides powerful commands to help you develop and test trigger-driven automations without deploying to production.
...
```bash
localagents triggers run <trigger_name>
```
...
<Warning>
  **Important Development Notes:**
  - Use `localagents triggers run <trigger>` to simulate trigger execution during development
  - Using `localagents run` will NOT simulate trigger calls and won't pass the trigger payload
...
The crew will automatically receive and can access the trigger payload through the standard LocalAI context mechanisms.

<Note>
  Crew and Flow inputs can include `localagents_trigger_payload`. LocalAI
  automatically injects this payload: - Tasks: appended to the first task's
  description by default ("Trigger Payload: {localagents_trigger_payload}") - Control
  via `allow_localagents_trigger_context`: set `True` to always inject, `False` to
  never inject - Flows: any `@start()` method that accepts a
  `localagents_trigger_payload` parameter will receive it
</Note>
...
All `@start()` methods in your flows will accept an additional parameter called `localagents_trigger_payload`:
...
class MyAutomatedFlow(Flow):
    @start()
    def handle_trigger(self, localagents_trigger_payload: dict = None):
        """
        This start method can receive trigger data
        """
        if localagents_trigger_payload:
            # Process the trigger data
            trigger_id = localagents_trigger_payload.get('id')
            event_data = localagents_trigger_payload.get('payload', {})
...
```python
@start()
def delegate_to_crew(self, localagents_trigger_payload: dict = None):
    """
    ...
    result = crew.crew().kickoff(
        inputs={
            ...
            'localagents_trigger_payload': localagents_trigger_payload
        },
    )
...
- Use `localagents triggers run <trigger_name>` to test locally and see the exact payload structure
- Verify your crew can handle the `localagents_trigger_payload` parameter
- Ensure your crew doesn't expect parameters that aren't included in the trigger payload

**Development issues:**

- Always test with `localagents triggers run <trigger>` before deploying to see the complete payload
- Remember that `localagents run` does NOT simulate trigger calls—use `localagents triggers run` instead
- Use `localagents triggers list` to verify which triggers are available for your connected integrations
- After deployment, your crew will receive the actual trigger payload, so test thoroughly locally first

Automation triggers transform your LocalAI deployments into responsive, event-driven systems that can seamlessly integrate with your existing business processes and tools.
