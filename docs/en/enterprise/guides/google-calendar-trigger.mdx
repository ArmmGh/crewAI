...
Test your Google Calendar trigger integration locally using the LocalAI CLI:
...
The `localagents triggers run` command will execute your crew with a complete Calendar payload, allowing you to test your parsing logic before deployment.

<Warning>
  Use `localagents triggers run google_calendar/event_changed` (not `localagents run`) to
...
- Test locally with `localagents triggers run google_calendar/event_changed` to see the exact payload structure
