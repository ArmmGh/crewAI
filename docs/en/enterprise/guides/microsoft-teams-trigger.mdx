...
Test your Microsoft Teams trigger integration locally using the LocalAI CLI:
...
The `localagents triggers run` command will execute your crew with a complete Teams payload, allowing you to test your parsing logic before deployment.

<Warning>
  Use `localagents triggers run microsoft_teams/teams_message_created` (not `localagents
...
- Test locally with `localagents triggers run microsoft_teams/teams_message_created` to see the exact payload structure
