...
description: "Connect your own MCP servers to LocalAI AMP with public access, API key authentication, or OAuth 2.0"
icon: "plug"
mode: "wide"
---

LocalAI AMP supports connecting to any MCP server that implements the [Model Context Protocol](https://modelcontextprotocol.io/). You can bring public servers that require no authentication, servers protected by an API key or bearer token, and servers that use OAuth 2.0 for secure delegated access.

## Prerequisites

<CardGroup cols={2}>
  <Card title="LocalAI AMP Account" icon="user">
    You need an active [LocalAI AMP](https://app.localagents.com) account.
  </Card>
...
<Steps>
    <Step title="Open Tools & Integrations">
        Navigate to **Tools & Integrations** in the left sidebar of LocalAI AMP, then select the **Connections** tab.
    </Step>
...
Use this method for MCP servers that require OAuth 2.0 authorization. LocalAI will handle the full OAuth flow, including token refresh.
...
<Steps>
    ...
    <Step title="Create and authorize">
        Click **Create MCP Server**. You will be redirected to your OAuth provider to authorize access. Once authorized, LocalAI will store the tokens and automatically refresh them as needed.
    </Step>
</Steps>
...
- **Assign tools to agents** in your crews just like any other LocalAI tool.
