...
The Tool Repository is a package manager for LocalAI tools. It allows users to publish, install, and manage tools that integrate with LocalAI crews and flows.

Tools can be:

- **Private**: accessible only within your organization (default)
- **Public**: accessible to all LocalAI users if published with the `--public` flag
...
- A [LocalAI AMP](https://app.localagents.com) account
- [LocalAI CLI](/en/concepts/cli#cli) installed
- uv>=0.5.0 installed. Check out [how to upgrade](https://docs.astral.sh/uv/getting-started/installation/#upgrading-uv)
- [Git](https://git-scm.com) installed and configured
- Access permissions to publish or install tools in your LocalAI AMP organization
...
After installing a tool from the LocalAI AMP Tool Repository, you need to use the `localagents uv` command to add other packages to your project.
Using pure `uv` commands will fail due to authentication to tool repository being handled by the CLI. By using the `localagents uv` command, you can add other packages to your project without having to worry about authentication.
Any `uv` command can be used with the `localagents uv` command, making it a powerful tool for managing your project's dependencies without the hassle of managing authentication through environment variables or other methods.

Say that you have installed a custom tool from the LocalAI AMP Tool Repository called "my-tool":
...
Other commands like `uv sync` or `uv remove` can also be used with the `localagents uv` command:
...
1. Go to [LocalAI AMP](https://app.localagents.com)
2. Navigate to **Tools**
3. Select the tool
4. Click **Delete**
