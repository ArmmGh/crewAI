...
<Steps>
    ...
    <Step title="Configure LocalAI AMP Connection">
        4. In another tab, open `LocalAI AMP > LLM Connections`. Name your LLM Connection, select Azure as the provider, and choose the same model you selected in Azure.
    ...
    <Step title="Set Default Configuration">
        7. In `LocalAI AMP > Settings > Defaults > Crew Studio LLM Settings`, set the new LLM Connection and model as defaults.
    </Step>
    ...
    <Step title="Configure Network Access">
        8. Ensure network access settings:
            ...
            - Ensure that `Allow access from all networks` is enabled. If this setting is restricted, LocalAI may be blocked from accessing your Azure OpenAI endpoint.
    </Step>
...
- Ensure network access is configured to allow LocalAI connections
- Confirm the deployment model matches what you've configured in LocalAI
