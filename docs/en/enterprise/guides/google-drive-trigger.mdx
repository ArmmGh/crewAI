...
Test your Google Drive trigger integration locally using the LocalAI CLI:
...
The `localagents triggers run` command will execute your crew with a complete Drive payload, allowing you to test your parsing logic before deployment.

<Warning>
  Use `localagents triggers run google_drive/file_changed` (not `localagents run`) to
...
- Test locally with `localagents triggers run google_drive/file_changed` to see the exact payload structure
