...
description: "Install private Python packages from authenticated PyPI registries in LocalAI AMP"
icon: "lock"
mode: "wide"
---

<Note>
  This guide covers how to configure your LocalAI project to install Python packages
  from private PyPI registries (Azure DevOps Artifacts, GitHub Packages, GitLab, AWS CodeArtifact, etc.)
  when deploying to LocalAI AMP.
</Note>
...
LocalAI AMP uses [UV](https://docs.astral.sh/uv/) for dependency resolution and installation.
...
<Warning>
  These environment variables **must** be added via the LocalAI AMP **Environment Variables** settings —
...
Private registry credentials must be configured as environment variables in LocalAI AMP.
...
<Tabs>
  ...
  <Tab title="CLI Deployment">
    Add the variables to your local `.env` file before running `localagents deploy create`.
...
When LocalAI AMP builds your automation, the resolution flow works like this:
