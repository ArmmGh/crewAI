...
Test your Gmail trigger integration locally using the LocalAI CLI:
...
The `localagents triggers run` command will execute your crew with a complete Gmail payload, allowing you to test your parsing logic before deployment.

<Warning>
  Use `localagents triggers run gmail/new_email_received` (not `localagents run`) to
...
- Test locally with `localagents triggers run gmail/new_email_received` to see the exact payload structure
