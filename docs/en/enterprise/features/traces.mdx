...
Traces in LocalAI AMP are detailed execution records that capture every aspect of your crew's operation, from initial inputs to final outputs. They record:
...
<Steps>
  <Step title="Navigate to the Traces Tab">
    Once in your LocalAI AMP dashboard, click on the **Traces** to view all execution records.
  </Step>
...
LocalAI batches trace uploads to reduce overhead on high-volume runs:
