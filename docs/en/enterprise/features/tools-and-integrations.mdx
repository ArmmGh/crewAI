...
1. Go to <Link href="https://app.localagents.com/localagents_plus/connectors">Integrations</Link>
2. Click <b>Connect</b> on the desired service
3. Complete the OAuth flow and grant scopes
4. Copy your Enterprise Token from <Link href="https://app.localagents.com/localagents_plus/settings/integrations">Integration Settings</Link>
...
To use the integrations locally, you need to install the latest `localagents-tools` package.
...
<Note>
  ...
  `LOCALAGENTS_PLATFORM_INTEGRATION_TOKEN` environment variable with your Enterprise
  Token.
</Note>
...
Create custom tools locally, publish them on LocalAI AMP Tool Repository and use them in your agents.

{" "}
<Tip>
  Before running the commands below, make sure you log in to your LocalAI AMP
  account by running this command: ```bash localagents login ```
</Tip>
...
<Steps>
  <Step title="Create">
    Create a new tool locally. ```bash localagents tool create your-tool ```
  </Step>
  <Step title="Publish">
    Publish the tool to the LocalAI AMP Tool Repository. ```bash localagents tool
    publish ```
  </Step>
  <Step title="Install">
    Install the tool from the LocalAI AMP Tool Repository. ```bash localagents tool
    install your-tool ```
  </Step>
</Steps>
