...
icon: "key"
description: Configure enterprise SSO authentication for LocalAI Platform — SaaS and Factory
---

## Overview

LocalAI Platform supports enterprise Single Sign-On (SSO) across both **SaaS (AMP)** and **Factory (self-hosted)** deployments. SSO enables your team to authenticate using your organization's existing identity provider, enforcing centralized access control, MFA policies, and user lifecycle management.
...
LocalAI's managed SaaS platform (AMP) uses **WorkOS** as the default authentication provider. When you sign up at [app.localagents.com](https://app.localagents.com), authentication is handled through `login.localagents.com` — no additional SSO configuration is required.

### Enterprise Custom SSO

Enterprise SaaS customers can configure SSO with their own identity provider (Entra ID, Okta, Auth0). Contact your LocalAI account team to enable custom SSO for your organization. Once configured:
...
3. The LocalAI CLI automatically detects your SSO configuration via `localagents enterprise configure`

### CLI Defaults (SaaS)

| Setting | Default Value |
|---|---|
| `enterprise_base_url` | `https://app.localagents.com` |
| `oauth2_provider` | `workos` |
| `oauth2_domain` | `login.localagents.com` |
...
<Steps>
  <Step title="Register an Application">
    ...
    2. Configure:
       - **Name:** `LocalAI` (or your preferred name)
  ...
  <Step title="Enable CLI Support (Optional)">
    To allow `localagents login` via Device Authorization Grant:

    1. Under **Authentication** → **Advanced settings**, enable **Allow public client flows**
    2. Under **Expose an API**, add an Application ID URI (e.g., `api://localagents-cli`)
    ...
    secrets:
      ...
      ENTRA_ID_CUSTOM_OPENID_SCOPE: "<scope URI, e.g. api://localagents-cli/read>"
    ```
  </Step>
</Steps>
...
<Steps>
  <Step title="Create App Integration">
    ...
    3. Configure:
       - **App integration name:** `LocalAI SSO`
...
<Steps>
  <Step title="Create a Client">
    ...
    2. **Clients** → **Create client**:
       ...
       - **Client ID:** `localagents-factory` (suggested)
    3. Capability config:
  ...
  <Step title="Enable CLI Support (Optional)">
    1. Create a **second** client:
       - **Client type:** OpenID Connect
       - **Client ID:** `localagents-factory-cli` (suggested)
...
The LocalAI CLI supports SSO authentication via the **Device Authorization Grant** flow. This allows developers to authenticate from their terminal without exposing credentials.
...
```bash
localagents enterprise configure https://your-factory-url.app
```
...
<Note>
  Requires LocalAI CLI version **1.6.0** or higher for Entra ID, **0.159.0** or higher for Okta, and **1.9.0** or higher for Keycloak.
</Note>

### Manual CLI Configuration

If you need to configure the CLI manually, use `localagents config set`:

```bash
# Set the provider
localagents config set oauth2_provider okta

# Set provider-specific values
localagents config set oauth2_domain your-domain.okta.com
localagents config set oauth2_client_id your-client-id
localagents config set oauth2_audience api://default

# Set the enterprise base URL
localagents config set enterprise_base_url https://your-factory-url.app
```
...
|---|---|---|
| `enterprise_base_url` | Your LocalAI instance URL | `https://localagents.yourcompany.com` |
...
1. Run `localagents login` — the CLI requests a device code from your IdP
...
LocalAI Platform provides granular RBAC that integrates with your SSO provider.
...
**Symptom:** `localagents login` returns an error or times out.
...
**Symptom:** `localagents enterprise configure` fails to connect.

**Fix:**
- Verify the Factory URL is reachable from your machine
- Check that `enterprise_base_url` is set correctly: `localagents config list`
...
| `ENTRA_ID_CUSTOM_OPENID_SCOPE` | CLI only | Custom scope from "Expose an API" (e.g., `api://localagents-cli/read`) |
...
- [Installation Guide](/installation) — Get started with LocalAI
