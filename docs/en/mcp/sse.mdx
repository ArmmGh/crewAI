---
title: SSE Transport
description: Learn how to connect LocalAI to remote MCP servers using Server-Sent Events (SSE) for real-time communication.
icon: wifi
mode: "wide"
---
...
Server-Sent Events (SSE) provide a standard way for a web server to send updates to a client over a single, long-lived HTTP connection. In the context of MCP, SSE is used for remote servers to stream data (like tool responses) to your LocalAI application in real-time.
