...
description: Learn how to use LocalAI's simple DSL syntax to integrate MCP servers directly with your agents using the mcps field.
icon: code
mode: "wide"
---
...
LocalAI's MCP DSL (Domain Specific Language) integration provides the **simplest way** to connect your agents to MCP (Model Context Protocol) servers. Just add an `mcps` field to your agent and LocalAI handles all the complexity automatically.
...
LocalAI automatically handles tool naming to prevent conflicts:

```python
# Original MCP server has tools: "search", "analyze"
# LocalAI creates tools: "mcp_exa_ai_search", "mcp_exa_ai_analyze"
...
MCP tools work seamlessly with other LocalAI features:
...
# LocalAI will log warnings and continue with other servers
