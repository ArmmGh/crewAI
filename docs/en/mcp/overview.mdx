---
title: "MCP Servers as Tools in LocalAI"
description: "Learn how to integrate MCP servers as tools in your LocalAI agents using the `localagents-tools` library."
icon: plug
mode: "wide"
---
...
LocalAI offers **two approaches** for MCP integration:
...
Perfect for remote HTTPS servers and connected MCP integrations from the LocalAI catalog:
...
```python
from localagents import Agent
from localagents.mcp import MCPServerStdio, MCPServerHTTP, MCPServerSSE
from localagents.mcp.filters import create_static_tool_filter
...
For advanced use cases requiring manual connection management, the `localagents-tools` library provides the `MCPServerAdapter` class.
...
Watch this video tutorial for a comprehensive guide on MCP integration with LocalAI:

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.youtube.com/embed/TpQ45lAZh48"
  title="LocalAI MCP Integration Guide"
  frameBorder="0"
...
LocalAI MCP integration requires the `mcp` library:
...
uv pip install 'localagents-tools[mcp]'
```
...
Connect MCP servers from the LocalAI catalog or bring your own. Once connected in your account, reference them by slug:
...
```python
from localagents.mcp import MCPServerStdio
from localagents.mcp.filters import create_static_tool_filter
...
```python
from localagents.mcp import MCPServerHTTP
...
```python
from localagents.mcp import MCPServerSSE
...
```python
from localagents.mcp import MCPServerStdio, MCPServerHTTP
...
```python
from localagents.mcp import MCPServerStdio
from localagents.mcp.filters import create_static_tool_filter, create_dynamic_tool_filter, ToolFilterContext
...
- 📊 **Transparent Integration**: Works seamlessly with existing LocalAI features
...
For complex scenarios requiring manual connection management, use the `MCPServerAdapter` class from `localagents-tools`. Using a Python context manager (`with` statement) is the recommended approach as it automatically handles starting and stopping the connection to the MCP server.
...
Checkout this repository for full demos and examples of MCP integration with LocalAI! 👇

<Card
  title="GitHub Repository"
  icon="github"
  href="https://github.com/tonykipkemboi/localagents-mcp-demo"
  target="_blank"
>
  LocalAI MCP Demo
</Card>
...
- **Supported Primitives**: Currently, `MCPServerAdapter` primarily supports adapting MCP `tools`.
  Other MCP primitives like `prompts` or `resources` are not directly integrated as LocalAI components through this adapter at this time.
