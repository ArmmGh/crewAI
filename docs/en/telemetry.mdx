---
title: Telemetry
description: Understanding the telemetry data collected by LocalAI and how it contributes to the enhancement of the library.
icon: signal-stream
mode: "wide"
---
...
LocalAI utilizes anonymous telemetry to gather usage statistics with the primary goal of enhancing the library. 
...
Users can disable telemetry by setting the environment variable `LOCALAGENTS_DISABLE_TELEMETRY` to `true` or by setting `OTEL_SDK_DISABLED` to `true` (note that the latter disables all OpenTelemetry instrumentation globally).

### Examples:
```python
# Disable LocalAI telemetry only
os.environ['LOCALAGENTS_DISABLE_TELEMETRY'] = 'true'

# Disable all OpenTelemetry (including LocalAI)
...
|:----------|:------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------|
| Yes       | LocalAI and Python Version                 | Tracks software versions. Example: LocalAI v1.2.3, Python 3.8.10. No personal data. |
...
| Yes       | Crew Deployment attempt using localAI CLI | Includes: The fact a deploy is being made and crew id, and if it's trying to pull logs, no other data. |
