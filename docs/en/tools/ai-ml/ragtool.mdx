...
The `RagTool` is designed to answer questions by leveraging the power of Retrieval-Augmented Generation (RAG) through LocalAI's native RAG system.
...
- **adapter**: Optional. A custom adapter for the knowledge base. If not provided, a LocalAIRagAdapter will be used.
- **config**: Optional. Configuration for the underlying LocalAI RAG system. Accepts a `RagToolConfig` TypedDict with optional `embedding_model` (ProviderSpec) and `vectordb` (VectorDbConfig) keys. All configuration values provided programmatically take precedence over environment variables.
...
Here's how to integrate the `RagTool` with a LocalAI agent:
...
```python Code
from localagents_tools import RagTool
from localagents_tools.tools.rag import RagToolConfig, VectorDbConfig, ProviderSpec
...
The `embedding_model` parameter accepts a `localagents.rag.embeddings.types.ProviderSpec` dictionary with the structure:
...
<AccordionGroup>
  ...
  <Accordion title="Google Generative AI">
    ```python main.py
    from localagents.rag.embeddings.providers.google.types import GenerativeAiProviderSpec
  ...
  <Accordion title="Google Vertex AI">
    ```python main.py
    from localagents.rag.embeddings.providers.google.types import VertexAIProviderSpec
  ...
  <Accordion title="Sentence Transformer">
    ```python main.py
    from localagents.rag.embeddings.providers.sentence_transformer.types import SentenceTransformerProviderSpec
  ...
  <Accordion title="WatsonX (IBM)">
    ```python main.py
    from localagents.rag.embeddings.providers.ibm.types import WatsonXProviderSpec
  ...
  <Accordion title="Custom">
    ...
    **Note:** Custom embedding functions must implement the `EmbeddingFunction` protocol defined in `localagents.rag.core.base_embeddings_callable`. The `__call__` method should accept input data and return embeddings as a list of numpy arrays (or compatible format that will be normalized). The returned embeddings are automatically normalized and validated.
  </Accordion>
</AccordionGroup>
