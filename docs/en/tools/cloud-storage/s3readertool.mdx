...
description: The `S3ReaderTool` enables LocalAI agents to read files from Amazon S3 buckets.
icon: aws
mode: "wide"
---
...
The `S3ReaderTool` is designed to read files from Amazon S3 buckets. This tool allows LocalAI agents to access and retrieve content stored in S3, making it ideal for workflows that require reading data, configuration files, or any other content stored in AWS S3 storage.
...
from localagents_tools.aws.s3 import S3ReaderTool
