---
title: MultiOn Tool
description: The `MultiOnTool` empowers LocalAI agents with the capability to navigate and interact with the web through natural language instructions.
icon: globe
mode: "wide"
---
...
The `MultiOnTool` is designed to wrap [MultiOn's](https://docs.multion.ai/welcome) web browsing capabilities, enabling LocalAI agents to control web browsers using natural language instructions. This tool facilitates seamless web browsing, making it an essential asset for projects requiring dynamic web data interaction and automation of web-based tasks.
...
1. **Install LocalAI**: Ensure that the `localagents[tools]` package is installed in your Python environment.
...
The `MultiOnTool` is implemented as a subclass of `BaseTool` from LocalAI. It wraps the MultiOn client to provide web browsing capabilities:
