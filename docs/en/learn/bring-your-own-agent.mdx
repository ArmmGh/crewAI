...
Interoperability is a core concept in LocalAI. This guide will show you how to bring your own agents that work within a Crew.
...
To integrate an agent from a different framework (e.g., LangGraph, Autogen, OpenAI Assistants) into LocalAI, you need to create a custom adapter by inheriting from `BaseAgentAdapter`. This adapter acts as a compatibility layer, translating between the LocalAI interfaces and the specific requirements of your external agent.
...
2.  **Implement `__init__`**:
    The constructor should call the parent class constructor `super().__init__(**kwargs)` and perform any initialization specific to your external agent. You can use the optional `agent_config` dictionary passed during LocalAI's `Agent` initialization to configure your adapter and the underlying agent.
...
2.  **Implement `configure_tools`**:
    This is the core abstract method you must implement. It receives a list of LocalAI `BaseTool` instances provided to the agent. Your task is to iterate through this list, adapt each `BaseTool` into the format expected by your external framework, and store the converted tools in the `self.converted_tools` list (which is initialized in the base class constructor).
...
## BaseConverter
The `BaseConverterAdapter` plays a crucial role when a LocalAI `Task` requires an agent to return its final output in a specific structured format, such as JSON or a Pydantic model. It bridges the gap between LocalAI's structured output requirements and the capabilities of your external agent.
...
By implementing these methods, your `MyCustomConverterAdapter` ensures that structured output requests from LocalAI tasks are correctly handled by your integrated external agent, improving the reliability and usability of your custom agent within the LocalAI framework.
