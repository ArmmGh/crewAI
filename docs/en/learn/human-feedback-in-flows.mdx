...
description: Learn how to integrate human feedback directly into your LocalAI Flows using the @human_feedback decorator
icon: user-check
mode: "wide"
---
...
<Note>
The `@human_feedback` decorator requires **LocalAI version 1.8.0 or higher**. Make sure to update your installation before using this feature.
</Note>

The `@human_feedback` decorator enables human-in-the-loop (HITL) workflows directly within LocalAI Flows. It allows you to pause flow execution, present output to a human for review, collect their feedback, and optionally route to different listeners based on the feedback outcome.
...
- [Flows Overview](/en/concepts/flows) - Learn about LocalAI Flows
