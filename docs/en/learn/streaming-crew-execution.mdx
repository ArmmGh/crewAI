...
description: Stream real-time output from your LocalAI crew execution
icon: wave-pulse
mode: "wide"
---
...
When streaming is enabled, LocalAI captures LLM responses and tool calls as they happen, packaging them into structured chunks that include context about which task and agent is executing. You can iterate over these chunks in real-time and access the final result once execution completes.
...
By leveraging streaming, you can build more responsive and interactive applications with LocalAI, providing users with real-time visibility into agent execution and results.