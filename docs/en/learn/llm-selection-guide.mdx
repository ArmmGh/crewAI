...
description: "Strategic framework for choosing the right LLM for your LocalAI AI agents and writing effective task and agent definitions"
icon: "brain-circuit"
mode: "wide"
---

## The LocalAI Approach to LLM Selection
...
<Tabs>
  <Tab title="Manager LLM">
    The manager LLM plays a crucial role in hierarchical LocalAI processes, serving as the coordination point for multiple agents and tasks. This model needs to excel at delegation, task prioritization, and maintaining context across multiple concurrent operations.
...
Effective task definition is often more important than model selection in determining the quality of LocalAI outputs. Well-defined tasks provide clear direction and context that enable even modest models to perform well, while poorly defined tasks can cause even sophisticated models to produce unsatisfactory results.
...
Rather than repeating the strategic framework, here's a tactical checklist for implementing your LLM selection decisions in LocalAI:
...
<Steps>
  ...
  <Step title="Validate with Enterprise Testing" icon="test-tube">
    **Once you deploy your agents to production:**
    - Use [LocalAI AMP platform](https://app.localagents.com) to A/B test your model selections
...
## Common LocalAI Model Selection Pitfalls
...
<AccordionGroup>
  <Accordion title="The 'One Model Fits All' Trap" icon="triangle-exclamation">
    ...
    **LocalAI Solution**: Leverage agent-specific LLM configuration to match model capabilities with agent roles:
    ```python
  ...
  <Accordion title="Ignoring Crew-Level vs Agent-Level LLM Hierarchy" icon="shuffle">
    **The Problem**: Not understanding how LocalAI's LLM hierarchy works - crew LLM, manager LLM, and agent LLM settings can conflict or be poorly coordinated.
    ...
    **LocalAI Solution**: Plan your LLM hierarchy strategically:
    ```python
    crew = Crew(
  ...
  <Accordion title="Function Calling Model Mismatch" icon="screwdriver-wrench">
    **The Problem**: Choosing models based on general capabilities while ignoring function calling performance for tool-heavy LocalAI workflows.
    ...
    **LocalAI Solution**: Prioritize function calling capabilities for tool-heavy agents:
    ```python
  ...
  <Accordion title="Premature Optimization Without Testing" icon="gear">
    **The Problem**: Making complex model selection decisions based on theoretical performance without validating with actual LocalAI workflows and tasks.
    ...
    **LocalAI Solution**: Start simple, then optimize based on real performance data:
    ```python
    # Start with this
  ...
  <Accordion title="Overlooking Context and Memory Limitations" icon="brain">
    **The Problem**: Not considering how model context windows interact with LocalAI's memory and context sharing between agents.
    ...
    **LocalAI Solution**: Match context capabilities to crew communication patterns.
...
For teams serious about optimizing their LLM selection, the **LocalAI AMP platform** provides sophisticated testing capabilities that go far beyond basic CLI testing. The platform enables comprehensive model evaluation that helps you make data-driven decisions about your LLM strategy.
...
Go to [app.localagents.com](https://app.localagents.com) to get started!
...
The tables below show a representative sample of current top-performing models across different categories, with guidance on their suitability for LocalAI agents:
...
<Tabs>
  <Tab title="Reasoning & Planning">
    **Best for Manager LLMs and Complex Analysis**
    ...
    | Model | Intelligence Score | Cost ($/M tokens) | Speed | Best Use in LocalAI |
    |:------|:------------------|:------------------|:------|:------------------|
  ...
  <Tab title="Coding & Technical">
    **Best for Development and Tool-Heavy Workflows**
    ...
    | Model | Coding Performance | Tool Use Score | Cost ($/M tokens) | Best Use in LocalAI |
    |:------|:------------------|:---------------|:------------------|:------------------|
  ...
  <Tab title="Speed & Efficiency">
    **Best for High-Throughput and Real-Time Applications**
    ...
    | Model | Speed (tokens/s) | Latency (TTFT) | Cost ($/M tokens) | Best Use in LocalAI |
    |:------|:-----------------|:---------------|:------------------|:------------------|
  ...
  <Tab title="Balanced Performance">
    **Best All-Around Models for General Crews**
    ...
    | Model | Overall Score | Versatility | Cost ($/M tokens) | Best Use in LocalAI |
    |:------|:--------------|:------------|:------------------|:------------------|
