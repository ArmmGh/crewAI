...
description: Learn how to use LocalAI's fingerprinting system to uniquely identify and track components throughout their lifecycle.
icon: fingerprint
mode: "wide"
---
...
Fingerprints in LocalAI provide a way to uniquely identify and track components throughout their lifecycle. Each `Agent`, `Crew`, and `Task` automatically receives a unique fingerprint when created, which cannot be manually overridden.
...
A fingerprint is an instance of the `Fingerprint` class from the `localagents.security` module. Each fingerprint contains:
