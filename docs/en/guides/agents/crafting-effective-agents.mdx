...
At the heart of LocalAI lies the agent - a specialized AI entity designed to perform specific roles within a collaborative framework. While creating basic agents is simple, crafting truly effective agents that produce exceptional results requires understanding key design principles and best practices.
...
The most powerful agents in LocalAI are built on a strong foundation of three key elements:
...
- Explore [LocalAI Flows](/en/guides/flows/first-flow) for more advanced orchestration
