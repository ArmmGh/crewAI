...
description: How to build, package, and publish your own LocalAI-compatible tools to PyPI so any LocalAI user can install and use them.
icon: box-open
mode: "wide"
---
...
LocalAI's tool system is designed to be extended. If you've built a tool that could benefit others, you can package it as a standalone Python library, publish it to PyPI, and make it available to any LocalAI user — no PR to the LocalAI repo required.
...
Every LocalAI tool must satisfy one of two interfaces:

### Option 1: Subclass `BaseTool`

Subclass `localagents.tools.BaseTool` and implement the `_run` method. Define `name`, `description`, and optionally an `args_schema` for input validation.
...
For simpler tools, the `@tool` decorator turns a function into a LocalAI tool. The function **must** have a docstring (used as the tool description) and type annotations.
...
Define a Pydantic model as your `args_schema` to get automatic input validation and clear error messages. If you don't provide one, LocalAI will infer it from your `_run` method's signature.
...
version = "0.1.0"
description = "A LocalAI tool for geolocating street addresses."
...
Declare `localagents` as a dependency so users get a compatible version automatically.
...
- **Package name**: Use the prefix `localagents-` (e.g., `localagents-geolocate`). This makes your tool discoverable when users search PyPI.
- **Module name**: Use underscores (e.g., `localagents_geolocate`).
