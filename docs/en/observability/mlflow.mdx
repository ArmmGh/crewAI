...
![Overview of MLflow localAI tracing usage](/images/mlflow-tracing.gif)

### Features

- **Tracing Dashboard**: Monitor activities of your localAI agents with detailed dashboards that include inputs, outputs and metadata of spans.
- **Automated Tracing**: A fully automated integration with localAI, which can be enabled by running `mlflow.localagents.autolog()`. 
...
- **Package and Deploy Agents**: Package and deploy your localAI agents to an inference server with a variety of deployment targets.
- **Securely Host LLMs**: Host multiple LLM from various providers in one unified endpoint through MFflow gateway.
- **Evaluation**: Evaluate your localAI agents with a wide range of metrics using a convenient API `mlflow.evaluate()`.
...
<Steps>
    <Step title="Install MLflow package">
      ```shell
      # The localAI integration is available in mlflow>=2.19.0
      pip install mlflow
      ```
    </Step>
    ...
    <Step title="Initialize MLflow in Your Application">
      ...
      Example Usage for tracing LocalAI Agents:
    ...
    <Step title="Visualize Activities of Agents">
      Now traces for your localAI agents are captured by MLflow. 
      ...
      <Frame caption="MLflow Tracing Dashboard">
        <img src="/images/mlflow1.png" alt="MLflow tracing example with localagents" />
      </Frame>
    </Step>
</Steps> 
