...
description: Govern, secure, and optimize your LocalAI workflows with LangDB AI Gateway—access 350+ models, automatic routing, cost optimization, and full observability.
icon: database
mode: "wide"
---
...
[LangDB AI Gateway](https://langdb.ai) provides OpenAI-compatible APIs to connect with multiple Large Language Models and serves as an observability platform that makes it effortless to trace LocalAI workflows end-to-end while providing access to 350+ language models. With a single `init()` call, all agent interactions, task executions, and LLM calls are captured, providing comprehensive observability and production-ready AI infrastructure for your applications.

<Frame caption="LangDB LocalAI Trace Example">
  <img src="/images/langdb-1.png" alt="LangDB LocalAI trace example" />
</Frame>
...
- **Automatic Tracing**: Single `init()` call captures all LocalAI interactions
...
<Steps>
  <Step title="Install LangDB">
    Install the LangDB client with LocalAI feature flag:
    ```bash
  ...
  <Step title="Initialize Tracing">
    Import and initialize LangDB before configuring your LocalAI code:
    ```python
  ...
  <Step title="Configure LocalAI with LangDB">
...
Here's a simple example to get you started with LangDB and LocalAI:
...
# Initialize LangDB before any LocalAI imports
init()
...
import sys
from pylangdb.localagents import init
init()  # Initialize LangDB before any LocalAI imports
...
After running your LocalAI application, you can view detailed traces in the LangDB dashboard:

<Frame caption="LangDB Trace Dashboard">
  <img src="/images/langdb-2.png" alt="LangDB trace dashboard showing LocalAI workflow" />
</Frame>
...
- **No traces appearing**: Ensure `init()` is called before any LocalAI imports
...
<CardGroup cols={3}>
  ...
  <Card title="GitHub Examples" icon="github" href="https://github.com/langdb/langdb-samples/tree/main/examples/localagents" >
    Complete LocalAI integration examples
  </Card>
...
This guide covered the basics of integrating LangDB AI Gateway with LocalAI. To further enhance your AI workflows, explore:
