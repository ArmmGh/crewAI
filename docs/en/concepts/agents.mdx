---
title: Agents
description: Detailed guide on creating and managing agents within the LocalAI framework.
icon: robot
mode: "wide"
---
...
In the LocalAI framework, an `Agent` is an autonomous unit that can:
...
LocalAI AMP includes a Visual Agent Builder that simplifies agent creation and configuration without writing code. Design your agents visually and test them in real-time.
...
There are two ways to create agents in LocalAI: using **YAML configuration (recommended)** or defining them **directly in code**.

### YAML Configuration (Recommended)

Using YAML configuration provides a cleaner, more maintainable way to define agents. We strongly recommend using this approach in your LocalAI projects.

After creating your LocalAI project as outlined in the [Installation](/en/installation) section, navigate to the `src/latest_ai_development/config/agents.yaml` file and modify the template to match your requirements.
...
Agents can be equipped with various tools to enhance their capabilities. LocalAI supports tools from:
...
LocalAI includes sophisticated automatic context window management to handle situations where conversations exceed the language model's token limits. This powerful feature is controlled by the `respect_context_window` parameter.

### How Context Window Management Works

When an agent's conversation history grows too large for the LLM's context window, LocalAI automatically detects this situation and can either:
...
This is the **default and recommended setting** for most use cases. When enabled, LocalAI will:
...
- 🔄 **Automatic summarization**: LocalAI intelligently summarizes the conversation history
...
- ❌ **Error message**: `"Context length exceeded. Consider using smaller text or RAG tools from localagents_tools."`
...
<Note>
  ...
  `respect_context_window` to your preferred behavior and LocalAI handles the
  rest!
</Note>
