---
title: Training
description: Learn how to train your LocalAI agents by giving them feedback early on and get consistent results.
icon: dumbbell
mode: "wide"
---
...
The training feature in LocalAI allows you to train your AI agents using the command-line interface (CLI).
By running the command `localagents train -n <n_iterations>`, you can specify the number of iterations for the training process.

During training, LocalAI utilizes techniques to optimize the performance of your agents along with human feedback.
...
```shell
localagents train -n <n_iterations> -f <filename.pkl>
```
<Tip>
...
LocalAI uses the training artifacts in two ways: during training to incorporate your human feedback, and after training to guide agents with consolidated suggestions.

### Training data flow

```mermaid
flowchart TD
  A["Start training<br/>CLI: localagents train -n -f<br/>or Python: crew.train(...)"] --> B["Setup training mode<br/>- task.human_input = true<br/>- disable delegation<br/>- init training_data.pkl + trained file"]
...
- When `train(...)` finishes, LocalAI evaluates the collected training data per agent and produces a consolidated result containing:
...
<Tabs>
  ...
  <Tab title="Small Model Usage">
    ...
    <Warning>
      While LocalAI includes optimizations for small models, expect less reliable and less nuanced evaluation results that may require more human intervention during training.
    </Warning>
  </Tab>
</Tabs>
...
- You can change the output filename when calling `localagents train` with `-f/--filename`. Absolute paths are supported if you want to save outside the CWD.
