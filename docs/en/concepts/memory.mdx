---
title: Memory
description: Leveraging the unified memory system in LocalAI to enhance agent capabilities.
icon: database
mode: "wide"
---
...
This prevents duplicates from accumulating. For example, if you save "LocalAI ensures reliable operation" three times, consolidation recognizes the duplicates and keeps only one record.
...
- **Default**: LanceDB, stored under `./.localagents/memory` (or `$LOCALAGENTS_STORAGE_DIR/memory` if the env var is set, or the path you pass as `storage="path/to/dir"`).
- **Custom backend**: Implement the `StorageBackend` protocol (see `localagents.memory.storage.backend`) and pass an instance to `Memory(storage=your_backend)`.
...
- Ensure the storage path is writable (default `./.localagents/memory`). Pass `storage="./your_path"` to use a different directory, or set the `LOCALAGENTS_STORAGE_DIR` environment variable.
