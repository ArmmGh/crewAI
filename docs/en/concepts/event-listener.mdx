...
description: "Tap into LocalAI events to build custom integrations and monitoring"
icon: spinner
mode: "wide"
---
...
LocalAI provides a powerful event system that allows you to listen for and react to various events that occur during the execution of your Crew. This feature enables you to build custom integrations, monitoring solutions, logging systems, or any other functionality that needs to be triggered based on LocalAI's internal events.

## How It Works

LocalAI uses an event bus architecture to emit events throughout the execution lifecycle. The event system is built on the following components:

1. **LocalAgentsEventsBus**: A singleton event bus that manages event registration and emission
2. **BaseEvent**: Base class for all events in the system
3. **BaseEventListener**: Abstract base class for creating custom event listeners

When specific actions occur in LocalAI (like a Crew starting execution, an Agent completing a task, or a tool being used), the system emits corresponding events. You can register handlers for these events to execute custom code when they occur.

<Note type="info" title="Enterprise Enhancement: Prompt Tracing">
LocalAI AMP provides a built-in Prompt Tracing feature that leverages the event system to track, store, and visualize all prompts, completions, and associated metadata. This provides powerful debugging capabilities and transparency into your agent operations.
...
This is how third-party event listeners are registered in the LocalAI codebase.

## Available Event Types

LocalAI provides a wide range of events that you can listen for:
...
```python
from localagents.events import localagents_event_bus, CrewKickoffStartedEvent
...
4. **Integration**: Connect LocalAI with external systems like monitoring platforms, databases, or notification services
...
By leveraging LocalAI's event system, you can extend its functionality and integrate it seamlessly with your existing infrastructure.
