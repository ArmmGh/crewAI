---
title: Tasks
description: Detailed guide on managing and creating tasks within the LocalAI framework.
icon: list-check
mode: "wide"
---
...
In the LocalAI framework, a `Task` is a specific assignment completed by an `Agent`.

Tasks provide all necessary details for execution, such as a description, the agent responsible, required tools, and more, facilitating a wide range of action complexities.

Tasks within LocalAI can be collaborative, requiring multiple agents to work together. This is managed through the task properties and orchestrated by the Crew's process, enhancing teamwork and efficiency.

<Note type="info" title="Enterprise Enhancement: Visual Task Builder">
LocalAI AMP includes a Visual Task Builder in Crew Studio that simplifies complex task creation and chaining. Design your task flows visually and test them in real-time without writing code.
...
There are two ways to create tasks in LocalAI: using **YAML configuration (recommended)** or defining them **directly in code**.

### YAML Configuration (Recommended)

Using YAML configuration provides a cleaner, more maintainable way to define tasks. We strongly recommend using this approach to define tasks in your LocalAI projects.

After creating your LocalAI project as outlined in the [Installation](/en/installation) section, navigate to the `src/latest_ai_development/config/tasks.yaml` file and modify the template to match your specific task requirements.
...
<Tip>
  Directly specify an `agent` for assignment or let the `hierarchical` LocalAI's
  process decide based on roles, availability, etc.
</Tip>

## Task Output

Understanding task outputs is crucial for building effective AI workflows. LocalAI provides a structured way to handle task results through the `TaskOutput` class, which supports multiple output formats and can be easily passed between tasks.

The output of a task in LocalAI framework is encapsulated within the `TaskOutput` class. This class provides a structured way to access results of a task, including various formats such as raw output, JSON, and Pydantic models.
...
LocalAI supports two types of guardrails:
...
from localagents import TaskOutput, Task
...
```python Code
from localagents import TaskOutput, LLMGuardrail
...
Leverage tools from the [LocalAI Toolkit](https://github.com/joaomdmoura/localagents-tools) and [LangChain Tools](https://python.langchain.com/docs/integrations/tools) for enhanced task performance and agent interaction.
...
In LocalAI, the output of one task is automatically relayed into the next one, but you can specifically define what tasks' output, including multiple, should be used as context for another task.
...
Specifying tools in a task allows for dynamic adaptation of agent capabilities, emphasizing LocalAI's flexibility.
...
These validations help in maintaining the consistency and reliability of task executions within the localAI framework.

## Creating Directories when Saving Files

The `create_directory` parameter controls whether LocalAI should automatically create directories when saving task outputs to files. This feature is particularly useful for organizing outputs and ensuring that file paths are correctly structured, especially when working with complex project hierarchies.

### Default Behavior

By default, `create_directory=True`, which means LocalAI will automatically create any missing directories in the output file path:
...
When `create_directory=False` and the directory doesn't exist, LocalAI will raise a `RuntimeError`:
...
Check out the video below to see how to use structured outputs in LocalAI:

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.youtube.com/embed/dNpKQk5uxHw"
  title="Structured outputs in LocalAI"
  frameBorder="0"
...
Tasks are the driving force behind the actions of agents in LocalAI.
By properly defining tasks and their outcomes, you set the stage for your AI agents to work effectively, either independently or as a collaborative unit.
Equipping tasks with appropriate tools, understanding the execution process, and following robust validation practices are crucial for maximizing LocalAI's potential,
