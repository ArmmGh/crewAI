---
title: Knowledge
description: What is knowledge in LocalAI and how to use it.
icon: book
mode: "wide"
---
...
Knowledge in LocalAI is a powerful system that allows AI agents to access and utilize external information sources during their tasks.
...
LocalAI exposes a provider-neutral RAG client abstraction for vector stores. The default provider is ChromaDB, and Qdrant is supported as well. You can switch providers using configuration utilities.
...
```python Code
from localagents.rag.config.utils import set_rag_config, get_rag_client, clear_rag_config
...
client.add_documents(
    ...
    documents=[{"id": "1", "content": "LocalAI enables collaborative AI agents."}],
)
...
```python Code
from localagents import LLM, Agent, Crew, Process, Task
...
LocalAI supports various types of knowledge sources out of the box:
...
<Info>
**Understanding Knowledge Levels**: LocalAI supports knowledge at both agent and crew levels. This section clarifies exactly how each works, when they're initialized, and addresses common misconceptions about dependencies.
</Info>
...
# Path: ~/.local/share/LocalAI/{project}/knowledge/
...
<Info>
**Understanding Knowledge Storage**: LocalAI automatically stores knowledge sources in platform-specific directories using ChromaDB for vector storage. Understanding these locations and defaults helps with production deployments, debugging, and storage management.
</Info>

### Where LocalAI Stores Knowledge Files

By default, LocalAI uses the same storage system as memory, storing knowledge in platform-specific directories:

#### Default Storage Locations by Platform

**macOS:**
```
~/Library/Application Support/LocalAI/{project_name}/
...
**Linux:**
```
~/.local/share/LocalAI/{project_name}/
└── knowledge/
    ├── chroma.sqlite3
    ├── {collection_id}/
...
**Windows:**
```
C:\Users\{username}\AppData\Local\LocalAI\{project_name}\
└── knowledge\
    ├── chroma.sqlite3
    ├── {collection_id}\
    └── knowledge_{collection}\
...
To see exactly where LocalAI is storing your knowledge files:
...
# Set custom storage location for all LocalAI data
os.environ["LOCALAGENTS_STORAGE_DIR"] = "./my_project_storage"
...
os.environ["LOCALAGENTS_STORAGE_DIR"] = str(knowledge_dir)
...
<Info>
**Default Embedding Provider**: LocalAI defaults to OpenAI embeddings (`text-embedding-3-small`) for knowledge storage, even when using different LLM providers. You can easily customize this to match your setup.
</Info>

#### Understanding Default Behavior
```python
from localagents import Agent, Crew, LLM
from localagents.knowledge.source.string_knowledge_source import StringKnowledgeSource
...
# LocalAI will still use OpenAI embeddings by default for knowledge
...
LocalAI implements an intelligent query rewriting mechanism to optimize knowledge retrieval. When an agent needs to search through knowledge sources, the raw task prompt is automatically transformed into a more effective search query.
...
LocalAI emits events during the knowledge retrieval process that you can listen for using the event system. These events allow you to monitor, debug, and analyze how knowledge is being retrieved and used by your agents.
...
```python
from localagents.events import (
...
LocalAI allows you to create custom knowledge sources for any type of data by extending the `BaseKnowledgeSource` class. Let's create a practical example that fetches and processes space news articles.
...
# Connect to LocalAI's knowledge ChromaDB
knowledge_path = os.path.join(db_storage_path(), "knowledge")
...
chmod -R 755 ~/.local/share/LocalAI/
```
...
print("LOCALAGENTS_STORAGE_DIR:", os.getenv("LOCALAGENTS_STORAGE_DIR"))
...
# CLI commands
# localagents reset-memories --agent-knowledge  # Agent knowledge only
# localagents reset-memories --knowledge        # All knowledge
```

### Clearing Knowledge

If you need to clear the knowledge stored in LocalAI, you can use the `localagents reset-memories` command with the `--knowledge` option.

```bash Command
localagents reset-memories --knowledge
```
...
<AccordionGroup>
  ...
  <Accordion title="One Time Knowledge">
    - With the typical file structure provided by LocalAI, knowledge sources are embedded every time the kickoff is triggered.
    ...
    - Link to the issue to get complete idea [Github Issue](https://github.com/localAIInc/localAI/issues/2755)
  </Accordion>
  ...
  <Accordion title="Production Best Practices">
    - Set `LOCALAGENTS_STORAGE_DIR` to a known location in production
