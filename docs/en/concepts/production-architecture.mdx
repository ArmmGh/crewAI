...
description: Best practices for building production-ready AI applications with LocalAI
icon: server
mode: "wide"
---
...
When building production AI applications with LocalAI, **we recommend starting with a Flow**.
...
3.  **Observability**: Flows provide a clear structure that makes it easier to trace execution, debug issues, and monitor performance. We recommend using [LocalAI Tracing](/en/observability/tracing) for detailed insights. Simply run `localagents login` to enable free observability features.

## The Architecture

A typical production LocalAI application looks like this:
...
Leverage LocalAI's control primitives to add robustness and control to your Crews.
...
### LocalAI Enterprise
The easiest way to deploy your Flow is using LocalAI Enterprise. It handles the infrastructure, authentication, and monitoring for you.
