---
title: 'LLMs'
description: 'A comprehensive guide to configuring and using Large Language Models (LLMs) in your LocalAI projects'
icon: 'microchip-ai'
mode: "wide"
---
...
LocalAI integrates with multiple LLM providers through providers native sdks, giving you the flexibility to choose the right model for your specific use case. This guide will help you understand how to configure and use different LLM providers in your LocalAI projects.


## What are LLMs?

Large Language Models (LLMs) are the core intelligence behind LocalAI agents. They enable agents to understand context, make decisions, and generate human-like responses. Here's what you need to know:

<CardGroup cols={2}>
  <Card title="LLM Basics" icon="brain">
    Large Language Models are AI systems trained on vast amounts of text data. They power the intelligence of your LocalAI agents, enabling them to understand and generate human-like text.
  </Card>
...
There are different places in LocalAI code where you can specify the model to use. Once you specify the model you are using, you will need to provide the configuration (like an API key) for each of the model providers you use. See the [provider configuration examples](#provider-configuration-examples) section for your provider.

<Tabs>
  <Tab title="1. Environment Variables">
    The simplest way to get started. Set the model in your environment directly, through an `.env` file or in your app code. If you used `localagents create` to bootstrap your project, it will be set already.
...
<Info>
  LocalAI provides native SDK integrations for OpenAI, Anthropic, Google (Gemini API), Azure, and AWS Bedrock — no extra install needed beyond the provider-specific extras (e.g. `uv add "localagents[openai]"`).

  All other providers are powered by **LiteLLM**. If you plan to use any of them, add it as a dependency to your project:
  ```bash
  uv add 'localagents[litellm]'
  ```
</Info>

## Provider Configuration Examples

LocalAI supports a multitude of LLM providers, each offering unique features, authentication methods, and model capabilities.
In this section, you'll find detailed examples that help you select, configure, and optimize the LLM that best fits your project's needs.

<AccordionGroup>
  <Accordion title="OpenAI">
    LocalAI provides native integration with OpenAI through the OpenAI Python SDK.
  ...
  <Accordion title="Meta-Llama">
    ...
    The API is available through the [Meta Llama API](https://llama.developer.meta.com?utm_source=partner-localagents&utm_medium=website).
    ...
    Example usage in your LocalAI project:
    ```python Code
    from localagents import LLM
  ...
  <Accordion title="Anthropic">
    LocalAI provides native integration with Anthropic through the Anthropic Python SDK.
    ...
    **Extended Thinking (Claude Sonnet 4 and Beyond):**
    ...
    LocalAI supports Anthropic's Extended Thinking feature, which allows Claude to think through problems in a more human-like way before responding. This is particularly useful for complex reasoning, analysis, and problem-solving tasks.
  ...
  <Accordion title="Google (Gemini API)">
    LocalAI provides native integration with Google Gemini through the Google Gen AI Python SDK.
    ...
    **Note:** To use Google Gemini, install the required dependencies:
    ```bash
    uv add "localagents[google-genai]"
    ```
  ...
  <Accordion title="Azure">
    LocalAI provides native integration with Azure AI Inference and Azure OpenAI through the Azure AI Inference Python SDK.
    ...
    **Note:** To use Azure AI Inference, install the required dependencies:
    ```bash
    uv add "localagents[azure-ai-inference]"
    ```
  </Accordion>

  <Accordion title="AWS Bedrock">
    LocalAI provides native integration with AWS Bedrock through the boto3 SDK using the Converse API.
  ...
  <Accordion title="Local NVIDIA NIM Deployed using WSL2">
    ...
    3. Configure your localagents local models:
...
LocalAI supports streaming responses from LLMs, allowing your application to receive and process outputs in real-time as they're generated.
...
<Tabs>
  ...
  <Tab title="Event Handling">
    LocalAI emits events for each chunk received during streaming:

    ```python
    from localagents.events import (
      LLMStreamChunkEvent
    )
    from localagents.events import BaseEventListener
  ...
  <Tab title="Agent & Task Tracking">
    All LLM events in LocalAI include agent and task information, allowing you to track and filter LLM interactions by specific agents or tasks:

    ```python
    from localagents import LLM, Agent, Task, Crew
    from localagents.events import LLMStreamChunkEvent
    from localagents.events import BaseEventListener
...
LocalAI supports asynchronous LLM calls for improved performance and concurrency in your AI workflows. Async calls allow you to run multiple LLM requests concurrently without blocking, making them ideal for high-throughput applications and parallel agent operations.
...
LocalAI supports structured responses from LLM calls by allowing you to define a `response_format` using a Pydantic model. This enables the framework to automatically parse and validate the output, making it easier to integrate the response into your application without manual post-processing.
...
<AccordionGroup>
  <Accordion title="Context Window Management">
    LocalAI includes smart context management features:

    ```python
    from localagents import LLM

    # LocalAI automatically handles:
  ...
  <Accordion title="Drop Additional Parameters">
    LocalAI internally uses native sdks for LLM calls, which allows you to drop additional parameters that are not needed for your specific use case. This can help simplify your code and reduce the complexity of your LLM configuration.
  ...
  <Accordion title="Transport Interceptors">
    LocalAI provides message interceptors for several providers, allowing you to hook into request/response cycles at the transport layer.
...
import httpx
from localagents import LLM
from localagents.llms.hooks import BaseInterceptor
