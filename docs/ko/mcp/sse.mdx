---
title: SSE 트랜스포트
description: 서버 전송 이벤트(SSE)를 사용하여 LocalAI를 원격 MCP 서버에 연결하여 실시간 통신을 구현하는 방법을 알아보세요.
icon: wifi
mode: "wide"
---
...
Server-Sent Events(SSE)는 웹 서버가 하나의 장기 실행 HTTP 연결을 통해 클라이언트에 업데이트를 전송할 수 있는 표준 방식을 제공합니다. MCP의 맥락에서 SSE는 원격 서버가 데이터(예: tool 응답)를 실시간으로 LocalAI 애플리케이션에 스트리밍하는 데 사용됩니다.
