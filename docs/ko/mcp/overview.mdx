---
title: "LocalAI에서 MCP 서버를 도구로 활용하기"
description: "`localagents-tools` 라이브러리를 사용하여 MCP 서버를 LocalAI agent에 도구로 통합하는 방법을 알아봅니다."
icon: plug
mode: "wide"
---
...
LocalAI는 MCP 통합을 위한 **두 가지 접근 방식**을 제공합니다:
...
수동 연결 관리가 필요한 고급 사용 사례의 경우 `localagents-tools` 라이브러리는 `MCPServerAdapter` 클래스를 제공합니다.
...
LocalAI와 MCP 통합에 대한 종합적인 안내를 위해 이 비디오 튜토리얼을 시청하세요:
...
`localagents-tools`와 함께 MCP를 사용하기 전에, 아래 명령어를 통해 `mcp` 추가 `localagents-tools` 종속성을 설치해야 합니다:
...
`localagents-tools`의 `MCPServerAdapter` 클래스는 MCP 서버에 연결하고 해당 도구들을 LocalAI 에이전트에서 사용할 수 있도록 하는 기본 방법입니다. 다양한 전송 메커니즘을 지원하며 연결 관리를 간소화합니다.
...
LocalAI와의 MCP 통합에 대한 전체 데모와 예제를 보려면 이 저장소를 확인하세요! 👇
...
- **지원되는 프리미티브**: 현재 `MCPServerAdapter`는 주로 MCP `tools`를 어댑팅하는 기능을 지원합니다. 다른 MCP 프리미티브(예: `prompts` 또는 `resources`)는 현재 이 어댑터를 통해 LocalAI 컴포넌트로 직접 통합되어 있지 않습니다.
