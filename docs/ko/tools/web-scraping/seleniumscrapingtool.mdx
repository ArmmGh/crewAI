...
이 도구를 사용하려면 LocalAI tools 패키지와 Selenium을 설치해야 합니다:
...
다음 예시는 `SeleniumScrapingTool`을 LocalAI agent와 함께 사용하는 방법을 보여줍니다:
...
여기서는 `SeleniumScrapingTool`을 LocalAI 에이전트와 통합하는 방법에 대해 더 자세히 설명합니다.
