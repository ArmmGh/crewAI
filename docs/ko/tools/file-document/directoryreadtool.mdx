...
프로젝트에서 DirectoryReadTool을 사용하려면 `localagents_tools` 패키지를 설치해야 합니다. 이 패키지가 아직 환경에 없다면 아래 명령어를 사용하여 pip로 설치할 수 있습니다:

```shell
pip install 'localagents[tools]'
```

이 명령어는 `localagents_tools` 패키지의 최신 버전을 설치하며, DirectoryReadTool을 비롯한 다양한 유틸리티를 사용할 수 있습니다.
