---
title: Apify 액터
description: "`ApifyActorsTool`을(를) 사용하면 Apify 액터를 호출하여 LocalAI 워크플로우에 웹 스크래핑, 크롤링, 데이터 추출 및 웹 자동화 기능을 제공할 수 있습니다."
...
[Apify Actors](https://apify.com/actors)를 LocalAI 워크플로우에 통합합니다.

## 설명

`ApifyActorsTool`은 [Apify Actors](https://apify.com/actors)와 LocalAI 워크플로우를 연결합니다. Apify Actors는 웹 스크래핑 및 자동화를 위한 클라우드 기반 프로그램입니다.
...
자세한 내용은 Apify 문서의 [Apify LocalAI 통합](https://docs.apify.com/platform/integrations/localagents)을 참조하세요.

## 시작 단계

<Steps>
    <Step title="의존성 설치">
        `localagents[tools]`와 `langchain-apify`를 pip으로 설치하세요: `pip install 'localagents[tools]' langchain-apify`.
    </Step>
...
에이전트와 함께 사용하는 예시는 [LocalAI Actor 템플릿](https://apify.com/templates/python-localagents)을 참고하세요.
...
- **[LocalAI 통합 가이드](https://docs.apify.com/platform/integrations/localagents)**: Apify와 LocalAI를 통합하는 공식 가이드를 따라보세요.