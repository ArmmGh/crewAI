...
YoutubeChannelSearchTool을 사용하려면 `localagents_tools` 패키지를 설치해야 합니다. 설치하려면 셸에서 다음 명령어를 실행하세요:
...
다음 예제는 LocalAI 에이전트와 함께 `YoutubeChannelSearchTool`을 사용하는 방법을 보여줍니다:
...
다음은 `YoutubeChannelSearchTool`을 LocalAI 에이전트와 통합하는 방법에 대한 좀 더 자세한 예시입니다:
