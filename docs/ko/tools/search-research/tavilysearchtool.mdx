...
`TavilySearchTool`은 Tavily Search API에 대한 인터페이스를 제공하여 LocalAI 에이전트가 포괄적인 웹 검색을 수행할 수 있도록 합니다. 이 도구는 검색 깊이, 주제, 시간 범위, 포함/제외 도메인, 그리고 결과에 직접 답변, 원시 콘텐츠, 이미지 포함 여부 등을 지정할 수 있게 해줍니다.
...
다음은 LocalAI agent 내에서 `TavilySearchTool`을 초기화하고 사용하는 방법입니다:
