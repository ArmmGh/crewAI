...
이 도구는 `localagents_tools` 패키지의 일부로, Youtube 동영상 콘텐츠 내에서 의미 기반 검색을 수행하도록 설계되었으며 Retrieval-Augmented Generation (RAG) 기술을 활용합니다.
...
`YoutubeVideoSearchTool`을 사용하려면 먼저 `localagents_tools` 패키지를 설치해야 합니다.
...
다음 예시는 `YoutubeVideoSearchTool`을 LocalAI agent와 함께 사용하는 방법을 보여줍니다.
...
아래는 `YoutubeVideoSearchTool`을 LocalAI 에이전트와 통합하는 방법에 대한 보다 자세한 예제입니다.
