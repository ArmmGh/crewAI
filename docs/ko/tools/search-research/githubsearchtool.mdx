...
GithubSearchTool을 사용하려면 먼저 Python 환경에 localagents_tools 패키지가 설치되어 있어야 합니다:

```shell
pip install 'localagents[tools]'
```

이 명령어는 GithubSearchTool과 localagents_tools 패키지에 포함된 기타 도구들을 실행하는 데 필요한 패키지를 설치합니다.
