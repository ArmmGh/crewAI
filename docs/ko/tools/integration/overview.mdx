---
title: "개요"
description: "LocalAI 에이전트를 외부 자동화 및 관리형 AI 서비스와 연결"
icon: "plug"
mode: "wide"
---

통합 도구를 사용하면 에이전트가 다른 자동화 플랫폼이나 관리형 AI 서비스에 작업을 위임할 수 있습니다. 이미 운영 중인 LocalAI Platform 자동화를 호출하거나 Amazon Bedrock과 같은 전문 제공업체에 태스크를 넘겨야 할 때 활용하세요.

## **사용 가능한 도구**

<CardGroup cols={2}>
  <Card title="LocalAI 자동화 실행 도구" icon="robot" href="/ko/tools/integration/localagentsautomationtool">
    실행 중인 LocalAI Platform 자동화를 호출하고 사용자 입력을 전달하며, 결과를 에이전트로 다시 수집합니다.
  </Card>
...
- **자동화 연결**: 한 크루 또는 플로우에서 다른 LocalAI 자동화를 연속 실행
- **엔터프라이즈 핸드오프**: 사내 정책과 가드레일을 담고 있는 Bedrock Agent에 태스크 위임
- **하이브리드 워크플로우**: LocalAI의 추론 능력과 외부의 에이전트 API를 결합
...
- **사용량 모니터링**: LocalAI Platform 로그나 AWS CloudWatch를 통해 할당량 초과와 실패를 조기에 감지하세요
