...
description: "LocalAI AMP에서 인증된 PyPI 레지스트리의 프라이빗 Python 패키지 설치하기"
icon: "lock"
mode: "wide"
---

<Note>
  이 가이드는 LocalAI AMP에 배포할 때 프라이빗 PyPI 레지스트리(Azure DevOps Artifacts, GitHub Packages,
  GitLab, AWS CodeArtifact 등)에서 Python 패키지를 설치하도록 LocalAI 프로젝트를 구성하는 방법을 다룹니다.
</Note>
...
LocalAI AMP는 의존성 해결 및 설치에 [UV](https://docs.astral.sh/uv/)를 사용합니다.
...
<Warning>
  이 환경 변수는 LocalAI AMP **환경 변수** 설정을 통해 **반드시** 추가해야 합니다 —
...
프라이빗 레지스트리 자격 증명은 LocalAI AMP에서 환경 변수로 구성해야 합니다.
두 가지 옵션이 있습니다:
...
<Tabs>
  ...
  <Tab title="CLI 배포">
    `localagents deploy create`를 실행하기 전에 로컬 `.env` 파일에 변수를 추가합니다.
...
LocalAI AMP가 자동화를 빌드할 때, 해결 흐름은 다음과 같이 작동합니다:
