...
LocalAI CLI를 사용하여 Google Calendar 트리거 통합을 로컬에서 테스트하세요:
...
`localagents triggers run` 명령은 완전한 Calendar payload로 크루를 실행하여 배포 전에 파싱 로직을 테스트할 수 있게 해줍니다.

<Warning>
  개발 중에는 `localagents triggers run google_calendar/event_changed`를 사용하세요
  (`localagents run`이 아님). 배포 후에는 크루가 자동으로 트리거 payload를 받습니다.
</Warning>
...
- `localagents triggers run google_calendar/event_changed`로 로컬 테스트하여 정확한 payload 구조를 확인하세요
...
- 주의: 트리거 실행을 시뮬레이션하려면 `localagents triggers run`을 사용하세요 (`localagents run`이 아님)
