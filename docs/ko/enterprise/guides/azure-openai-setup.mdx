...
<Steps>
    ...
    <Step title="LocalAI AMP 연결 구성">
        4. 다른 탭에서 `LocalAI AMP > LLM Connections`를 엽니다. LLM Connection에 이름을 지정하고, 공급자로 Azure를 선택한 다음, Azure에서 선택한 것과 동일한 모델을 선택하세요.
    ...
    <Step title="기본 구성 설정">
        7. `LocalAI AMP > Settings > Defaults > Crew Studio LLM Settings`에서 새 LLM Connection과 모델을 기본값으로 설정합니다.
    </Step>
    ...
    <Step title="네트워크 액세스 구성">
        8. 네트워크 액세스 설정을 확인하세요:
            ...
            - `Allow access from all networks`가 활성화되어 있는지 확인하세요. 이 설정이 제한되어 있으면 LocalAI가 Azure OpenAI 엔드포인트에 접근하지 못할 수 있습니다.
    </Step>
...
- 네트워크 액세스가 LocalAI 연결을 허용하도록 구성되어 있는지 확인하세요
- 배포 모델이 LocalAI에서 구성한 것과 일치하는지 확인하세요
