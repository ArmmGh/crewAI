---
title: "HITL 워크플로우"
description: "LocalAI에서 의사결정 향상을 위한 Human-In-The-Loop 워크플로우 구현 방법을 알아보세요"
icon: "user-check"
mode: "wide"
---

인간-중심(Human-In-The-Loop, HITL)은 인공지능과 인간 전문 지식을 결합하여 의사결정을 강화하고 작업 결과를 향상시키는 강력한 접근 방식입니다. 이 가이드는 LocalAI Enterprise 내에서 HITL을 구현하는 방법을 보여줍니다.

## LocalAI의 HITL 접근 방식

LocalAI는 human-in-the-loop 워크플로우를 구현하기 위한 두 가지 접근 방식을 제공합니다:
...
Flow에서 `@human_feedback` 데코레이터를 사용하면, LocalAI Enterprise는 이메일 주소가 있는 누구나 검토 요청에 응답할 수 있는 **이메일 우선 HITL 시스템**을 제공합니다:
