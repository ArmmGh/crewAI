---
title: "커스텀 MCP 서버"
description: "공개 액세스, API 키 인증 또는 OAuth 2.0을 사용하여 자체 MCP 서버를 LocalAI AMP에 연결하세요"
icon: "plug"
mode: "wide"
---

LocalAI AMP는 [Model Context Protocol](https://modelcontextprotocol.io/)을 구현하는 모든 MCP 서버에 연결할 수 있습니다. 인증이 필요 없는 공개 서버, API 키 또는 Bearer 토큰으로 보호되는 서버, OAuth 2.0을 사용하는 서버를 연결할 수 있습니다.

## 사전 요구사항

<CardGroup cols={2}>
  <Card title="LocalAI AMP 계정" icon="user">
    활성화된 [LocalAI AMP](https://app.localagents.com) 계정이 필요합니다.
  </Card>
...
<Steps>
    <Step title="Tools & Integrations 열기">
        LocalAI AMP 왼쪽 사이드바에서 **Tools & Integrations**로 이동한 후 **Connections** 탭을 선택합니다.
    </Step>
...
OAuth 2.0 인증이 필요한 MCP 서버에 이 방법을 사용합니다. LocalAI가 토큰 갱신을 포함한 전체 OAuth 흐름을 처리합니다.
...
<Steps>
    ...
    <Step title="생성 및 인증">
        **Create MCP Server**를 클릭합니다. OAuth 제공자로 리디렉션되어 접근을 인증합니다. 인증 완료 후 LocalAI가 토큰을 저장하고 필요에 따라 자동으로 갱신합니다.
    </Step>
</Steps>
...
- 다른 LocalAI 도구와 마찬가지로 crew의 **에이전트에 도구를 할당**합니다.
