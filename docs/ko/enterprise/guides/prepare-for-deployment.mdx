---
title: "배포 준비하기"
description: "Crew 또는 Flow가 LocalAI AMP에 배포될 준비가 되었는지 확인하기"
icon: "clipboard-check"
mode: "wide"
---

<Note>
  LocalAI AMP에 배포하기 전에, 프로젝트가 올바르게 구성되어 있는지 확인하는 것이 중요합니다.
...
LocalAI AMP에서 **자동화(automations)**는 배포 가능한 Agentic AI 프로젝트의 총칭입니다. 자동화는 다음 중 하나일 수 있습니다:
...
| **CLI 생성 명령어** | `localagents create crew name` | `localagents create flow name` |
...
`localagents create crew my_crew`를 실행하면 다음 구조를 얻습니다:
...
`localagents create flow my_flow`를 실행하면 다음 구조를 얻습니다:
...
`pyproject.toml`에 올바른 `[tool.localagents]` 섹션이 포함되어야 합니다:
...
LocalAI는 의존성 관리를 위해 `uv`를 사용합니다. `uv.lock` 파일은 재현 가능한 빌드를 보장하며 배포에 **필수**입니다.
...
<Card title="AMP에 배포하기" icon="rocket" href="/ko/enterprise/guides/deploy-to-amp">
  CLI, 웹 인터페이스 또는 CI/CD 통합을 사용하여 Crew 또는 Flow를 LocalAI AMP에
  배포하려면 배포 가이드를 따르세요.
</Card>
