---
title: "AMP에 배포하기"
description: "Crew 또는 Flow를 LocalAI AMP에 배포하기"
icon: "rocket"
mode: "wide"
---

<Note>
  로컬에서 또는 Crew Studio를 통해 Crew나 Flow를 생성한 후, 다음 단계는 이를 LocalAI AMP
...
<Steps>
  <Step title="LocalAI CLI 설치">
    ...
    <Tip>
    CLI는 기본 LocalAI 패키지에 포함되어 있지만, `[tools]` 추가 옵션을 사용하면 모든 배포 종속성을 함께 설치할 수 있습니다.
    </Tip>

  </Step>

  <Step title="Enterprise 플랫폼에 인증">
    먼저, LocalAI AMP 플랫폼에 CLI를 인증해야 합니다:

    ```bash
    # 이미 LocalAI AMP 계정이 있거나 새로 생성하고 싶을 때:
    localagents login
    ```
...
LocalAI CLI는 배포를 관리하기 위한 여러 명령어를 제공합니다:
...
GitHub 계정을 연결하여 LocalAI AMP 웹 인터페이스를 통해 Crew 또는 Flow를 직접 배포할 수도 있습니다. 이 방법은 로컬 머신에서 CLI를 사용할 필요가 없습니다. 플랫폼은 자동으로 프로젝트 유형을 감지하고 적절하게 빌드를 처리합니다.
...
Crew를 GitHub 저장소에 푸시해야 합니다. 아직 Crew를 만들지 않았다면, [이 튜토리얼](/ko/quickstart)을 따라할 수 있습니다.
  ...
  <Step title="GitHub를 LocalAI AMP에 연결하기">

    1. [LocalAI AMP](https://app.localagents.com)에 로그인합니다.
...
CI/CD 파이프라인에서 자동화된 배포를 위해 LocalAI API를 사용하여 기존 crew의 재배포를 트리거할 수 있습니다. 이 방법은 GitHub Actions, Jenkins 또는 기타 자동화 워크플로우에 특히 유용합니다.

<Steps>
  <Step title="개인 액세스 토큰 발급">

    LocalAI AMP 계정 설정에서 API 토큰을 생성합니다:

    1. [app.localagents.com](https://app.localagents.com)으로 이동합니다
  ...
  <Step title="Automation UUID 찾기">
    ...
    1. LocalAI AMP 대시보드에서 **Automations**로 이동합니다
  ...
  <Step title="GitHub Actions 통합 예시">
    ...
    <Tip>
    `LOCALAGENTS_PAT`와 `LOCALAGENTS_AUTOMATION_UUID`를 저장소 시크릿으로 추가하세요. PR 배포의 경우 "deploy" 라벨을 추가하여 워크플로우를 트리거합니다.
    </Tip>
...
2. **웹 인터페이스**: [app.localagents.com](https://app.localagents.com)에 방문하여 다음을 확인할 수 있습니다:
...
**해결책**: `[tool.localagents]` 섹션이 프로젝트 유형과 일치하는지 확인합니다:
...
<Card title="도움이 필요하신가요?" icon="headset" href="mailto:support@localagents.com">
