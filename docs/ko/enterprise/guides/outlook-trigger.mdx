...
LocalAI CLI를 사용하여 Outlook 트리거 통합을 로컬에서 테스트하세요:
...
`localagents triggers run` 명령은 완전한 Outlook payload로 크루를 실행하여 배포 전에 파싱 로직을 테스트할 수 있게 해줍니다.

<Warning>
  개발 중에는 `localagents triggers run microsoft_outlook/email_received`를
...
- `localagents triggers run microsoft_outlook/email_received`로 로컬 테스트하여 정확한 payload 구조를 확인하세요
