...
1. <Link href="https://app.localagents.com/localagents_plus/connectors">
     Integrations
   </Link>
   로 이동
2. 원하는 서비스에서 <b>Connect</b> 클릭
3. OAuth 플로우 완료 및 스코프 승인
4. <Link href="https://app.localagents.com/localagents_plus/settings/integrations">
     통합 설정
   </Link>
...
로컬에서 통합을 사용하려면 최신 `localagents-tools` 패키지를 설치하세요.
...
로컬에서 도구를 만들고, LocalAI AMP 도구 저장소에 게시한 후, 에이전트에서 사용하세요.

{" "}
<Tip>
  아래 명령을 실행하기 전에 LocalAI AMP 계정에 로그인하세요: ```bash localagents login```
</Tip>
...
<Steps>
  <Step title="생성">
    로컬에서 새 도구 생성 ```bash localagents tool create your-tool ```
  </Step>
  <Step title="게시">도구 저장소에 게시 ```bash localagents tool publish ```</Step>
  <Step title="설치">
    도구 저장소에서 설치 ```bash localagents tool install your-tool ```
  </Step>
</Steps>
