---
title: 프로덕션 아키텍처
description: LocalAI로 프로덕션 수준의 AI 애플리케이션을 구축하기 위한 모범 사례
icon: server
mode: "wide"
---
...
LocalAI로 프로덕션 AI 애플리케이션을 구축할 때는 **Flow로 시작하는 것을 권장합니다**.
...
3.  **관측 가능성 (Observability)**: Flows는 실행을 추적하고, 문제를 디버깅하며, 성능을 모니터링하기 쉽게 만드는 명확한 구조를 제공합니다. 자세한 통찰력을 얻으려면 [LocalAI Tracing](/ko/observability/tracing)을 사용하는 것이 좋습니다. `localagents login`을 실행하여 무료 관측 가능성 기능을 활성화하세요.

## 아키텍처

일반적인 프로덕션 LocalAI 애플리케이션은 다음과 같습니다:
...
LocalAI의 Control Primitives를 활용하여 Crew에 견고함과 제어력을 더하세요.
...
### LocalAI Enterprise
Flow를 배포하는 가장 쉬운 방법은 LocalAI Enterprise를 사용하는 것입니다. 인프라, 인증 및 모니터링을 대신 처리합니다.
