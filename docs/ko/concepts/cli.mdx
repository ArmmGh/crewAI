---
title: CLI
description: LocalAI CLI를 사용하여 LocalAI와 상호 작용하는 방법을 알아보세요.
icon: terminal
mode: "wide"
---

<Warning>
  릴리즈 0.140.0부터 LocalAI AOP는 로그인 제공자 마이그레이션 프로세스를
  시작했습니다. 이에 따라 CLI를 통한 인증 흐름이 업데이트되었습니다. Google을
  통해 로그인하거나 2025년 7월 3일 이후에 계정을 생성한 사용자는 이전 버전의
  `localagents` 라이브러리로는 로그인할 수 없습니다.
</Warning>

## 개요

LocalAI CLI는 LocalAI와 상호작용할 수 있는 명령어 세트를 제공하여 crew 및 flow를 생성, 학습, 실행, 관리할 수 있습니다.

## 설치

LocalAI CLI를 사용하려면, LocalAI가 설치되어 있어야 합니다:
...
LocalAI CLI 명령어의 기본 구조는 다음과 같습니다:
...
설치된 LocalAI의 버전을 표시합니다.

```shell Terminal
localagents version [OPTIONS]
```

- `--tools`: (선택 사항) 설치된 LocalAI tools의 버전을 표시합니다.
...
<Note>
  버전 0.103.0부터 `localagents run` 명령은 표준 crew와 flow 모두를 실행하는 데
  사용할 수 있습니다. flow의 경우 pyproject.toml에서 유형을 자동으로 감지하여
  적절한 명령을 실행합니다. 이제 crew와 flow 모두를 실행하는 권장 방법입니다.
</Note>

<Note>
  이 명령들은 LocalAI 프로젝트가 설정된 디렉터리에서 실행해야 합니다. 일부 명령은
  프로젝트 구조 내에서 추가 구성 또는 설정이 필요할 수 있습니다.
</Note>

### 9. Chat

버전 `0.98.0`부터 `localagents chat` 명령어를 실행하면 크루와의 대화형 세션이 시작됩니다. AI 어시스턴트가 크루를 실행하는 데 필요한 입력값을 요청하며 안내합니다. 모든 입력값이 제공되면 크루가 작업을 실행합니다.
...
<Note>
이 명령어들은 LocalAI 프로젝트의 루트 디렉터리에서 실행해야 합니다.
</Note>
<Note>
...
crew 또는 flow를 [LocalAI AMP](https://app.localagents.com)에 배포하세요.

- **인증**: LocalAI AOP에 배포하려면 인증이 필요합니다.
...
<Note>
  이러한 조직 관리 명령어를 사용하려면 LocalAI AOP에 인증되어 있어야 합니다.
</Note>
...
- **Crew 배포**: 인증이 완료되면 crew 또는 flow를 LocalAI AOP에 배포할 수 있습니다.

  ```shell Terminal
  localagents deploy push
  ```

  - LocalAI AMP 플랫폼에서 배포 프로세스를 시작합니다.
...
- **배포 삭제**: 배포를 삭제하려면 다음을 사용합니다:
  ...
  이 명령은 LocalAI AMP 플랫폼에서 배포를 삭제합니다.

- **도움말 명령어**: CLI에 대한 도움말을 보려면 다음을 사용합니다:
  ```shell Terminal
  localagents deploy --help
  ```
  이 명령은 LocalAI Deploy CLI에 대한 도움말 메시지를 표시합니다.

CLI를 사용하여 [LocalAI AMP](http://app.localagents.com)에 crew를 배포하는 단계별 시연은 아래 비디오 튜토리얼을 참조하십시오.
...
`localagents create crew` 명령어를 실행하면, CLI에서 선택할 수 있는 LLM 제공업체 목록이 표시되고, 그 다음으로 선택한 제공업체에 대한 모델 선택이 이어집니다.
...
- `enterprise_base_url`: LocalAI AMP 인스턴스의 기본 URL
...
| 설정                | 값                     | 설명                                                                 |
| :------------------ | :--------------------- | :------------------------------------------------------------------- |
| enterprise_base_url | https://app.localagents.com | LocalAI AMP 인스턴스의 기본 URL                                       |
...
| oauth2_domain       | login.localagents.com       | OAuth2 제공자의 도메인 (예: your-org.auth0.com)                      |
...
<Note>
  설정 값은 `~/.config/localagents/settings.json`에 저장됩니다. 조직 이름과 UUID와
