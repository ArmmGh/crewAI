---
title: 메모리
description: LocalAI의 통합 메모리 시스템을 활용하여 에이전트 역량을 강화합니다.
icon: database
mode: "wide"
---
...
LocalAI는 **통합 메모리 시스템**을 제공합니다 -- 단기, 장기, 엔터티, 외부 메모리 유형을 하나의 지능형 API인 단일 `Memory` 클래스로 대체합니다. 메모리는 저장 시 LLM을 사용하여 콘텐츠를 분석하고(범위, 카테고리, 중요도 추론) 의미 유사도, 최신성, 중요도를 혼합한 복합 점수로 적응형 깊이 recall을 지원합니다.
...
이를 통해 중복이 축적되는 것을 방지합니다. 예를 들어, "LocalAI ensures reliable operation"을 세 번 저장하면 통합이 중복을 인식하고 하나의 레코드만 유지합니다.
...
memory.remember_many([
    ...
    "LocalAI supports complex workflows.",  # 배치 내 중복 제거로 삭제
])
```
...
- **기본값**: LanceDB, `./.localagents/memory` 아래에 저장 (또는 환경 변수가 설정된 경우 `$LOCALAGENTS_STORAGE_DIR/memory`, 또는 `storage="path/to/dir"`로 전달한 경로).
- **사용자 정의 백엔드**: `StorageBackend` 프로토콜을 구현하고(`localagents.memory.storage.backend` 참조) `Memory(storage=your_backend)`에 인스턴스를 전달합니다.
...
**메모리가 유지되지 않나요?**
- 저장 경로에 쓰기 권한이 있는지 확인하세요(기본값 `./.localagents/memory`). 다른 디렉터리를 사용하려면 `storage="./your_path"`를 전달하거나 `LOCALAGENTS_STORAGE_DIR` 환경 변수를 설정하세요.
...
**터미널에서 메모리 탐색:**
```bash
localagents memory                              # TUI 브라우저 열기
localagents memory --storage-path ./my_memory   # 특정 디렉터리 지정
```
