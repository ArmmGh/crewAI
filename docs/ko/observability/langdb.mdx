---
title: LangDB 통합
description: LangDB AI Gateway로 LocalAI 워크플로우를 관리, 보안, 최적화하세요—350개 이상의 모델 액세스, 자동 라우팅, 비용 최적화, 완전한 가시성을 제공합니다.
icon: database
mode: "wide"
---
...
<Frame caption="LangDB LocalAI 추적 예시">
...
### 가시성 및 추적
- **자동 추적**: 단일 `init()` 호출로 모든 LocalAI 상호작용을 캡처
...
<Steps>
  <Step title="LangDB 설치">
    LocalAI 기능 플래그와 함께 LangDB 클라이언트를 설치하세요:
    ```bash
  ...
  <Step title="추적(Tracing) 초기화">
    LocalAI 코드를 설정하기 전에 LangDB를 임포트하고 초기화하세요:
    ```python
  ...
  <Step title="LocalAI와 LangDB 연동 설정">
...
여기 LangDB와 LocalAI를 시작하는 간단한 예제가 있습니다:
...
LocalAI 애플리케이션을 실행한 후, LangDB 대시보드에서 자세한 트레이스를 확인할 수 있습니다:

<Frame caption="LangDB 트레이스 대시보드">
  <img src="/images/langdb-2.png" alt="LangDB 트레이스 대시보드에서 LocalAI 워크플로우 표시" />
</Frame>
...
- **추적이 나타나지 않음**: `init()`이 LocalAI 임포트 이전에 호출되었는지 확인하세요
...
<CardGroup cols={3}>
  ...
  <Card title="GitHub 예제" icon="github" href="https://github.com/langdb/langdb-samples/tree/main/examples/localagents" >
    LocalAI 통합 전체 예제
  </Card>
...
이 가이드에서는 LangDB AI Gateway를 LocalAI와 통합하는 기본 사항을 다루었습니다. AI 워크플로우를 더욱 강화하려면 다음을 탐색해보세요:
