...
![MLflow localAI 트레이싱 사용 개요](/images/mlflow-tracing.gif)

### 기능

- **트레이싱 대시보드**: localAI 에이전트의 활동을 입력값, 출력값, 스팬의 메타데이터와 함께 자세한 대시보드로 모니터링할 수 있습니다.
- **자동 트레이싱**: 완전 자동화된 localAI 통합 기능으로, `mlflow.localagents.autolog()`를 실행하여 활성화할 수 있습니다.
...
- **에이전트 패키징 및 배포**: localAI 에이전트를 다양한 배포 대상으로 추론 서버에 패키징 및 배포할 수 있습니다.
- **LLM을 안전하게 호스팅**: 여러 공급자의 LLM을 MFflow 게이트웨이를 통해 하나의 통합 엔드포인트에서 호스팅할 수 있습니다.
- **평가**: 편리한 API `mlflow.evaluate()`를 사용하여 다양한 지표로 localAI 에이전트를 평가할 수 있습니다.
...
<Steps>
    <Step title="MLflow 패키지 설치">
      ```shell
      # localAI 연동은 mlflow>=2.19.0 에서 사용할 수 있습니다.
      pip install mlflow
      ```
    </Step>
    ...
    <Step title="에이전트 활동 시각화">
      이제 localAI agents의 추적 정보가 MLflow에서 캡처됩니다.
