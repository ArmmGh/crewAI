---
title: 커스텀 도구 배포하기
description: PyPI에 게시할 수 있는 LocalAI 호환 도구를 빌드, 패키징, 배포하는 방법을 안내합니다.
icon: box-open
mode: "wide"
---
...
LocalAI의 도구 시스템은 확장 가능하도록 설계되었습니다. 다른 사용자에게도 유용한 도구를 만들었다면, 독립적인 Python 라이브러리로 패키징하여 PyPI에 게시하고 모든 LocalAI 사용자가 사용할 수 있도록 할 수 있습니다. LocalAI 저장소에 PR을 보낼 필요가 없습니다.
...
모든 LocalAI 도구는 다음 두 가지 인터페이스 중 하나를 충족해야 합니다:

### 옵션 1: `BaseTool` 서브클래싱

`localagents.tools.BaseTool`을 서브클래싱하고 `_run` 메서드를 구현합니다. `name`, `description`, 그리고 선택적으로 입력 검증을 위한 `args_schema`를 정의합니다.
...
간단한 도구의 경우, `@tool` 데코레이터로 함수를 LocalAI 도구로 변환할 수 있습니다. 함수에는 반드시 독스트링(도구 설명으로 사용됨)과 타입 어노테이션이 있어야 합니다.
...
Pydantic 모델을 `args_schema`로 정의하면 자동 입력 검증과 명확한 에러 메시지를 받을 수 있습니다. 제공하지 않으면 LocalAI가 `_run` 메서드의 시그니처에서 추론합니다.
...
version = "0.1.0"
description = "도로명 주소를 지오코딩하는 LocalAI 도구."
...
사용자가 자동으로 호환 버전을 받을 수 있도록 `localagents`를 의존성으로 선언합니다.
...
- **패키지 이름**: `localagents-` 접두사를 사용합니다 (예: `localagents-geolocate`). PyPI에서 검색할 때 도구를 쉽게 찾을 수 있습니다.
- **모듈 이름**: 밑줄을 사용합니다 (예: `localagents_geolocate`).
