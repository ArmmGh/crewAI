---
title: 커스텀 LLM 구현
description: LocalAI에서 커스텀 LLM 구현을 만드는 방법을 알아보세요.
icon: code
mode: "wide"
---
...
LocalAI는 `BaseLLM` 추상 기반 클래스를 통해 커스텀 LLM 구현을 지원합니다. 이를 통해 LiteLLM에 내장 지원이 없는 모든 LLM 제공자를 통합하거나, 커스텀 인증 메커니즘을 구현할 수 있습니다.
...
이 가이드는 LocalAI에서 커스텀 LLM을 구현하는 주요 사항을 다룹니다.